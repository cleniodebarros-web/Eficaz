...
type
  TFrmNotas_Canceladas = class(TForm)
    Panel1: TPanel;
    btnRetorna: TBitBtn;
...
var
  FrmNotas_Canceladas: TFrmNotas_Canceladas;
...
procedure TFrmNotas_Canceladas.btnEstornaClick(Sender: TObject);
var
Id_Trans: Integer;
Num_Doc: String;
Vl_Trans: Real;
...
procedure TFrmNotas_Canceladas.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmNotas_Canceladas.btnSelecionarClick(Sender: TObject);
begin
 ...
 Begin
  ...
  QNotas.SQL.Add('FROM(SELECT NOTAS_CANCELADAS.TRANSACAO_ID, NO_DOC_FISCAL,DT_SAIDA,HR_SAIDA, CLIENTES.NOME, TRANSACOES.CFOP, TRANSACOES.VALOR,NOTAS_CANCELADAS.CHAVE_NFE,PROTOCOLO,RECIBO,SINTEGRA, NOTAS_CANCELADAS.EMPRESA_ID FROM NOTAS_CANCELADAS ');
  QNotas.SQL.Add('INNER JOIN TRANSACOES ON TRANSACOES.TRANSACAO_ID = NOTAS_CANCELADAS.TRANSACAO_ID');
  QNotas.SQL.Add('INNER JOIN CLIENTES ON CLIENTES.CLIENTE_ID = TRANSACOES.CLIENTE_ID');
  QNotas.SQL.Add('UNION ALL');
  QNotas.SQL.Add('SELECT NOTAS_CANCELADAS.TRANSACAO_ID , NO_DOC_FISCAL,DT_SAIDA,HR_SAIDA, FORNECEDORES.NOME, TRANSACOES.CFOP, TRANSACOES.VALOR,NOTAS_CANCELADAS.CHAVE_NFE,PROTOCOLO,RECIBO,SINTEGRA, NOTAS_CANCELADAS.EMPRESA_ID FROM NOTAS_CANCELADAS');
  QNotas.SQL.Add('INNER JOIN TRANSACOES ON TRANSACOES.TRANSACAO_ID = NOTAS_CANCELADAS.TRANSACAO_ID');
 ...
 begin
  ...
  QNotas.SQL.Add('FROM NOTAS_CANCELADAS');
...
procedure TFrmNotas_Canceladas.DtmaiKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    btnSelecionarClick(Sender);
end;

procedure TFrmNotas_Canceladas.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmNotas_Canceladas.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmNotas_Canceladas.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmNotas_Canceladas.FormCreate(Sender: TObject);
begin
...
procedure TFrmNotas_Canceladas.NotasDblClick(Sender: TObject);
begin
  QRel.Sql.Clear;
  QRel.Sql.Add('SELECT * FROM TRANSACOES');
  QRel.Sql.Add('WHERE');
...
procedure TFrmNotas_Canceladas.NotasKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmNotas_Canceladas.NotasKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    QRel.Sql.Clear;
...
procedure TFrmNotas_Canceladas.QNotasAfterOpen(DataSet: TDataSet);
begin
