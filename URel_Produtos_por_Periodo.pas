unit URel_Produtos_por_Periodo;
...
type
  TFrmRel_Produtos_por_Periodo = class(TForm)
    Bevel1: TBevel;
    ...
    Chk_Movimentacao: TCheckBox;
    Label10: TLabel;
    Selecao: TComboBox;
    Cmb_Movimento: TComboBox;
    Label11: TLabel;
    ...
    procedure DtmaiKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    procedure DtmaiEnter(Sender: TObject);
...
var
  FrmRel_Produtos_por_Periodo: TFrmRel_Produtos_por_Periodo;
  Pagina, linha, C1, C2, C3, C4, C5, C6: Integer;
  Condicao : String;
...
procedure TFrmRel_Produtos_por_Periodo.btnExecutaClick(Sender: TObject);
var
Data : string;
Subtotal_Qtd, Subtotal_Valor, Total: Real;
begin
try
          QRel.Sql.Clear;

          if Cmb_movimento.Text = 'Compra' Then
          Begin
             if Chk_Movimentacao.Checked then
             begin
             QRel.SQL.Add('SELECT PRODUTO_ID, DESCRICAO, COD_BARRA, DT_COMPRA, QUANTIDADE_C ESTOQUE FROM PRODUTOS WHERE PRODUTOS.DT_COMPRA BETWEEN :DT_INICIAL AND :DT_FINAL');
             QRel.Sql.Add('AND PRODUTOS.STATUS = :STATUS');
             end
             Else
             Begin
             QRel.SQL.Add('SELECT PRODUTO_ID, DESCRICAO, COD_BARRA, DT_COMPRA, QUANTIDADE_C ESTOQUE FROM PRODUTOS WHERE PRODUTOS.PRODUTO_ID NOT IN(');
             QRel.SQL.Add('SELECT PRODUTO_ID FROM PRODUTOS WHERE PRODUTOS.DT_COMPRA BETWEEN :DT_INICIAL AND :DT_FINAL)');
             QRel.Sql.Add('AND PRODUTOS.STATUS = :STATUS AND DT_COMPRA <= :DT_FINAL ');
             End;
          End
          Else
          Begin
             if Chk_Movimentacao.Checked then
             begin
             QRel.SQL.Add('SELECT PRODUTO_ID, DESCRICAO, COD_BARRA, DT_MOVIMENTO, QUANTIDADE_C ESTOQUE FROM PRODUTOS WHERE PRODUTOS.DT_MOVIMENTO BETWEEN :DT_INICIAL AND :DT_FINAL');
             QRel.Sql.Add('AND PRODUTOS.STATUS = :STATUS');
             end
             Else
             Begin
             QRel.SQL.Add('SELECT PRODUTO_ID, DESCRICAO, COD_BARRA, DT_MOVIMENTO, QUANTIDADE_C ESTOQUE FROM PRODUTOS WHERE PRODUTOS.PRODUTO_ID NOT IN(');
             QRel.SQL.Add('SELECT PRODUTO_ID FROM PRODUTOS WHERE PRODUTOS.DT_MOVIMENTO BETWEEN :DT_INICIAL AND :DT_FINAL)');
             QRel.Sql.Add('AND PRODUTOS.STATUS = :STATUS AND DT_MOVIMENTO <= :DT_FINAL');
             End;
          ...
          End;
           ...
           if Selecao.Text = 'Igual a 0' then
            QRel.Sql.Add('AND (PRODUTOS.QUANTIDADE_C = 0)')
          else if Selecao.Text = 'Maior do que 0' then
            QRel.Sql.Add('AND (PRODUTOS.QUANTIDADE_C > 0)')
          else if Selecao.Text = 'Menor do que 0' then
            QRel.Sql.Add('AND (PRODUTOS.QUANTIDADE_C < 0)')
          else if Selecao.Text = 'Todos' then
            QRel.Sql.Add('AND ((PRODUTOS.QUANTIDADE_C < 0) OR (PRODUTOS.QUANTIDADE_C = 0) OR (PRODUTOS.QUANTIDADE_C > 0))')
          else
            QRel.Sql.Add('AND (PRODUTOS.QUANTIDADE_C <> 0)');
         ...
         QRel.ParamByName('TIPO_ITEM').AsString := TIPO_ITEM.Text;

         if Ordem.Text  = 'Descrição' then
            QRel.SQL.Add('ORDER BY PRODUTOS.DESCRICAO')
         else
            if Cmb_movimento.Text = 'Compra' Then
            QRel.SQL.Add('ORDER BY PRODUTOS.DT_COMPRA')
            Else
             QRel.SQL.Add('ORDER BY PRODUTOS.DT_MOVIMENTO');
      ...
      Begin
            ...
            Begin
                 ...
                 RDprint1.ImpF(linha, C2, Copy(AllTrim(QRel.FieldByName('COD_BARRA').AsString), 1, 14), []);
                 RDprint1.ImpF(linha, C3, Copy(AllTrim(QRel.FieldByName('DESCRICAO').AsString), 1, 35), []);
                 if Cmb_movimento.Text = 'Compra' Then
                 RDprint1.ImpF(linha, C5, DateToStr(QRel.FieldByName('DT_COMPRA').AsDateTime), [])
                 ELSE
                 RDprint1.ImpF(linha, C5, DateToStr(QRel.FieldByName('DT_MOVIMENTO').AsDateTime), []);

                 RDprint1.ImpVal(linha,C4,'##0.00',QRel.FieldByName('ESTOQUE').AsFloat,[]);
...
procedure TFrmRel_Produtos_por_Periodo.RDprint1NewPage(Sender: TObject;
  Pagina: Integer);
begin
           //posição das colulas
           C1  := 01;
          ...
          RDprint1.imp(03, 01, 'Período:'+ datetoStr(dtmen.date) + ' à ' + datetoStr(dtmai.date));
           ...
           Begin
           if Chk_Movimentacao.Checked then
           RDprint1.impC(04, 40, '*** PRODUTOS COM MOVIMENTAÇÃO DE COMPRAS  ***',[negrito])
           Else
           RDprint1.impC(04, 40, '*** PRODUTOS SEM MOVIMENTAÇÃO DE COMPRAS  ***',[negrito])
           End
           Else
           Begin
            if Chk_Movimentacao.Checked then
            RDprint1.impC(04, 40, '*** PRODUTOS COM MOVIMENTAÇÃO DE VENDAS ***',[negrito])
            Else
            RDprint1.impC(04, 40, '*** PRODUTOS SEM MOVIMENTAÇÃO DE VENDAS ***',[negrito])
           End;


          RDprint1.LinhaH(05,01,80,0,false);
          RDprint1.ImpF(06, C1, 'ID',[negrito]);
          RDprint1.ImpF(06, C2, 'COD. BARRAS', [negrito]);
          RDprint1.ImpF(06, C3, 'DESCRIÇÃO', [negrito]);
          RDprint1.ImpF(06, C4, 'ESTQ.', [negrito]);
          RDprint1.ImpF(06, C5 - 3, 'ULT. MOVIMENTO', [negrito]);
...
procedure TFrmRel_Produtos_por_Periodo.DetailSearch(Tabela: String);
begin
...
procedure TFrmRel_Produtos_por_Periodo.DtmaiEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmRel_Produtos_por_Periodo.DtmaiKeyDown(Sender: TObject;
...
procedure TFrmRel_Produtos_por_Periodo.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmRel_Produtos_por_Periodo.DtmenKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if (Key = Vk_Return) or (Key = Vk_Down) then
   // Perform(Wm_NextDlgctl, 0, 0);
...
procedure TFrmRel_Produtos_por_Periodo.SubTipoExit(Sender: TObject);
begin
     DetailSearch('Subtipo');
end;

procedure TFrmRel_Produtos_por_Periodo.TributacaoExit(Sender: TObject);
begin
  DetailSearch('Tributação');
end;

procedure TFrmRel_Produtos_por_Periodo.btnFornecedorClick(Sender: TObject);
begin
   try
...
procedure TFrmRel_Produtos_por_Periodo.btnGrupoClick(Sender: TObject);
begin
 try
...
procedure TFrmRel_Produtos_por_Periodo.btnLocalizacaoClick(Sender: TObject);
begin
  try
...
procedure TFrmRel_Produtos_por_Periodo.btnProd_finalClick(Sender: TObject);
begin
    try
...
procedure TFrmRel_Produtos_por_Periodo.btnProd_InicialClick(Sender: TObject);
begin
  try
...
procedure TFrmRel_Produtos_por_Periodo.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmRel_Produtos_por_Periodo.btnSub_TipoClick(Sender: TObject);
begin
 try
...
procedure TFrmRel_Produtos_por_Periodo.BtnTributacaoClick(Sender: TObject);
begin
  try
...
procedure TFrmRel_Produtos_por_Periodo.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmRel_Produtos_por_Periodo.FormCreate(Sender: TObject);
begin
  Dtmen.Date := StrToDate('01/' + Copy(DateToStr(date), 4, 7));
  Dtmai.Date := Ult_Dia_Mes(date);
end;

procedure TFrmRel_Produtos_por_Periodo.FormShow(Sender: TObject);
begin
...
procedure TFrmRel_Produtos_por_Periodo.FornecedorExit(Sender: TObject);
begin
 DetailSearch('Fornecedor');
end;

procedure TFrmRel_Produtos_por_Periodo.FornecedorKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
...
procedure TFrmRel_Produtos_por_Periodo.GrupoExit(Sender: TObject);
begin
 DetailSearch('Grupo');
end;

procedure TFrmRel_Produtos_por_Periodo.GrupoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if (Key = Vk_F7) and (Sender = Fornecedor) then
    btnFornecedorClick(Self);
...
procedure TFrmRel_Produtos_por_Periodo.LocalizacaoExit(Sender: TObject);
begin
