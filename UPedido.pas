...
uses
  ...
  IBUpdateSQL, Math, QRCtrls, QuickRpt, RDprint, xmldom, XMLIntf, msxmldom,
  XMLDoc, FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param,
...
procedure TFrmPedido.PedidoBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TFrmPedido.QTabelaAfterOpen(DataSet: TDataSet);
begin
...
procedure TFrmPedido.BtnImportXMLClick(Sender: TObject);
var
I : Integer;
FileName, CNPJ:string;
...
begin
   ...
   begin
     Application.MessageBox('Informe o funcionário.', '', MB_OK +MB_ICONWARNING);

     if Funcionario_id.CanFocus then
        Funcionario_id.SetFocus;
  ...
  begin
    XMLDOM.FileName := OpenDialog1.FileName;
    XMLDOM.Active := True;

    FileName := ExtractFileName(OpenDialog1.FileName);
    CNPJ     := Formata_CNPJ(Copy(FileName, 1, 14));

    IQuery.SQL.Clear;
    IQuery.SQL.Add('SELECT * FROM FORNECEDORES WHERE CNPJ = :CNPJ');
    IQuery.ParamByName('CNPJ').AsString := CNPJ;
    IQuery.Prepare;
    IQuery.Open;

    FORNECEDOR_ID.Value := IQuery.FieldByName('FORNECEDOR_ID').AsInteger;
    CEP.Text            := IQuery.FieldByName('CEP').AsString;
    MUNICIPIO.Text      := IQuery.FieldByName('MUNICIPIO').AsString;
    ENDERECO.Text       := IQuery.FieldByName('ENDERECO').AsString;
    BAIRRO.Text         := IQuery.FieldByName('BAIRRO').AsString;
    ESTADO.Text         := IQuery.FieldByName('ESTADO').AsString;
    NOME.Text           := IQuery.FieldByName('NOME').AsString;

    btnSave.Click;
    Sleep(2000);

    for I := 0 to XMLDOM.DocumentElement.ChildNodes.Count - 1 do
    begin

      if XMLDOM.DocumentElement.ChildNodes[I].ChildNodes['CODIGO_BARRAS'].Text <> '0000000000000' then
      begin

        IQuery.SQL.Clear;
        IQuery.SQL.Add('SELECT PRODUTO_ID, PRODUTOS.DESCRICAO, UNIDADE, PRODUTOS.TRIBUTO_ID, PRODUTOS.ALIQUOTA_ICMS,');
        IQuery.SQL.Add('TRIBUTOS.CSOSN, CONCAT(SUBSTR(ORIGEM,1,1), SUBSTR(TRIBUTACAO,1,2)) CST');
        IQuery.SQL.Add('FROM PRODUTOS');
        IQuery.SQL.Add('inner join TRIBUTOS on TRIBUTOS.tributo_id = PRODUTOS.tributo_id');
        IQuery.SQL.Add('WHERE COD_BARRA = :COD_BARRA');
        IQuery.ParamByName('COD_BARRA').AsString  :=  XMLDOM.DocumentElement.ChildNodes[I].ChildNodes['CODIGO_BARRAS'].Text;
        IQuery.Prepare;
        IQuery.Open;
        ...
        begin
          ...
          QSub_Detail.FieldByName('PRODUTO_ID').AsInteger    := IQuery.FieldByName('PRODUTO_ID').AsInteger;
          QSub_Detail.FieldByName('DESCRICAO').AsString      := IQuery.FieldByName('DESCRICAO').AsString;
          QSub_Detail.FieldByName('QUANTIDADE').AsFloat      := StrToFloat(StringReplace(XMLDOM.DocumentElement.ChildNodes[I].ChildNodes['QUANTIDADE'].Text, '.', ',', []));
          QSub_Detail.FieldByName('UNIDADE').AsString        := IQuery.FieldByName('UNIDADE').AsString;
          QSub_Detail.FieldByName('VR_UNITARIO').AsFloat     := StrToFloat(StringReplace(XMLDOM.DocumentElement.ChildNodes[I].ChildNodes['PRECO'].Text, '.', ',', []));
          QSub_Detail.FieldByName('VR_TOTAL').AsFloat        := StrToFloat(StringReplace(XMLDOM.DocumentElement.ChildNodes[I].ChildNodes['SUBTOTAL'].Text, '.', ',', []));
          QSub_Detail.FieldByName('TRIBUTO_ID').AsInteger    := IQuery.FieldByName('TRIBUTO_ID').AsInteger;
          QSub_Detail.FieldByName('ALIQUOTA_ICMS').AsFloat   := IQuery.FieldByName('ALIQUOTA_ICMS').AsFloat;
          if FrmPrincipal.QEmpresa.FieldByName('CRT').AsString = '1 - SIMPLES NACIONAL' then
            QSub_Detail.FieldByName('CST').AsString          := IQuery.FieldByName('CSOSN').AsString
          else
            QSub_Detail.FieldByName('CST').AsString          := IQuery.FieldByName('CST').AsString;

          QSub_Detail.Post;
        end
        else
          ShowMessage('PRODUTO COM CÓDIGO DE BARRAS '+ XMLDOM.DocumentElement.ChildNodes[I].ChildNodes['CODIGO_BARRAS'].Text +' NÃO FOI LOCALIZADO');

      end
      else
        ShowMessage('PRODUTO '+ XMLDOM.DocumentElement.ChildNodes[I].ChildNodes['PRODUTO'].Text +' COM CÓDIGO DE BARRAS INVÁLIDO');
