...
var
X,  Comando, Dividir: Integer;
MyFile: TextFile;
Vr_Fin, Vr_Troco, Meio, Valor,VLC, Vr_Sangria_Resumida, Vr_Suprimento_Resumido: Real;
...
begin
  try
    ...
    QRel.Sql.Clear;
    QRel.Sql.Add('SELECT DATA, CAST(HORA as TIME) AS HORA, CUPOM, LEGENDA, SUM(VALOR_CUPOM), SUM(VALOR_FINALIZADORA) VALOR_FINALIZADORA, SUM(DESCONTO) DESCONTO, SUM(TROCO) TROCO');
    QRel.Sql.Add('FROM (');
    QRel.Sql.Add('SELECT DATA, HORA, CUPOM, LEGENDA, CASE WHEN (VALOR_CUPOM ) = VALOR_FINALIZADORA');
   ...
   // QRel.SQL.Add('INNER JOIN TRANSACOES ON(TRANSACOES.TRANSACAO_ID=CAST(FECHAMENTO_CUPOM.CUPOM AS INTEGER))');
    QRel.Sql.Add('WHERE');
  ...
  //  QRel.Sql.Add('AND (LEGENDA <> :LEGENDA)');
   ...
   { QRel.Sql.Add('SELECT DATA , HORA,  CUPOM , LEGENDA , VALOR_CUPOM, VALOR_FINALIZADORA, DESCONTO, 0 TROCO');
    QRel.Sql.Add('FROM FECHAMENTO_CUPOM');
    QRel.Sql.Add('WHERE');
    QRel.Sql.Add('(DATA BETWEEN :DT_INICIAL AND :DT_FINAL)');
  ...
  //  QRel.Sql.Add('ORDER BY DATA, HORA, CUPOM, LEGENDA');

    QRel.Sql.Add('GROUP BY DATA ,CUPOM ,LEGENDA, HORA ');
   ...
   // QRel.ParamByName('LEGENDA').AsString      :=  'Cancelado';
    ...
    begin
      ...
      QRel.Sql.Clear;
      QRel.Sql.Add('SELECT  LEGENDA, SUM(VALOR_FINALIZADORA) VALOR  ');
      QRel.Sql.Add('FROM');
      QRel.Sql.Add('(');
      QRel.Sql.Add('SELECT  LEGENDA, CASE WHEN (VALOR_CUPOM) = VALOR_FINALIZADORA ');
      QRel.Sql.Add('THEN (VALOR_CUPOM + ACRESCIMO - DESCONTO) WHEN (VALOR_CUPOM + ACRESCIMO - DESCONTO) > VALOR_FINALIZADORA ');
      QRel.Sql.Add('THEN (VALOR_FINALIZADORA) ELSE (VALOR_CUPOM + ACRESCIMO - DESCONTO) END VALOR , VALOR_FINALIZADORA');
      QRel.Sql.Add('FROM FECHAMENTO_CUPOM');
      QRel.Sql.Add('LEFT JOIN OPERADORES ON (CAST(OPERADORES.OPERADOR_ID AS INTEGER) = FECHAMENTO_CUPOM.OPERADOR_ID)');
      QRel.Sql.Add('WHERE');
      QRel.Sql.Add('(DATA BETWEEN :DT_INICIAL AND :DT_FINAL)');
    ...
    //  QRel.Sql.Add('AND (LEGENDA <> :LEGENDA)');
     ...
     // QRel.ParamByName('LEGENDA').AsString      := 'Cancelado';
      ...
      if Caixa_Banco.Value > 0 then
      QSangriaSuprimentoSintetico.ParamByName('CAIXA_ID').AsInteger    := StrToInt(Caixa_Banco.Text);
      ...
      begin
        ...
        if Trim(LowerCase(QRel.FieldByName('LEGENDA').AsString)) = 'dinheiro' then
        begin

          Writeln(MyFile, ForcaComprimento(QRel.FieldByName('LEGENDA').AsString, 39, ' ') +
            ForcaComprimento(' ', (9 - Length(FormatFloat('#,##0.00', (QRel.FieldByName('VALOR').AsFloat + Vr_Suprimento_Resumido + Vr_Sangria_Resumida) ))), ' ') +
            FormatFloat('#,##0.00', QRel.FieldByName('VALOR').AsFloat + Vr_Suprimento_Resumido + Vr_Sangria_Resumida));
        ...
        Vr_Fin := Vr_Fin +  QRel.FieldByName('VALOR').AsFloat
      end
      else
      begin
       Writeln(MyFile, ForcaComprimento(QRel.FieldByName('LEGENDA').AsString, 39, ' ') +
      ...
      QRel.SQL.Clear;
      QRel.SQL.Add('SELECT DATA, HORA,CAIXA_ID, LEGENDA, VALOR_CUPOM');
      QRel.SQL.Add('FROM FECHAMENTO_CUPOM');
      QRel.SQL.Add('WHERE');
      QRel.SQL.Add('(DATA BETWEEN :DT_INICIAL AND :DT_FINAL)');
      ...
      QRel.SQL.Add('ORDER BY DATA,HORA');
      ...
      begin
        while not QRel.Eof do
        begin
          if QRel.FieldByName('VALOR_CUPOM').AsFloat > 0 then
          Begin
            Writeln(MyFile, 'Suprimento: ' + DateToStr(QRel.FieldByName('DATA').AsDateTime) + ' - ' + QRel.FieldByName('HORA').AsString +
              ' - R$' + FormatFloat('#,##0.00',  QRel.FieldByName('VALOR_CUPOM').AsFloat));

          End
          Else
          Begin
            Writeln(MyFile,'Sangria   : ' + DateToStr(QRel.FieldByName('DATA').AsDateTime) + ' - ' + QRel.FieldByName('HORA').AsString +
              ' - R$' + FormatFloat('#,##0.00',QRel.FieldByName('VALOR_CUPOM').AsFloat));
          End;
      ...
      QRel.SQL.Clear;
      QRel.SQL.Add('SELECT CUPOM,DATA, HORA,CAIXA_ID, LEGENDA, SUM(VALOR_FINALIZADORA) VALOR_FINALIZADORA');
      QRel.SQL.Add('FROM FECHAMENTO_CUPOM');
      QRel.SQL.Add('WHERE');
      QRel.SQL.Add('(DATA BETWEEN :DT_INICIAL AND :DT_FINAL)');
     ...
     if Caixa_Banco.Value > 0 then
      ...
      QRel.SQL.Add('AND (LEGENDA = :LEGENDA) GROUP BY CUPOM,DATA,HORA,CAIXA_ID,LEGENDA');
      QRel.SQL.Add('ORDER BY DATA,HORA');

      QRel.ParamByName('DT_INICIAL').AsDateTime := Dtmen.Date;
      QRel.ParamByName('DT_FINAL').AsDateTime := Dtmen.Date;
      ...
      Begin
       ...
       Writeln(MyFile, '------------------------------------------------');
        ...
        while not QRel.Eof do
        begin
          if QRel.FieldByName('VALOR_FINALIZADORA').AsFloat > 0 then

            Writeln(MyFile, 'Cancelado: ' + QRel.FieldByName('CUPOM').AsString + ' - ' + QRel.FieldByName('HORA').AsString +
              ' - R$' + FormatFloat('#,##0.00',  QRel.FieldByName('VALOR_FINALIZADORA').AsFloat));

           VLC :=  QRel.FieldByName('VALOR_FINALIZADORA').AsFloat + VLC;
     ...
     QRel.SQL.Clear;
     QRel.SQL.Add('SELECT FINALIZADORAS.LEGENDA, SUM (VALOR) QUITACAO ');
     QRel.SQL.Add('FROM TRANSACOES');
     QRel.SQL.Add('INNER JOIN FINALIZADORAS ON FINALIZADORAS.FINALIZADORA_ID = TRANSACOES.FINALIZADORA_ID');
     QRel.SQL.Add('WHERE DT_ENT_SAI BETWEEN :DT_INICIAL AND :DT_FINAL AND CONDUTA = :CONDUTA');

     if Caixa_Banco.Text <> '' Then
     ...
     Begin
      ...
      Writeln(MyFile, ForcaComprimento(QRel.FieldByName('LEGENDA').AsString, 48 - Length(FormatFloat('#,##0.00', QRel.FieldByName('QUITACAO').AsFloat)), ' ') + FormatFloat('#,##0.00', QRel.FieldByName('QUITACAO').AsFloat));
