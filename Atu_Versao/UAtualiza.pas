...
var
Janela: HWND;
Local, Server, Old,kill: String;
begin
 KILL := 'TASKKILL /F /IM EFICAZSTREAMING.EXE' ;

 AddDeleteServico(KILL);

  Janela := FindWindow('TFrmControle_Nfce', nil);

  if Janela <> 0 then
    PostMessage(Janela, WM_QUIT, 0, 0);

  Old    := ExtractFilePath(ParamStr(0)) + 'EficazStreaming.Old';
  Local  := ExtractFilePath(ParamStr(0)) + 'EficazStreaming.Exe';
  Server := ExtractFilePath(ParamStr(0)) + '/Atualizar/EficazStreaming.Exe';
  ...
  KILL := 'TASKKILL /F /IM Atu_Versao_Streaming.EXE' ;
