...
var
  FrmOrcamento_7: TFrmOrcamento_7;
  Operacao: String;
...
procedure  TFrmOrcamento_7.controle_registro(tabela:STRING;chave_id,TP:Integer);
Begin
  ...
  QControle_Registro.Open;
    ...
    Begin
     ...
     QUpdate.Sql.Text := 'INSERT INTO CONTROLE_REGISTRO(CHAVE_ID,TABELA,DT_CHAVE,HR_CHAVE,USUARIO_ID,CONEXAO_ID,EMPRESA_ID) ' +
                         'VALUES(:CHAVE_ID,''ORCAMENTO'',:DT_CHAVE,:HR_CHAVE,:USUARIO_ID,:CONEXAO_ID,:EMPRESA_ID)';
...
procedure TFrmOrcamento_7.ServicoSearch;
begin
  QServico.Close;
...
procedure TFrmOrcamento_7.OrcamentoBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TFrmOrcamento_7.Orcamento_ReduzidoBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
if FileExists(FrmPrincipal.QEmpresa.FieldByName('LOGO_EMPRESA').AsString ) then
QrLogo.Picture.LoadFromFile(FrmPrincipal.QEmpresa.FieldByName('LOGO_EMPRESA').AsString);
...
procedure  TFrmOrcamento_7.Ordenar(Valores: tStrings);
var i, j : integer;
    temp : string;
begin
...
procedure TFrmOrcamento_7.CLIENTE_IDExit(Sender: TObject);
begin
  DetailSearch('Cliente');
end;

procedure TFrmOrcamento_7.ConsultaShow(Sender: TObject);
begin
  ...
  QSearch.ParamByName('CHAVE_ID').AsInteger   := QTabela.FieldByName('ORCAMENTO_ID').AsInteger;
  QSearch.Prepare;
  QSearch.Open;

  if not QSearch.IsEmpty Then
  CONTROLE_REGISTRO('ORCAMENTO',QTabela.FieldByName('ORCAMENTO_ID').AsInteger,1);

end;

procedure TFrmOrcamento_7.Botoes_Disable;
begin
...
procedure TFrmOrcamento_7.Botoes_Editing;
begin
  btnPrior.Enabled     := False;
  btnNext.Enabled      := False;
  btnInsert.Enabled    := False;
...
procedure TFrmOrcamento_7.Botoes_Normal;
begin
  ...
  btnCliente.Enabled  := False;
  btnVendedor.Enabled := False;
  if not FrmPrincipal.AvisoOrcamento7 then
    btnPesquisa.Enabled := True;
end;

procedure TFrmOrcamento_7.DetailSearch(Tabela: String);
begin
  ...
  begin
    ...
    begin
      ...
      Tipo_cliente.Text := QCliente.FieldByName('TIPO_CLIENTE').AsString;
      INSCRICAO.Text := QCliente.FieldByName('INSCRICAO').AsString;

       IF SemMascara(QCliente.FieldByName('CNPJ').AsString) <> ''  Then
       Begin
        Cnpj.EditMask := '';
        CNPJ.Text      := (QCliente.FieldByName('CNPJ').AsString);
       End;

      IF QCliente.FieldByName('EMAIL').AsString <> '' Then
      EMAIL.Text     := QCliente.FieldByName('EMAIL').AsString;

      if alltrim(SemMascara(QCliente.FieldByName('TELEFONE_1').AsString))  <> '' Then
      Begin
      Telefone.EditMask := '';
      Telefone.Text     :=  QCliente.FieldByName('TELEFONE_1').AsString;
      End
      Else if (alltrim(SemMascara(QCliente.FieldByName('TELEFONE_1').AsString)) = '' ) and (alltrim(SemMascara(QCliente.FieldByName('TELEFONE_2').AsString)) <> '' )  Then
      Begin
      Telefone.EditMask := '';
      Telefone.Text     :=  QCliente.FieldByName('TELEFONE_2').AsString;
      End
      Else if (alltrim(SemMascara(QCliente.FieldByName('TELEFONE_1').AsString)) = '' ) and (alltrim(SemMascara(QCliente.FieldByName('TELEFONE_2').AsString)) = '' ) and (alltrim(SemMascara(QCliente.FieldByName('FAX').AsString)) <> '' )  Then
      Begin
      Telefone.EditMask := '';
      Telefone.Text     :=  QCliente.FieldByName('FAX').AsString;
      End
      Else
      Telefone.EditMask :='(99) 99999-9999;1;_'
...
procedure TFrmOrcamento_7.DiasClick(Sender: TObject);
begin
  ...
  begin
    CmdSelectNull := 'WHERE (ORCAMENTO_ID IS NOT NULL) AND (DT_MOVIMENTO <= :DT_MOVIMENTO) AND (EMPRESA_ID = :EMPRESA_ID) AND (COMANDA IS NULL)';
    CmdOrderBy    := 'ORDER BY DT_ORCAMENTO, ORCAMENTO_ID';
...
procedure TFrmOrcamento_7.DT_ORCAMENTOEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmOrcamento_7.DT_ORCAMENTOKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmOrcamento_7.Habilitar(Status: Boolean);
var
I: Integer;
Temp: TComponent;
begin
...
procedure TFrmOrcamento_7.Insert;
var
I: Integer;
Temp: TComponent;
Sql, Par: String;
...
procedure TFrmOrcamento_7.Edit;
var
I: Integer;
Temp: TComponent;
Sql: String;
...
procedure TFrmOrcamento_7.ProdutoSearch;
begin
  QProduto.Close;
...
procedure TFrmOrcamento_7.TIPO_CLIENTEChange(Sender: TObject);
begin
  if SemMascara(cnpj.text) = '' Then
  Begin
...
procedure TFrmOrcamento_7.TributoSearch;
begin
  QTributo.Close;
...
procedure TFrmOrcamento_7.EMPRESA_IDExit(Sender: TObject);
begin
  DetailSearch('Empresa');
end;

procedure TFrmOrcamento_7.EMPRESA_IDKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
function TFrmOrcamento_7.Validacao: Boolean;
begin
  Result := False;
  ...
  Municipio.Color   := clWindow;
  Contato.Color     := clWindow;
  Telefone.Color    := clWindow;
  ...
  Else
  Begin
   if (Alltrim(nome.text) = '') or (Alltrim(Semmascara(telefone.text)) = '') or (Alltrim(contato.text) = '') or
      (Alltrim(municipio.text) = '') or (Alltrim(endereco.text) = '')  Then
   Begin
      //Application.MessageBox('Dados do cliente inconpletos favor verificar.', PChar(Msg_Title), mb_IconStop);

      if(Alltrim(nome.text) = '') Then
      Begin
      Application.MessageBox('Dados do cliente (nome) inconpleto favor verificar.', PChar(Msg_Title), mb_IconStop);
      nome.Color := clYellow;
      nome.SetFocus;
       exit;
      End;

      if(Alltrim(endereco.text) = '')Then
      Begin
      Application.MessageBox('Dados do cliente (endereço) inconpletos favor verificar.', PChar(Msg_Title), mb_IconStop);
      endereco.Color := clYellow;
      endereco.SetFocus;
       exit;
      End;

      if(Alltrim(municipio.text) = '') Then
      Begin
      Application.MessageBox('Dados do cliente (municipio) inconpleto favor verificar.', PChar(Msg_Title), mb_IconStop);
      municipio.Color := clYellow;
      municipio.SetFocus;
       exit;
      End;

      if(Alltrim(contato.text) = '') Then
      Begin
      Application.MessageBox('Dados do cliente (contato) inconpleto favor verificar.', PChar(Msg_Title), mb_IconStop);
      contato.Color := clYellow;
      contato.SetFocus;
       exit;
      End;

      if(Alltrim(Semmascara(telefone.text)) = '') Then
      Begin
      Application.MessageBox('Dados do cliente (telefone) inconpleto favor verificar.', PChar(Msg_Title), mb_IconStop);
      telefone.Color := clYellow;
      telefone.SetFocus;
        exit;
      End;
  ...
  if (Status.Text = 'CANCELADO') or (Status.Text = 'PERDIDO') then
  begin
    if Cancelamento.Text = '' then
    begin
      Application.MessageBox('Favor preencher o motivo!', PChar(Msg_Title), mb_IconSTop);
...
procedure TFrmOrcamento_7.VENDEDOR_IDExit(Sender: TObject);
begin
  DetailSearch('Vendedor');
end;

procedure TFrmOrcamento_7.VR_DESCONTOExit(Sender: TObject);
begin
{if FrmPrincipal.Config.FieldByName('DESC_ORC_USU').AsString = 'True' then
Begin
Desc_por := 0;
...
procedure TFrmOrcamento_7.ManutencaoShow(Sender: TObject);
begin
  QTabela.Refresh;
  Set_Campos(False);
  Botoes_Normal;
  ...
  CONTROLE_REGISTRO('ORCAMENTO',QTabela.FieldByName('ORCAMENTO_ID').AsInteger,0);


end;

procedure TFrmOrcamento_7.OBSERVACAOEnter(Sender: TObject);
begin
Observacao.Text := Observacao.Text + ' ';
end;

procedure TFrmOrcamento_7.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  {
  IF Semmascara(QTabela.FieldByName('CNPJ').AsString) <> '' Then
  QrDbText70.DataSet :=  QTabela
  Else
  QrDbText70.DataSet :=  QCliente;

  IF QTabela.FieldByName('INSCRICAO').AsString <> '' Then
  QrDbText71.DataSet :=  QTabela
  Else
  QrDbText71.DataSet :=  QCliente;

  IF QTabela.FieldByName('EMAIL').AsString <> '' Then
  QrDbText68.DataSet :=  QTabela
  Else
  QrDbText68.DataSet :=  QCliente;


  if Cliente_id.value = 0 Then
  Begin
  QrDbText27.DataSet   :=  QTabela;
  QrDbText70.DataSet   :=  QTabela;
  QrDbText71.DataSet   :=  QTabela;
  QrDbText68.DataSet   :=  QTabela;
  QrDbText42.DataField := 'Telefone';
  QrDbText42.DataSet   :=  QTabela;
  End
  Else
  Begin
  QrDbText27.DataSet   :=  QCliente;
  QrDbText70.DataSet   :=  QCliente;
  QrDbText71.DataSet   :=  QCliente;
  QrDbText68.DataSet   :=  QCliente;
  QrDbText42.DataField := 'Telefone_1';
  QrDbText42.DataSet   :=  QCliente;
  End;
  }
end;

procedure TFrmOrcamento_7.QSub_DetailAfterDelete(DataSet: TDataSet);
var
S:Integer;
begin
  S := 1;
  ...
  QUpdate.SQL.Clear;
  QUpdate.SQL.Add('UPDATE ORCITENS SET SEQUENCIA = :SEQUENCIA WHERE PRODUTO_ID = :PRODUTO_ID AND ORCAMENTO_ID = :ORCAMENTO_ID');
...
procedure TFrmOrcamento_7.QSub_DetailAfterOpen(DataSet: TDataSet);
begin
...
procedure TFrmOrcamento_7.QSub_DetailBeforeDelete(DataSet: TDataSet);
begin
...
procedure TFrmOrcamento_7.QSub_DetailBeforeEdit(DataSet: TDataSet);
begin
...
procedure TFrmOrcamento_7.QSub_DetailBeforeInsert(DataSet: TDataSet);
begin
  {
  if (FrmPrincipal.Config.FieldByName('OBJETO_ORDEM').AsString <> 'Madereira') and (QTabela.FieldByName('DT_MOVIMENTO').AsDateTime < FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime) then
  begin
...
procedure TFrmOrcamento_7.QSub_DetailBeforePost(DataSet: TDataSet);
var
Cst: String;
Promocao: Boolean;
itemtemp:integer;
...
begin
  ...
  if (QProduto.IsEmpty) And (QServico.IsEmpty) then
  begin
    Application.MessageBox('Produto/Serviço inexistente', PChar(Msg_Title), mb_IconStop);
...
procedure TFrmOrcamento_7.QSub_DetailNewRecord(DataSet: TDataSet);
begin
...
procedure TFrmOrcamento_7.QTabelaAfterOpen(DataSet: TDataSet);
begin
TFloatField(QTabela.FieldByName('VALOR')).DisplayFormat  := '#,##0.00';
end;

procedure TFrmOrcamento_7.RDprint1BeforeNewPage(Sender: TObject; Pagina: Integer);
begin
...
procedure TFrmOrcamento_7.RDprint1NewPage(Sender: TObject; Pagina: Integer);
begin
  ...
  RDprint1.Imp(02, 55, Copy(FrmPrincipal.QEmpresa.FieldByName('MUNICIPIO').AsString,1,20));
  RDprint1.Imp(02, 72, FrmPrincipal.QEmpresa.FieldByName('CEP').AsString);
  RDprint1.Imp(03, 01, 'Cnpj.:' + FrmPrincipal.QEmpresa.FieldByName('CNPJ').AsString);
  RDprint1.Imp(03, 30, 'I.E.: ' + FrmPrincipal.QEmpresa.FieldByName('INSCR_ESTADUAL').AsString);
  RDprint1.Imp(03, 55, 'Fone: ' + FrmPrincipal.QEmpresa.FieldByName('TELEFONE').AsString);
  RDprint1.Imp(04, 01, 'Data: ' + DateToStr(date) + ' - ' + TimeToStr(now));
  RDprint1.impDir(04, 80, 80, 'Página: ' + IntToStr(Pagina), []);
  RDprint1.Imp(05, 30, '*** ORCAMENTO ***');
  RDprint1.Imp(06, 01, '--------------------------------------------------------------------------------');
  linha := 07;
  Linha3 := 07;
...
procedure TFrmOrcamento_7.RDprint2BeforeNewPage(Sender: TObject; Pagina: Integer);
begin
  if linha <= 62  then
  Begin
  RDprint1.Imp(63, 01, '==================================');
...
procedure TFrmOrcamento_7.Set_Campos(Vazio: Boolean);
var
I: Integer;
Temp: TComponent;
begin
  ...
  begin
   ...
   If (Status.Text = 'FATURADO')   Then
    Begin
      ...
      IQuery.SQL.Clear;
      IQuery.Sql.Add('SELECT NUM_DOC,DT_ENT_SAI FROM TRANSACOES WHERE TRANSACAO_ID = :TRANSACAO_ID ');
      IQuery.ParamByName('TRANSACAO_ID').Asinteger := QTabela.FieldByName('TRANSACAO_ID').AsInteger;
      IQuery.Prepare;
      IQuery.Open;

      Lbl_Faturamento.Visible:= True;
      Lbl_Faturamento.Caption := 'Nf: '+IQuery.FieldByname('NUM_DOC').AsString;


    End
   Else
    Lbl_Faturamento.Visible:= False;
...
procedure TFrmOrcamento_7.StatusChange(Sender: TObject);
begin
   if (Status.Text = 'CANCELADO') OR (Status.Text = 'PERDIDO') then
   Begin
     Label18.Visible      := True;
...
procedure TFrmOrcamento_7.btnDeleteClick(Sender: TObject);
begin
  if FrmData.QAcesso.FieldByName('EXCLUSAO').AsString = 'NÃO' then
  begin
    Application.MessageBox('Usuário sem permissão para exclusão', PChar(Msg_Title), mb_IconStop);
  ...
  begin
   ...
   if QTabela.FieldByName('PEDIDO_ID').AsInteger > 0 then
  begin
    Application.MessageBox(PChar('Orçamento importado no pedido ' + QTabela.FieldByName('PEDIDO_ID').AsString + #13 + 'Favor Excluir Pedido de Venda para Exclusão.'), PChar(Msg_Title), mb_IconStop);
    abort;
  end;
...
procedure TFrmOrcamento_7.btnDiscardClick(Sender: TObject);
begin
  Botoes_Normal;
  Set_Campos(False);
...
procedure TFrmOrcamento_7.btnEditClick(Sender: TObject);
begin
  ...
  if QTabela.FieldByName('PEDIDO_ID').AsInteger > 0 then
  begin
    Application.MessageBox(PChar('Orçamento importado no pedido: ' + QTabela.FieldByName('PEDIDO_ID').AsString + #13 + 'Favor Excluir Pedido de Venda para Alteração.'), PChar(Msg_Title), mb_IconStop);
    abort;
  end;
  ...
  if QTabela.FieldByName('STATUS').AsString = 'CANCELADO' then
  begin
    ...
    VENDEDOR_ID.Value := FrmData.QAcesso.FieldByName('FUNCIONARIO_ID').AsInteger;
    Status.Text       := 'NORMAL';
    Cancelamento.Text := '';
    VENDEDOR_IDExit(Self);
  end;

end;

procedure TFrmOrcamento_7.btnEmpresaClick(Sender: TObject);
begin
  try
...
procedure TFrmOrcamento_7.btnClienteClick(Sender: TObject);
begin
  try
...
procedure TFrmOrcamento_7.btnInsertClick(Sender: TObject);
begin
...
procedure TFrmOrcamento_7.btnLogClick(Sender: TObject);
var
F1: TextFile;
begin
   Set_Campos(False);
...
procedure TFrmOrcamento_7.btnNextClick(Sender: TObject);
begin
  if not QTabela.Eof then
  begin
    CONTROLE_REGISTRO('ORCAMENTO',QTabela.FieldByName('ORCAMENTO_ID').AsInteger,1);
...
procedure TFrmOrcamento_7.btnOrcamentoClick(Sender: TObject);
var
Copias,Linhas,Dividir,id_orcamento:Integer;
...
begin
   {
   id_orcamento := QTabela.FieldByName('ORCAMENTO_ID').AsInteger ;
  ...
  QTabela.Close;
  QTabela.Open;
  QTabela.Locate('ORCAMENTO_ID', id_orcamento , [loCaseInsensitive]);
  ...
  inc(linha);
  RDprint1.Imp(linha, 01, 'Cód.  Descrição                             Qtd.    Pr.        Desc.   Pr. Total');
  inc(linha);
  RDprint1.Imp(linha, 01, '--------------------------------------------------------------------------------');
  inc(linha);
  ...
  RDprint1.Imp(linha,6,Copy(QSub_Detail.FieldByName('DESCRICAO').AsString ,1,38));
 ...
 { Else if LeIni(Arq_Ini, 'Sistema', 'Imp. Orçamento') = 'RDPrint MP-4200' then
  ...
  Begin
      ...
      // Comentado cadastrar cliente ao realizar impressao de orçamento sem cliente cadastrado
      {
      if QCliente.IsEmpty then
      begin
        QCliente.Append;
        QCliente.FieldByName('NOME').AsString       := NOME.Text;
        QCliente.FieldByName('ENDERECO').AsString   := ENDERECO.Text;
        QCliente.FieldByName('NUMERO').AsString     := NUMERO.Text;
        QCliente.FieldByName('BAIRRO').AsString     := BAIRRO.Text;
        QCliente.FieldByName('MUNICIPIO').AsString  := MUNICIPIO.Text;
        QCliente.FieldByName('ESTADO').AsString     := ESTADO.Text;
        QCliente.FieldByName('CEP').AsString        := CEP.Text;
        QCliente.FieldByName('CONTATO').AsString    := CONTATO.Text;
        QCliente.FieldByName('DDD').AsString        := Copy(TELEFONE.Text,1,4);
        QCliente.FieldByName('TELEFONE_1').AsString := Copy(TELEFONE.Text,6,10);
        QCliente.Post;
      end;
      }
     ...
     QrMemo4.Font.Size := 8;
      ...
      QRMemo3.Lines.Clear;
      QRMemo3.Lines.Text := 'Observação: ' + QTabela.FieldByName('OBSERVACAO').AsString;
...
procedure TFrmOrcamento_7.btnParcelasClick(Sender: TObject);
begin
  if QTabela.FieldByName('VALOR').AsFloat > 0 then
...
procedure TFrmOrcamento_7.btnPesquisaClick(Sender: TObject);
var
Condicao: String;
begin
...
procedure TFrmOrcamento_7.btnPriorClick(Sender: TObject);
begin
  if not QTabela.Bof then
  begin
    CONTROLE_REGISTRO('ORCAMENTO',QTabela.FieldByName('ORCAMENTO_ID').AsInteger,1);
...
procedure TFrmOrcamento_7.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmOrcamento_7.btnSaveClick(Sender: TObject);
begin
  DetailSearch('');

  if Validacao then
  begin
    Funcionario_id.Value := VENDEDOR_ID.Value ;
    ...
    if (COND_PAGTO.Text = 'A PRAZO') and (Status.Text = 'NORMAL') and (not FrmPrincipal.AvisoOrcamento7) then
      Begin
...
procedure TFrmOrcamento_7.btnVendedorClick(Sender: TObject);
begin
  try
...
procedure TFrmOrcamento_7.btnVendedor_2Click(Sender: TObject);
begin
try
...
procedure TFrmOrcamento_7.DBGrid1DblClick(Sender: TObject);
begin
  Manutencao.Show;
end;

procedure TFrmOrcamento_7.DBGrid1DrawColumnCell(Sender: TObject;
...
begin
 DBGrid1.Canvas.Brush.Color := clInfoBk;
  ...
  Else if  (QTabela.FieldByName('STATUS').AsString = 'CANCELADO') OR (QTabela.FieldByName('STATUS').AsString = 'PERDIDO')  then
  ...
  if FrmPrincipal.AvisoOrcamento7 then
  begin
    if QTabela.FieldByName('STATUS').AsString <> 'AGUARD. PRODUTO' then
    begin
      if FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime - QTabela.FieldByName('DT_ORCAMENTO').AsDateTime >= 15 then
      begin
        TDBGrid(Sender).Canvas.Brush.Color := clWebLightPink;
        TDBGrid(Sender).Canvas.FillRect(Rect);
      end
      else
      if FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime - QTabela.FieldByName('DT_ORCAMENTO').AsDateTime = 14 then
      begin
        TDBGrid(Sender).Canvas.Brush.Color := clWebLightSalmon;
        TDBGrid(Sender).Canvas.FillRect(Rect);
      end
      else
      if FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime - QTabela.FieldByName('DT_ORCAMENTO').AsDateTime >= 11 then
      begin
        TDBGrid(Sender).Canvas.Brush.Color := clWebYellow;
        TDBGrid(Sender).Canvas.FillRect(Rect);
      end
      else
      if FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime - QTabela.FieldByName('DT_ORCAMENTO').AsDateTime >= 8 then
      begin
        TDBGrid(Sender).Canvas.Brush.Color := clWebLightGreen;
        TDBGrid(Sender).Canvas.FillRect(Rect);
      end;
    end
    else
    begin
      if FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime - QTabela.FieldByName('DT_ORCAMENTO').AsDateTime >= 45 then
      begin
        TDBGrid(Sender).Canvas.Brush.Color := clWebLightPink;
        TDBGrid(Sender).Canvas.FillRect(Rect);
      end
      else
      if FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime - QTabela.FieldByName('DT_ORCAMENTO').AsDateTime = 44 then
      begin
        TDBGrid(Sender).Canvas.Brush.Color := clWebLightSalmon;
        TDBGrid(Sender).Canvas.FillRect(Rect);
      end
      else
      if FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime - QTabela.FieldByName('DT_ORCAMENTO').AsDateTime >= 41 then
      begin
        TDBGrid(Sender).Canvas.Brush.Color := clWebYellow;
        TDBGrid(Sender).Canvas.FillRect(Rect);
      end
      else
      if FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime - QTabela.FieldByName('DT_ORCAMENTO').AsDateTime >= 38 then
      begin
        TDBGrid(Sender).Canvas.Brush.Color := clWebLightGreen;
        TDBGrid(Sender).Canvas.FillRect(Rect);
      end;
...
procedure TFrmOrcamento_7.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    Manutencao.Show;
end;

procedure TFrmOrcamento_7.DBGrid1TitleClick(Column: TColumn);
begin
  if FrmPrincipal.AvisoOrcamento7 then
  begin
    QTabela.Refresh;
...
procedure TFrmOrcamento_7.ItensColEnter(Sender: TObject);
begin
...
procedure TFrmOrcamento_7.ItensColExit(Sender: TObject);
begin
  if QSub_Detail.State in [dsInsert, dsEdit] then
  begin
    ...
    begin
      ...
      begin
        ...
        Begin
           ...
           QSub_Detail.FieldByName('MT_2').AsFloat := (QSub_Detail.FieldByName('QUANTIDADE').AsFloat * QProduto.FieldByName('FRACAO_VENDA').AsFloat);
...
procedure TFrmOrcamento_7.ItensEnter(Sender: TObject);
begin
  Botoes_Disable;
  Consulta.TabVisible := False;
end;

procedure TFrmOrcamento_7.ItensExit(Sender: TObject);
begin
  if QSub_Detail.State in [dsInsert, dsEdit] then
  begin
    QSub_Detail.Cancel;
...
procedure TFrmOrcamento_7.ItensKeyDown(Sender: TObject;
...
procedure TFrmOrcamento_7.ItensKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TFrmOrcamento_7.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  QTabela.First;
  if (not QTabela.IsEmpty) and (not QTabela.Locate('STATUS', 'CANCELADO', [loCaseInsensitive])) and (FrmPrincipal.AvisoOrcamento7 = True) then
  begin
    if FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime - QTabela.FieldByName('DT_ORCAMENTO').AsDateTime >= 15 then
    begin
      Application.MessageBox(PChar('Há Orçamentos vencidos que devem ser cancelados!'), PChar(Msg_Title), mb_IconStop);
      Abort;
    end;
  end;
  FrmPrincipal.AvisoOrcamento7 := False;
  Action := caFree;
end;

procedure TFrmOrcamento_7.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
...
procedure TFrmOrcamento_7.FormCreate(Sender: TObject);
var
X, Ult_Dia: Integer;
begin
  ...
  if FrmPrincipal.AvisoOrcamento7 then
  begin
    CmdSelectNull := 'WHERE (ORCAMENTO_ID IS NOT NULL) AND (EMPRESA_ID = :EMPRESA_ID) AND DT_ORCAMENTO <= :DT_MOVIMENTO::DATE - 8 AND (COMANDA IS NULL) AND (VENDEDOR_ID = :VENDEDOR_ID) AND STATUS = ''NORMAL''';
    CmdOrderBy    := 'ORDER BY DT_ORCAMENTO, ORCAMENTO_ID';
    Dias.Visible := False;
  ...
  if  (Caso(Copy(FrmData.QAcesso.FieldByName('OPCOES').AsString, 66, 1))  = True) or FrmPrincipal.AvisoOrcamento7 Then
  QTabela.ParamByName('VENDEDOR_ID').AsInteger    := FrmData.QAcesso.FieldByName('FUNCIONARIO_ID').AsInteger;

  QTabela.Prepare;
  QTabela.Open;

  //voltando o where padrão pro aviso não influenciar na abertura normal do form
  CmdSelectNull := 'WHERE (ORCAMENTO_ID IS NOT NULL) AND (DT_MOVIMENTO = :DT_MOVIMENTO) AND (EMPRESA_ID = :EMPRESA_ID) AND (COMANDA IS NULL)';
  if  Caso(Copy(FrmData.QAcesso.FieldByName('OPCOES').AsString, 66, 1)) = True Then
   CmdSelectNull :=  CmdSelectNull + ' AND (VENDEDOR_ID = :VENDEDOR_ID)';
  CmdOrderBy    := 'ORDER BY ORCAMENTO_ID';
  ...
  if FrmPrincipal.Config.FieldByName('DESCONTOS_PRODUTOS').AsString = 'True'  Then
  Begin
  Itens.Columns[5].ReadOnly := False;
...
procedure TFrmOrcamento_7.FormShow(Sender: TObject);
begin
  DBGrid1.SetFocus;
end;
