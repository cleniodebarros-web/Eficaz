...
type
  TFrmRemarca_Novo = class(TForm)
    ...
    Margem_Varejo_Atual: TRxCalcEdit;
    PMZ: TRxCalcEdit;
    Preco_Varejo_Atual: TRxCalcEdit;
    Preco_Sugerido: TRxCalcEdit;
    Preco_Varejo_Novo: TRxCalcEdit;
    Panel1: TPanel;
    btnRetorna: TBitBtn;
    ...
    DBText1: TDBText;
    btnRemarca: TBitBtn;
    Margem_Varejo_Novo: TRxCalcEdit;
    Rem_Margem_Atual: TCheckBox;
    Label2: TLabel;
    Quantidade: TRxCalcEdit;
    Label3: TLabel;
    Preco_Prazo_Atual: TRxCalcEdit;
    Label4: TLabel;
    Preco_Prazo_Sugerido: TRxCalcEdit;
    Preco_Prazo_Novo: TRxCalcEdit;
    BtnRemarcaT: TBitBtn;
    ...
    Preco_Atacado_Novo: TRxCalcEdit;
    Label8: TLabel;
    Label9: TLabel;
    Margem_Prazo_Atual: TRxCalcEdit;
    Label10: TLabel;
    Preco_Atacado_Atual: TRxCalcEdit;
    Label11: TLabel;
    Margem_Atacado_Atual: TRxCalcEdit;
    Label12: TLabel;
    Margem_Prazo_Novo: TRxCalcEdit;
    Margem_Atacado_Novo: TRxCalcEdit;
    Label1: TLabel;
    Label5: TLabel;
    Label6: TLabel;
    Label13: TLabel;
    ...
    procedure Margem_Varejo_NovoExit(Sender: TObject);
    procedure Preco_Varejo_NovoExit(Sender: TObject);
    ...
    procedure Preco_Prazo_NovoExit(Sender: TObject);
    procedure Margem_Prazo_NovoExit(Sender: TObject);
    procedure Preco_Atacado_NovoExit(Sender: TObject);
    procedure Margem_Atacado_NovoExit(Sender: TObject);
    procedure Custo_NovoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure Custo_NovoExit(Sender: TObject);
    procedure FormShow(Sender: TObject);
    procedure Margem_Varejo_NovoChange(Sender: TObject);
  private
...
var
  FrmRemarca_Novo: TFrmRemarca_Novo;
  ID: Integer;
  procedure RemarcaNovo(Transacao: Integer);
...
begin
  ID := Transacao;
  Application.CreateForm(TFrmRemarca_Novo, FrmRemarca_Novo);
  try
    FrmRemarca_Novo.ShowModal;
  finally
    FrmRemarca_Novo.Release;
  end;
end;

procedure TFrmRemarca_Novo.Set_Campos;
var
...
begin
 ...
 //   begin
  ...
  //  end;
    ...
    //ShowMessage(FloatToStr(New_Custo) + ' -- ' + FloatToStr(New_IPI) + ' -- ' + FloatToStr(New_Frt) + ' -- ' + FloatToStr(New_Desp) + ' -- ' + FloatToStr(New_Fcp) + ' -- ' + FloatToStr(New_Desp_Custo) + ' -- ' + FloatToStr(ICMS_Sub) + ' -- ' + FloatToStr(New_Dif_ICMS));

    Cst_Entr := New_Custo + New_IPI + New_Frt + New_Desp + New_Fcp + New_Desp_Custo + ICMS_Sub + New_Dif_ICMS ;

    Custo_Atual.Value           := Cst_Entr;
    Custo_Novo.Value            := Cst_Entr;
    Despesas.Value              := (QIndices.FieldByName('VALOR').AsFloat + QTabela.FieldByName('ICMS_SAIDA').AsFloat);
    PMZ.Value                   := New_PMZ;
    Preco_Varejo_Atual.Value    := QTabela.FieldByName('PRECO_VAREJO').AsFloat;
    Preco_Varejo_Novo.Value     := Cst_Entr + (Cst_Entr * QTabela.FieldByName('MARGEM_LUCRO').AsFloat)/100;
    Preco_Sugerido.Value        := New_Sugestao;
    Margem_Varejo_Atual.Value   := QTabela.FieldByName('MARGEM_LUCRO').AsFloat;
    Margem_Varejo_Novo.Value    := Margem_Varejo_Atual.Value;
    //if (Margem_Varejo_Atual.Value > 0) and (Preco_Varejo_Atual.Value > 0.011) then
    //  Margem_Varejo_Atual.Value := (Preco_Varejo_Atual.Value - Custo_Atual.Value)*100/Custo_Atual.Value;
    if Preco_Varejo_Atual.Value > 0.011 then
      Margem_Varejo_Novo.Value    := (Preco_Varejo_Atual.Value - Custo_Atual.Value)*100/Custo_Atual.Value;
    Preco_Prazo_Atual.Value     := QTabela.FieldByName('PRECO_PRAZO').AsFloat;
    ...
    Preco_Atacado_Atual.Value   := QTabela.FieldByName('PRECO_ATACADO').AsFloat;
    Quant_Maxima.Value          := QTabela.FieldByName('QUANT_MAXIMA').AsFloat;
    Margem_Prazo_Atual.Value    := (Preco_Prazo_Atual.Value - Custo_Atual.Value)*100/Custo_Atual.Value;
    Margem_Prazo_Novo.Value     := Margem_Prazo_Atual.Value;
    Margem_Atacado_Atual.Value  := (Preco_Atacado_Atual.Value - Custo_Atual.Value)*100/Custo_Atual.Value;
    Margem_Atacado_Novo.Value   := Margem_Atacado_Atual.Value;


    //try
      //Margem_Varejo_Novo.Value   := RoundTo((((Preco_Varejo_Atual.Value - Custo_Atual.Value) * 100) / Custo_Atual.Value), -2);
    //except

    //end;

    //if Margem_Varejo_Novo.Value = 0 then
    //  Margem_Varejo_Novo.Color := clRed
    //else Margem_Varejo_Novo.Color := clWindow;

    if Tipo_Remarca.ItemIndex = 1 then
    Begin
      //Preco_Varejo_Novo.Value   := (Custo_Atual.Value + ((Custo_Atual.Value * Margem_Varejo_Atual.Value) / 100));
      if Preco_Prazo_Atual.Value < Preco_Varejo_Atual.Value then
        Preco_Prazo_Novo.Value    := Preco_Varejo_Atual.Value
      else
        Preco_Prazo_Novo.Value    := (Custo_Atual.Value + ((Custo_Atual.Value * Margem_Prazo_Atual.Value) / 100));
      Margem_Prazo_Novo.Value     := (Preco_Prazo_Novo.Value - Custo_Atual.Value)*100/Custo_Atual.Value;

      if Margem_Atacado_Atual.Value > 0 then
        Preco_Atacado_Novo.Value  := (Custo_Atual.Value + ((Custo_Atual.Value * Margem_Atacado_Atual.Value) / 100))
      else
        Preco_Atacado_Novo.Value  := Preco_Atacado_Atual.Value; //caso o pre�o atacado esteja 0,01 n�o vai copiar o custo, vai manter 0,01
    End
    else
    Begin
      //Preco_Varejo_Novo.Value   := New_Sugestao;
      Preco_Prazo_Novo.Value    := New_Sugestao;
      Preco_Atacado_Novo.Value  := New_Sugestao;
    End;

    Preco_Varejo_Novo.SetFocus;
    Preco_Varejo_Novo.SelectAll;
  finally

  end;
end;

procedure TFrmRemarca_Novo.Tipo_RemarcaClick(Sender: TObject);
begin
  Set_Campos;
end;

procedure TFrmRemarca_Novo.Atualiz_Estoque_MinimoClick(Sender: TObject);
begin
...
procedure TFrmRemarca_Novo.btnRemarcaClick(Sender: TObject);
begin
  try
    QUpdate.Sql.Clear;
    QUpdate.Sql.Add('UPDATE PRODUTOS SET PRECO_VAREJO = :PRECO_VAREJO, PRECO_PRAZO = :PRECO_PRAZO, DT_PRECO = :DT_PRECO, QUANT_MAXIMA = :QUANT_MAXIMA, ' +
                    'ULT_QUANTIDADE = :ULT_QUANTIDADE, MARGEM_LUCRO = :MARGEM_LUCRO, QUANT_MINIMA = :ESTOQUE_MINIMO, FUNCIONARIO_ID = :FUNCIONARIO_ID, ' +
                    'CUSTO_COMPRA = :CUSTO_COMPRA, VALOR_COMPRA = :VALOR_COMPRA, PRECO_ATACADO = :PRECO_ATACADO ' +
                                         'WHERE ' +
                                         '(PRODUTO_ID = :PRODUTO_ID)');

    QUpdate.ParamByName('CUSTO_COMPRA').AsFloat   := Custo_Novo.Value;
    QUpdate.ParamByName('VALOR_COMPRA').AsFloat   := QTabela.FieldByName('VR_UNITARIO').AsFloat - QTabela.FieldByName('DESC_ITEM').AsFloat - (QTabela.FieldByName('DESC_ITEM_RODAPE').AsFloat / QTabela.FieldByName('QUANTIDADE').AsFloat);
    QUpdate.ParamByName('PRECO_VAREJO').AsFloat   := Preco_Varejo_Novo.Value;
    QUpdate.ParamByName('PRECO_PRAZO').AsFloat    := Preco_Prazo_Novo.Value;
    Qupdate.ParamByName('PRECO_ATACADO').AsFloat  := Preco_Atacado_Novo.Value;
    ...
    if Rem_Margem_Atual.Checked then
      QUpdate.ParamByName('MARGEM_LUCRO').AsFloat := Margem_Varejo_Novo.Value
    else
      QUpdate.ParamByName('MARGEM_LUCRO').AsFloat := QTabela.FieldByName('MARGEM_LUCRO').AsFloat;
    ...
    if (not QTabela.IsEmpty) and (LeIni(Arq_Ini, 'Sistema', 'Carga Autom�tica de Produtos') = 'True') and (FrmPrincipal.QEmpresa.FieldByName('CARGA').AsString = 'SIM') then
    ...
    begin
      ...
      if Rem_Margem_Atual.Checked then
        ...
        QUpdate.ParamByName('MARGEM_LUCRO').AsFloat := Margem_Varejo_Atual.Value;
   ...
   on e:Exception do
        begin
          ...
          Application.MessageBox(Pchar('N�o foi poss�vel atualizar o pre�o devido a um erro interno:' + #13 + 'Erro: ' + e.Message) , PChar(Msg_Title), mb_IconInformation);
...
procedure TFrmRemarca_Novo.BtnRemarcaCustoClick(Sender: TObject);
var
Margem_total : Real;
begin
if Application.MessageBox('Deseja realmente remarcar todos os produtos(Pre�o de Custo) desta nota?', PChar(Msg_Title), mb_YesNo + mb_IconQuestion + mb_DefButton2) = IDYES then
 Begin
  Margem_Total := Margem_Varejo_Novo.Value;
  QTabela.First;
    ...
    Set_Campos;
      try
        ...
        // Application.MessageBox('N�o foi poss�vel atualizar o pre�o devido a um erro interno', PChar(Msg_Title), mb_IconInformation);

       on e:Exception do
        begin

         Application.MessageBox(Pchar('N�o foi poss�vel atualizar o pre�o devido a um erro interno:' + #13 + e.Message) , PChar(Msg_Title), mb_IconInformation);
     ...
     QTabela.Next
    End;
  Application.MessageBox(' Todos Produtos(Pre�o de Custo) da nota  foram remarcados.', PChar(Msg_Title), mb_IconInformation);
 End
 Else
  Application.MessageBox('Nenhum produto(Pre�o de Custo) desta nota foi remarcado.', PChar(Msg_Title), mb_IconInformation);
end;

procedure TFrmRemarca_Novo.BtnRemarcaTClick(Sender: TObject);
var
Margem_total : Real;
begin
...
procedure TFrmRemarca_Novo.Custo_NovoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmRemarca_Novo.DBNavigator1Click(Sender: TObject; Button: TNavigateBtn);
begin
  Set_Campos;
end;

procedure TFrmRemarca_Novo.FormCreate(Sender: TObject);
begin
  QTabela.Sql.Clear;
...
procedure TFrmRemarca_Novo.FormShow(Sender: TObject);
begin
  Set_Campos;
  Margem_Varejo_NovoChange(Self);
end;

procedure TFrmRemarca_Novo.Margem_Atacado_NovoExit(Sender: TObject);
begin
  if Margem_Atacado_Novo.Value > 0 then
    Preco_Atacado_Novo.Value  := (Custo_Novo.value + ((Custo_Novo.value * Margem_Atacado_Novo.Value) / 100));
end;

procedure TFrmRemarca_Novo.Margem_Prazo_NovoExit(Sender: TObject);
begin
  Preco_Prazo_Novo.Value      := (Custo_Novo.value + ((Custo_Novo.value * Margem_Prazo_Novo.Value) / 100));
end;

procedure TFrmRemarca_Novo.Margem_Varejo_NovoChange(Sender: TObject);
begin
  if Margem_Varejo_Novo.Value = 0 then
    Margem_Varejo_Novo.Color := clRed
  else if Margem_Varejo_Novo.Value < FrmPrincipal.Config.FieldByName('ALERTA_REMARCAR').AsFloat then
    Margem_Varejo_Novo.Color := clYellow
  else
    Margem_Varejo_Novo.Color := clWindow;
end;

procedure TFrmRemarca_Novo.Margem_Varejo_NovoExit(Sender: TObject);
begin
  Preco_Varejo_Novo.Value     := (Custo_Novo.value + ((Custo_Novo.value * Margem_Varejo_Novo.Value) / 100));
end;

procedure TFrmRemarca_Novo.Preco_Atacado_NovoExit(Sender: TObject);
begin
  Margem_Atacado_Novo.Value := RoundTo((((Preco_Atacado_Novo.Value - Custo_Novo.Value) * 100) / Custo_Novo.Value), -3);
end;

procedure TFrmRemarca_Novo.Preco_Prazo_NovoExit(Sender: TObject);
begin
  Margem_Prazo_Novo.Value   := RoundTo((((Preco_Prazo_Novo.Value - Custo_Novo.Value) * 100) / Custo_Novo.Value), -3);
end;

procedure TFrmRemarca_Novo.Preco_Varejo_NovoExit(Sender: TObject);
begin
  Margem_Varejo_Novo.Value  := RoundTo((((Preco_Varejo_Novo.Value - Custo_Novo.Value) * 100) / Custo_Novo.Value), -3);
end;

procedure TFrmRemarca_Novo.Custo_NovoExit(Sender: TObject);
begin
  Margem_Varejo_NovoExit(Sender);
  Margem_Prazo_NovoExit(Sender);
  Margem_Atacado_NovoExit(Sender);
end;
