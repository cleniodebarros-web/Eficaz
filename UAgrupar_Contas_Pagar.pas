...
uses
  ...
  QuickRpt, IBStoredProc, FireDAC.Stan.Intf, FireDAC.Stan.Option,
  FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf,
  FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.DataSet,
  FireDAC.Comp.Client;

type
  TFrmAgrupar_Contas_Pagar = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    Dtmen: TDateEdit;
    Dtmai: TDateEdit;
...
var
  FrmAgrupar_Contas_Pagar: TFrmAgrupar_Contas_Pagar;
...
procedure TFrmAgrupar_Contas_Pagar.Cria_Dados_Temporarios;
begin
  Table_Temp.Append;
  ...
  Table_Temp.FieldByName('FORNECEDOR_ID').AsInteger  := QContas.FieldByName('FORNECEDOR_ID').AsInteger;

  Table_Temp.Post;
end;

procedure TFrmAgrupar_Contas_Pagar.Seleciona;
begin
  QContas.Sql.Clear;
  QContas.Sql.Add('SELECT FORNECEDORES.NOME, FORNECEDORES.ENDERECO, FORNECEDORES.NUMERO, TRANSPARCELAS.*, TRANSACOES.HISTORICO, ' +
                  'TRANSACOES.NUM_DOC, TRANSACOES.CONTA_ID, TRANSACOES.VENDEDOR_ID, TRANSACOES.EMPRESA_ID, TRANSACOES.TPCTB, TRANSACOES.C_CUSTO_ID, TRANSACOES.FORNECEDOR_ID');
  ...
  QContas.Sql.Add('ORDER BY FORNECEDORES.NOME, TRANSPARCELAS.DT_VENCIMENTO, TRANSPARCELAS.PARCELA_ID');
...
procedure TFrmAgrupar_Contas_Pagar.Soma;
begin
  Vr_Soma.Value := 0;
...
procedure TFrmAgrupar_Contas_Pagar.AlfabetoClick(Sender: TObject);
begin
  QContas.Sql.Clear;
  QContas.Sql.Add('SELECT FORNECEDORES.NOME, FORNECEDORES.ENDERECO, FORNECEDORES.NUMERO, TRANSPARCELAS.*, TRANSACOES.HISTORICO, ' +
                  'TRANSACOES.NUM_DOC, TRANSACOES.CONTA_ID, TRANSACOES.VENDEDOR_ID, TRANSACOES.EMPRESA_ID, TRANSACOES.TPCTB, TRANSACOES.C_CUSTO_ID, TRANSACOES.FORNECEDOR_ID');
...
procedure TFrmAgrupar_Contas_Pagar.btnExecutaClick(Sender: TObject);
var
Id_Trans: Integer;
begin
  ...
  begin
    ...
    QRel.Sql.Add('INSERT INTO TRANSACOES( ' +
                 ...
                 'COND_PAGTO,    FORNECEDOR_ID) VALUES(' +
                 ':TRANSACAO_ID, :DT_TRANS,   :DT_MOVIMENTO, :CONDUTA, ' +
                 ':DEPTO,        :EMPRESA_ID, :CONTA_ID,     :C_CUSTO_ID, ' +
                 ':VALOR,        :TPCTB,      :AUTORIZ_ID,   :HISTORICO, ' +
                 ':CONTAAUX_ID,  :NUM_DOC,    :BANCO_ID,     :BALANCO, ' +
                 ':COND_PAGTO,   :FORNECEDOR_ID)');
...
procedure TFrmAgrupar_Contas_Pagar.btnFornecedorClick(Sender: TObject);
begin
  try
...
procedure TFrmAgrupar_Contas_Pagar.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmAgrupar_Contas_Pagar.btnSomaClick(Sender: TObject);
begin
  Vr_Agrupar.Value := 0;
  ...
  while not Table_Temp.Eof do
  begin
    Vr_Agrupar.Value := Vr_Agrupar.Value + Table_Temp.FieldByName('VALOR').AsFloat;
...
procedure TFrmAgrupar_Contas_Pagar.DBGrid1KeyDown(Sender: TObject;
...
procedure TFrmAgrupar_Contas_Pagar.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmAgrupar_Contas_Pagar.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmAgrupar_Contas_Pagar.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmAgrupar_Contas_Pagar.FormCreate(Sender: TObject);
var
Temp_Name: String;
begin
  with Table_Temp do
  begin
    ...
    begin
      ...
      Add('FORNECEDOR_ID', ftInteger, 0, False);
    end;
...
procedure TFrmAgrupar_Contas_Pagar.FormDestroy(Sender: TObject);
begin
  Table_Temp.IndexName := '';
  Table_Temp.Close;
end;

procedure TFrmAgrupar_Contas_Pagar.FornecedorExit(Sender: TObject);
begin
  Seleciona;
end;

procedure TFrmAgrupar_Contas_Pagar.FornecedorKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
