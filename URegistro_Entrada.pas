...
type
  TFrmRegistro_Entrada = class(TForm)
    Label1: TLabel;
    Dtmen: TDateEdit;
    Dtmai: TDateEdit;
    Bevel1: TBevel;
...
var
  FrmRegistro_Entrada: TFrmRegistro_Entrada;
...
procedure TFrmRegistro_Entrada.btnExecutaClick(Sender: TObject);
var
Temp_Name: String;
begin
// 151
// 325
  try
    ...
    Begin
      ...
      QRel.Sql.Clear;
      QRel.Sql.Add('SELECT TRANSITENS.TRANSACAO_ID, TRANSACOES.DT_SPED DT_TRANS, TRANSACOES.SERIE, TRANSACOES.NUM_DOC, ' +
                   'FORNECEDORES.CNPJ, FORNECEDORES.ESTADO, TRANSITENS.CFOP, ' +
                   'SUM(TRANSITENS.VR_TOTAL + TRANSITENS.VALOR_ICMS_ST + TRANSITENS.VR_FRETE + TRANSITENS.VR_ACRESCIMO + ' +
                   'TRANSITENS.VR_IPI + TRANSITENS.VR_FCPST + TRANSITENS.VR_FCP - TRANSITENS.DESC_RODAPE - TRANSITENS.VALOR_ICMS_DESON) VR_CONTABIL, TRANSITENS.ALIQUOTA_ICMS, SUM(TRANSITENS.VALOR_ICMS) VALOR_ICMS, SUM(TRANSITENS.BASE_CALC_ICMS) BASE_CALC_ICMS');
      ...
      QRel.Sql.Clear;
      QRel.Sql.Add('SELECT TRANSACOES.*, FORNECEDORES.CNPJ, FORNECEDORES.ESTADO');
    ...
    QRel.Sql.Add('TRANSITENS.VR_IPI + TRANSITENS.VR_FCPST + TRANSITENS.VR_FCP - TRANSITENS.DESC_RODAPE - TRANSITENS.VALOR_ICMS_DESON) VR_CONTABIL,  SUM(TRANSITENS.VALOR_ICMS) VALOR_ICMS, SUM(TRANSITENS.BASE_CALC_ICMS) BASE_CALC_ICMS');
    QRel.Sql.Add('FROM TRANSITENS');
    QRel.Sql.Add('INNER JOIN TRANSACOES');
    QRel.Sql.Add('ON (TRANSITENS.TRANSACAO_ID = TRANSACOES.TRANSACAO_ID)');
    QRel.Sql.Add('LEFT JOIN FORNECEDORES');
    ...
    QRel.Sql.Add(') AS TESTE');
     ...
     QRLabel37.Caption := Dtmen.Text + ' a ' + Dtmai.Text;
...
procedure TFrmRegistro_Entrada.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmRegistro_Entrada.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmRegistro_Entrada.DtmenKeyDown(Sender: TObject;
...
procedure TFrmRegistro_Entrada.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmRegistro_Entrada.FormCreate(Sender: TObject);
begin
  Dtmen.Date := StrToDate('01/' + Copy(DateToStr(date), 4, 7));
  Dtmai.Date := Ult_Dia_Mes(date);
end;

procedure TFrmRegistro_Entrada.QRGroup1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
