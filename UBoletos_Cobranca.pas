...
type
  TFrmBoletos_Cobranca = class(TForm)
    Label1: TLabel;
    Dtmen: TDateEdit;
    Dtmai: TDateEdit;
    Label3: TLabel;
    ...
    procedure Tipo_BoletoChange(Sender: TObject);
  private
...
var
  FrmBoletos_Cobranca: TFrmBoletos_Cobranca;
...
function TFrmBoletos_Cobranca.Validacao: Boolean;
begin
  Result := False;
...
procedure TFrmBoletos_Cobranca.DetailBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TFrmBoletos_Cobranca.DetailSearch(Tabela: String);
begin
...
procedure TFrmBoletos_Cobranca.BancoExit(Sender: TObject);
begin
  DetailSearch('Banco');
end;

procedure TFrmBoletos_Cobranca.btn1Click(Sender: TObject);
var
x:Integer;
Titulo: TACBrTitulo;
texto: TMemo;
...
procedure TFrmBoletos_Cobranca.btnBancoClick(Sender: TObject);
begin
  try
...
procedure TFrmBoletos_Cobranca.btnClienteClick(Sender: TObject);
begin
  try
...
procedure TFrmBoletos_Cobranca.btnExecutaClick(Sender: TObject);
var
...
// 307, 327
  try
    ...
    begin
      ...
      Begin
        QRLabel167.Enabled  := True;
        QRLabel167.Caption  := Local_Pagto.Text;
        QRMemo4.Lines     := Instrucoes.Lines;
        Duplicata.PreviewModal;
      End;

      if Tipo_Boleto.Text = 'Duplicata Mercantil Mod. 2' then
      Begin
        QRLabel164.Enabled  := True;
        QRLabel164.Caption  := Local_Pagto.Text;
        QRMemo3.Lines     := Instrucoes.Lines;
        Duplicata_Mercantil.PreviewModal;
      End;

      if Tipo_Boleto.Text = 'Duplicata Locação' then
      Begin
        QRLabel159.Caption := Cond_Espec.Text;
        Duplicata_Locacao.PreviewModal;
      End;
...
procedure TFrmBoletos_Cobranca.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmBoletos_Cobranca.ClienteExit(Sender: TObject);
begin
  DetailSearch('Cliente');
end;

procedure TFrmBoletos_Cobranca.ClienteKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmBoletos_Cobranca.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmBoletos_Cobranca.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmBoletos_Cobranca.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmBoletos_Cobranca.FormCreate(Sender: TObject);
begin
  Dtmen.Date := StrToDate('01/' + Copy(DateToStr(date), 4, 7));
  Dtmai.Date := Ult_Dia_Mes(date);
end;

procedure TFrmBoletos_Cobranca.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TFrmBoletos_Cobranca.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TFrmBoletos_Cobranca.Tipo_BoletoChange(Sender: TObject);
begin
  if (Tipo_Boleto.Text = 'Boleto Simples') OR (Tipo_Boleto.Text = 'Duplicata Mercantil Mod. 1') OR (Tipo_Boleto.Text = 'Duplicata Mercantil Mod. 2')  Then
  Begin
   Local_Pagto.Enabled := True;
   Instrucoes.Enabled  := True;
  End
  Else
  Begin
   Local_Pagto.Enabled := False;
