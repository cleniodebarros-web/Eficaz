...
type
  TFrmConfig_NFe = class(TForm)
    ...
    QInsert: TFDQuery;
    TabSheet4: TTabSheet;
    Inf_Adicional_Cte: TMemo;
    cbCriptografia: TCheckBox;
    Tp_criptografia: TComboBox;
...
begin
  try
    ...
    begin
      ...
      begin
        Application.MessageBox('Escolha o arquivo para continuar com o teste.', PChar(Msg_Title), MB_ICONSTOP + MB_TASKMODAL);
        e_mail := False;
      end;
    end
    else
    begin
      Application.MessageBox('Preencha o email para teste!', PChar(Msg_Title), MB_ICONSTOP + MB_TASKMODAL);
      e_mail := False;
    end;
  except
...
begin
  GravarConfiguracao;
    ...
    IF Tp_criptografia.Text = 'SSL' Then
    Begin
    ...
    End
    Else if  Tp_criptografia.Text = 'WinCrypt' Then
    Begin
...
begin
 ...
 QRel.SQL.Clear;
 QRel.SQL.Add('SELECT * FROM CONFIG_NFISCAL WHERE EMPRESA_ID = :EMPRESA_ID');
 ...
 begin
  ...
  QInsert.SQL.Clear;
  QInsert.SQL.Add('UPDATE CONFIG_NFISCAL SET EMPRESA_ID = :EMPRESA_ID WHERE EMPRESA_ID IS NULL');
  ...
  IniName := 'NFe_'+ FrmPrincipal.QEmpresa.FieldByName('EMPRESA_ID').AsString;

  IniFile := ChangeFileExt(ExtractFilePath(ParamStr(0)) + IniName , '.Ini');
  ...
  try
    ...
    cbLT_TLSv1_2.Checked    := Ini.ReadBool('Certificado', 'LT_TLSv1_2', False);
    ...
    Inf_Adicional_Cte.Lines.LoadFromStream(StreamMemo);
  ...
  //begin
    ...
    edtCaminho.Text         := QRel.FieldByName('CAMINHO').AsString;
    tp_criptografia.Text    := QRel.FieldByName('TP_CRIPTOGRAFIA').AsString;
...
begin
  ...
  try
    ...
    Ini.WriteBool('Certificado', 'LT_TLSv1_2', cbLT_TLSv1_2.Checked);
    ...
    StreamMemo := TMemoryStream.Create;
    Inf_Adicional_CTE.Lines.SaveToStream(StreamMemo);
    StreamMemo.Seek(0, soFromBeginning);
    Ini.WriteBinaryStream('Informações Adicionais Cte', 'Texto', StreamMemo);
    StreamMemo.Free;
    ...
    QInsert.SQL.Clear;
    QInsert.SQL.Add('UPDATE CONFIG_NFISCAL SET SENHA_CERTIFICADO = :SENHA, IDCSC = :IDCSC, ');
    QInsert.SQL.Add('CSC = :CSC, NUMERO = :NUMERO, TIME_OUT = :TIME_OUT, SERVIDOR_SMTP = :SERVIDOR , PORTA = :PORTA,');
    QInsert.SQL.Add('USUARIO = :USUARIO, SENHA_EMAIL = :SENHA_EMAIL, SSL = :SSL,TP_CERTIFICADO = :TP_CERTIFICADO, TP_CRIPTOGRAFIA = :TP_CRIPTOGRAFIA');
    QInsert.SQL.Add(',CERTIFICADO = :CERTIFICADO WHERE EMPRESA_ID = :EMPRESA_ID' );
    QInsert.ParamByName('EMPRESA_ID').AsInteger := FrmPrincipal.QEmpresa.FieldByName('EMPRESA_ID').AsInteger;

    if (tp_certificado.Text = 'A1') AND (edtCaminho.Text <> '') then
     QInsert.ParamByName('CERTIFICADO').LoadFromFile(edtCaminho.Text, FTBlob)
     else
     QInsert.ParamByName('CERTIFICADO').AsByteStr := QRel.FieldByName('CERTIFICADO').AsString;


    QInsert.ParamByName('SENHA').AsString          := edtSenha.Text;
    QInsert.ParamByName('IDCSC').AsString          := IdCSC.Text;
    QInsert.ParamByName('CSC').AsString            := CSC.Text;
    QInsert.ParamByName('NUMERO').AsString         := edtNumSerie.Text;
    QInsert.ParamByName('TIME_OUT').AsString       := EdtTimeOut.Text;
    QInsert.ParamByName('SERVIDOR').AsString       := edtSmtpHost.Text;
    QInsert.ParamByName('PORTA').AsString          := edtSmtpPort.Text;
    QInsert.ParamByName('USUARIO').AsString        := edtSmtpUser.Text;
    QInsert.ParamByName('SENHA_EMAIL').AsString    := EncryptMsg(edtSmtpPass.Text, 5);
    QInsert.ParamByName('TP_CERTIFICADO').AsString := tp_certificado.Text;
    QInsert.ParamByName('TP_CRIPTOGRAFIA').AsString := Tp_criptografia.Text;

    if cbEmailSSL.Checked then
    QInsert.ParamByName('SSL').AsString            := 'True'
    Else
    QInsert.ParamByName('SSL').AsString            := 'False';

    End
    else
    begin
    QInsert.SQL.Clear;
    QInsert.SQL.Add('INSERT INTO CONFIG_NFISCAL(CAMPO,SENHA_CERTIFICADO , IDCSC, CSC,NUMERO,TIME_OUT,SERVIDOR_SMTP,PORTA,');
    QInsert.SQL.Add('USUARIO,SENHA_EMAIL,SSL,TP_CERTIFICADO,CERTIFICADO,CAMINHO,EMPRESA_ID)');
    QInsert.SQL.Add('VALUES(:EMPRESA, :SENHA, :IDCSC,:CSC, :NUMERO, :TIME_OUT, :SERVIDOR , :PORTA, ');
    QInsert.SQL.Add(':USUARIO, :SENHA_EMAIL, :SSL, :TP_CERTIFICADO, :CERTIFICADO, :CAMINHO, :EMPRESA_ID)');
    ...
    if (tp_certificado.Text = 'A1') AND (edtCaminho.Text <> '') then
     ...
     QInsert.ParamByName('CERTIFICADO').AsByteStr := '';

    QInsert.ParamByName('EMPRESA').AsString        := Copy(FrmPrincipal.QEmpresa.FieldByName('RAZAO').AsString,1,40);
    QInsert.ParamByName('SENHA').AsString          := edtSenha.Text;
    QInsert.ParamByName('IDCSC').AsString          := IdCSC.Text;
    QInsert.ParamByName('CSC').AsString            := CSC.Text;
    QInsert.ParamByName('NUMERO').AsString         := edtNumSerie.Text;
    ...
    QInsert.ParamByName('SENHA_EMAIL').AsString    := EncryptMsg(edtSmtpPass.Text, 5);
    QInsert.ParamByName('TP_CERTIFICADO').AsString := tp_certificado.Text;
    QInsert.ParamByName('CAMINHO').AsString        := edtCaminho.Text;
    QInsert.ParamByName('EMPRESA_ID').AsInteger    := FrmPrincipal.QEmpresa.FieldByName('EMPRESA_ID').AsInteger;
