...
var
  FrmRecuperar_Xml: TFrmRecuperar_Xml;
...
procedure TFrmRecuperar_Xml.DeleteIECache;
var
...
function TFrmRecuperar_Xml.DownloadFile(SourceFile, DestFile: string): Boolean;
...
function TFrmRecuperar_Xml.StripHTML(S: string): string;
var
...
procedure TFrmRecuperar_Xml.btnPegarHTMLClick(Sender: TObject);
begin
  PegarHTML;
end;

procedure TFrmRecuperar_Xml.PegarHTML;
begin
...
procedure TFrmRecuperar_Xml.FormCreate(Sender: TObject);
begin
  NovaConsulta;
end;

procedure TFrmRecuperar_Xml.WebBrowser1DocumentComplete(Sender: TObject;
  const pDisp: IDispatch; var URL: OleVariant);
var
  k, i: Integer;
  Source, dest: string;
...
procedure TFrmRecuperar_Xml.WebBrowser1ProgressChange(Sender: TObject; Progress,
  ProgressMax: Integer);
begin
...
procedure TFrmRecuperar_Xml.btnNovaConsultaClick(Sender: TObject);
begin
  NovaConsulta;
end;

procedure TFrmRecuperar_Xml.NovaConsulta;
begin
  btnNovaConsulta.Enabled := False;
  btnGerarXML.Enabled     := False;
  edtcaptcha.Text         :='';
  DeleteIECache;
  Memo2.Lines.Clear;
  WebBrowser1.Navigate('http://www.nfe.fazenda.gov.br/portal/consulta.aspx?tipoConsulta=completa&tipoConteudo=XbSeqxE8pl8%3d');
  //WebBrowser1.Navigate('http://www.nfe.fazenda.gov.br/portal/consulta.aspx?tipoConsulta=completa&tipoConteudo=XbSeqxE8pl8=');
end;

procedure TFrmRecuperar_Xml.btnGerarXMLClick(Sender: TObject);
begin
  GeraXML;
end;

procedure TFrmRecuperar_Xml.GeraXml;
begin
...
procedure TFrmRecuperar_Xml.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmRecuperar_Xml.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmRecuperar_Xml.btnSairClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmRecuperar_Xml.Label3Click(Sender: TObject);
begin
  NovaConsulta;
end;
