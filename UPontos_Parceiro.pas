...
// 125, 327
  QRel.Sql.Clear;
  QRel.Sql.Add('SELECT DT_TRANS, VALOR ,CREDITO_PARCEIRO , DEBITO_PARCEIRO, HIST, NOME, LEGENDA');
  QRel.Sql.Add('FROM(');

  QRel.Sql.Add('SELECT DT_TRANS, VALOR , PARCEIRO_PTS CREDITO_PARCEIRO, 0 DEBITO_PARCEIRO, SUBSTRING( TRANSACOES.historico FROM 7 for 40) as HIST, FUNCIONARIOS.NOME, FINALIZADORAS.LEGENDA');
  ...
  QRel.Sql.Add('SELECT DT_TRANS, 0 VALOR , PARCEIRO_PTS CREDITO_PARCEIRO, (VALOR) DEBITO_PARCEIRO  ,  SUBSTRING( TRANSACOES.historico FROM 0 for 40) as HIST, FUNCIONARIOS.NOME, ''Dinheiro'' LEGENDA');
  QRel.Sql.Add('FROM TRANSACOES');
  QRel.Sql.Add('INNER JOIN FUNCIONARIOS');
  QRel.Sql.Add('ON (FUNCIONARIOS.FUNCIONARIO_ID = TRANSACOES.PARCEIRO_ID)');
  QRel.Sql.Add('WHERE (TRANSACOES.EMPRESA_ID = :EMPRESA_ID)');
  QRel.Sql.Add('AND (TRANSACOES.CONDUTA = ''05'')');
  QRel.Sql.Add('AND (TRANSACOES.DEPTO = ''05'')');
  QRel.Sql.Add('AND (TRANSACOES.DT_TRANS BETWEEN :DT_INICIAL AND :DT_FINAL)');
  ...
  begin
    QRel.Sql.Clear;
    QRel.Sql.Add('SELECT NOME, LEGENDA , SUM(VALOR) VALOR ,SUM(CREDITO_PARCEIRO) CREDITO_PARCEIRO , SUM(DEBITO_PARCEIRO) DEBITO_PARCEIRO ');
    QRel.Sql.Add('FROM(');

    QRel.Sql.Add('SELECT FUNCIONARIOS.NOME, FINALIZADORAS.LEGENDA,  (TRANSACOES.VALOR) VALOR, (PARCEIRO_PTS) CREDITO_PARCEIRO , 0 DEBITO_PARCEIRO');
    QRel.Sql.Add('FROM TRANSACOES');
    QRel.Sql.Add('INNER JOIN FUNCIONARIOS');
    QRel.Sql.Add('ON (FUNCIONARIOS.FUNCIONARIO_ID = TRANSACOES.PARCEIRO_ID)');
    QRel.Sql.Add('LEFT JOIN FINALIZADORAS');
    ...
    begin
     ...
     QRel.Sql.Add('UNION ALL');
     QRel.Sql.Add('SELECT FUNCIONARIOS.NOME, ''Dinheiro'' LEGENDA , 0 VALOR , 0 CREDITO_PARCEIRO, (VALOR) DEBITO_PARCEIRO');
     QRel.Sql.Add('FROM TRANSACOES');
     QRel.Sql.Add('INNER JOIN FUNCIONARIOS');
     QRel.Sql.Add('ON (FUNCIONARIOS.FUNCIONARIO_ID = TRANSACOES.PARCEIRO_ID)');
     QRel.Sql.Add('WHERE (TRANSACOES.EMPRESA_ID = :EMPRESA_ID)');
...
begin
  try
    Vendedor.Value := GetConsulta('PARCEIRO', 0, 0, StrToInt(Vendedor.Text));
  except
    Vendedor.Value := GetConsulta('PARCEIRO', 0, 0, 0);
  end;
end;
