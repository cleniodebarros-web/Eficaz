...
// 325
  ...
  try
    ...
    begin
       QRel.Sql.Add('SELECT VENDEDOR, PRODUTO_ID, DESCRICAO, COD_BARRA, CLIENTES, SUM(VR_DESCONTO) VR_DESCONTO, SUM(VR_ACRESCIMO) VR_ACRESCIMO,');
       QRel.Sql.Add('SUM(QTD_VENDA) QTD_VENDA, SUM(VR_TOTAL) VR_TOTAL, MAX(DT_ENTREGA) DT_ENTREGA, MAX(PEDIDOS.DT_PEDIDO) DT_PEDIDO, STATUS, MAX(PEDIDO_ID) PEDIDO_ID FROM(');
    end;


    QRel.Sql.Add('SELECT FUNCIONARIOS.NOME VENDEDOR,PEDITENS.PRODUTO_ID,PRODUTOS.DESCRICAO,PRODUTOS.PRECO_VAREJO, PRODUTOS.VALOR_COMPRA, PRODUTOS.COD_BARRA, PEDIDOS.NOME CLIENTES,PEDIDOS.VR_DESCONTO,PEDIDOS.VR_ACRESCIMO, SUM(PEDITENS.QUANTIDADE) QTD_VENDA, ' +
                 'SUM(PEDITENS.VR_TOTAL - PEDITENS.DESC_RODAPE) VR_TOTAL, PEDIDOS.DT_ENTREGA,PEDIDOS.DT_PEDIDO,PEDIDOS.STATUS,PEDIDOS.PEDIDO_ID');
    ...
    QRel.Sql.Add('GROUP BY PEDIDOS.PEDIDO_ID,PEDIDOS.NOME, PEDITENS.PRODUTO_ID, PRODUTOS.DESCRICAO ,');
    QRel.Sql.Add('PRODUTOS.COD_BARRA, PEDIDOS.DT_ENTREGA,PEDIDOS.DT_PEDIDO,PEDIDOS.STATUS,PEDIDOS.VR_DESCONTO,PEDIDOS.VR_ACRESCIMO,FUNCIONARIOS.NOME,PRODUTOS.PRECO_VAREJO,PRODUTOS.VALOR_COMPRA');
    ...
    if chk_agrupar.Checked then
    begin
       QRel.Sql.Add(') AS RESULT GROUP BY PRODUTO_ID, DESCRICAO, COD_BARRA, CLIENTES, VR_DESCONTO, VR_ACRESCIMO, STATUS,VENDEDOR');
