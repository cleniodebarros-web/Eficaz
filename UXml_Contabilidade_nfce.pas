...
type
  TFrmXml_Contabilidade_nfce = class(TForm)
    Label1: TLabel;
    Dtmen: TDateEdit;
    Dtmai: TDateEdit;
    Bevel1: TBevel;
...
var
  FrmXml_Contabilidade_nfce: TFrmXml_Contabilidade_nfce;
  ZipFile: TZipFile;
  dir,dest_dir:string;
...
procedure TFrmXml_Contabilidade_nfce.Procura;

begin
  QNotas.Sql.Clear;
  QNotas.Sql.Add('SELECT * FROM COMPL_NFCEFISCAL');
  ...
  QNotas.ParamByName('EMPRESA_ID').AsInteger  := FrmData.QAcesso.FieldByName('EMPRESA_ID').AsInteger;
  ...
  begin
   ...
   FrmPrincipal.ACBrNFe1.NotasFiscais.GravarXML(FrmPrincipal.ACBrNFe1.Configuracoes.Arquivos.PathNFe + '\' + QNotas.FieldByName('CHAVE_NFCE').AsString + '-nfe.Xml' );
    ...
    if Chk_Pdf.Checked then
    Begin
    FrmPrincipal.ACBrNFeDANFeRL1.PathPDF :=  FrmPrincipal.ACBrNFe1.Configuracoes.Arquivos.PathNFe ;
    FrmPrincipal.ACBrNFe1.NotasFiscais.ImprimirPDF;//(Dir + '\' + QNotas.FieldByName('CHAVE_NFCE').AsString + '-nfe.Xml' );
    End;
  ...
  Begin
    ...
    begin
      ...
      if FileExists(FrmPrincipal.ACBrNFe1.Configuracoes.Arquivos.PathNFe + '\' + QNotas.FieldByName('CHAVE_NFCE').AsString + '-nfe.pdf') then
     // FileExists(ExtractFilePath(ParamStr(0)) + 'NFe\' + QNotas.FieldByName('CHAVE_NFE').AsString + '-nfe.Xml') then
        ZipFile.Add(FrmPrincipal.ACBrNFe1.Configuracoes.Arquivos.PathNFe + '\' + QNotas.FieldByName('CHAVE_NFCE').AsString + '-nfe.pdf');
  ...
  begin
    ...
    DeleteFile(FrmPrincipal.ACBrNFe1.Configuracoes.Arquivos.PathNFe + '\' + QNotas.FieldByName('CHAVE_NFCE').AsString + '-nfe.pdf');
    QNotas.Next;
  end;

end;

procedure TFrmXml_Contabilidade_nfce.btnExecutaClick(Sender: TObject);
begin
...
procedure TFrmXml_Contabilidade_nfce.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmXml_Contabilidade_nfce.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmXml_Contabilidade_nfce.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmXml_Contabilidade_nfce.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmXml_Contabilidade_nfce.FormCreate(Sender: TObject);
begin
