...
type
  TFrmRelatorio_Vendas = class(TForm)
    Label1: TLabel;
    Dtmen: TDateEdit;
    Dtmai: TDateEdit;
    Bevel1: TBevel;
...
var
  FrmRelatorio_Vendas: TFrmRelatorio_Vendas;
...
procedure TFrmRelatorio_Vendas.btnExecutaClick(Sender: TObject);
var
I : Integer;
Parametro, Grupo : string;
...
// 325
  ...
  try
    ...
    if Tipo_Rel.Text = 'Sintético' then
       QRel.Sql.Add('SELECT GRUPO, SUM(QTD_VENDA) QTD_VENDA, SUM(VR_TOTAL) VR_TOTAL FROM(');

    if Tipo_Rel.Text = 'Analítico' then
     QRel.Sql.Add('SELECT TRANSACOES.NUM_DOC,TRANSITENS.PRODUTO_ID, PRODUTOS.DESCRICAO, TRANSACOES.DT_ENT_SAI, TABELAS.DESCRICAO GRUPO, SUM(TRANSITENS.QUANTIDADE) QTD_VENDA, ' +
                 'SUM(TRANSITENS.VR_TOTAL + TRANSITENS.VALOR_ICMS_ST + TRANSITENS.VR_FRETE + TRANSITENS.VR_ACRESCIMO + TRANSITENS.VR_IPI + TRANSITENS.VR_FCPST + TRANSITENS.VR_FCP - TRANSITENS.DESC_RODAPE) VR_TOTAL, TRANSITENS.VR_UNITARIO, CLIENTES.NOME CLIENTE')
    else
    ...
    if Tipo_Rel.Text = 'Analítico' then
    QRel.Sql.Add('GROUP BY TRANSACOES.NUM_DOC,TRANSACOES.DT_ENT_SAI,TRANSITENS.PRODUTO_ID, PRODUTOS.DESCRICAO, PRODUTOS.COD_BARRA, TABELAS.DESCRICAO,TRANSITENS.VR_UNITARIO, CLIENTES.NOME')
    Else
   ...
   Begin
    ...
    if Ordem.Text = 'CÓDIGO' then
     ...
     else if Ordem.Text = 'DATA VENDA' then
        ...
        if Tipo_Rel.Text = 'Sintético' then
          ...
          RDprint1.Abrir;
              ...
              QRel.First;
                  ...
                  RDprint1.ImpF(linha, 45, 'Qtd. Venda', [negrito]);
                  RDprint1.ImpF(linha, 61, 'Vr. Venda', [negrito]);
                  Inc(linha);
             ...
             Begin
                ...
                RDprint1.ImpVal(linha, 45, ',###,##0.00', QRel.FieldByName('QTD_VENDA').AsFloat,[]);
                RDprint1.ImpVal(linha, 60, ',###,##0.00', QRel.FieldByName('VR_TOTAL').AsFloat,[]);

                Total_Qtd := Total_Qtd + QRel.FieldByName('QTD_Venda').AsFloat;
...
procedure TFrmRelatorio_Vendas.btnFamiliaClick(Sender: TObject);
begin
    try
...
procedure TFrmRelatorio_Vendas.btnFornecedorClick(Sender: TObject);
begin
  try
...
procedure TFrmRelatorio_Vendas.btnGrupoClick(Sender: TObject);
begin
  try
...
procedure TFrmRelatorio_Vendas.btnLocalizacaoClick(Sender: TObject);
begin
     try
...
procedure TFrmRelatorio_Vendas.btnProd_finalClick(Sender: TObject);
begin
    try
...
procedure TFrmRelatorio_Vendas.btnProd_InicialClick(Sender: TObject);
begin
  try
...
procedure TFrmRelatorio_Vendas.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmRelatorio_Vendas.btnSub_TipoClick(Sender: TObject);
begin
    try
...
procedure TFrmRelatorio_Vendas.BtnTributacaoClick(Sender: TObject);
begin
   try
...
procedure TFrmRelatorio_Vendas.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmRelatorio_Vendas.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmRelatorio_Vendas.ExAllBtnClick(Sender: TObject);
var
I: Integer;
begin
  QCfop.Close;
...
procedure TFrmRelatorio_Vendas.ExcludeBtnClick(Sender: TObject);
var
Index: Integer;
begin
    QCfop.Close;
...
procedure TFrmRelatorio_Vendas.DetailSearch(Tabela: String);
begin
...
procedure TFrmRelatorio_Vendas.FAMILIA_IDChange(Sender: TObject);
begin
   DetailSearch('Familia');
end;

procedure TFrmRelatorio_Vendas.FAMILIA_IDExit(Sender: TObject);
begin
   DetailSearch('Familia');
end;

procedure TFrmRelatorio_Vendas.FAMILIA_IDKeyDown(Sender: TObject;
...
procedure TFrmRelatorio_Vendas.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmRelatorio_Vendas.FormCreate(Sender: TObject);
begin
...
procedure TFrmRelatorio_Vendas.ClienteChange(Sender: TObject);
begin
  DetailSearch('Cliente');
end;

procedure TFrmRelatorio_Vendas.ClienteExit(Sender: TObject);
begin
    DetailSearch('Cliente');
end;

procedure TFrmRelatorio_Vendas.ClienteKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
    if (Key = Vk_F7) and (Sender = Cliente) then
    btnFornecedorClick(Self);
...
procedure TFrmRelatorio_Vendas.IncAllBtnClick(Sender: TObject);
var
I: Integer;
begin
    QCfop.Close;
...
procedure TFrmRelatorio_Vendas.IncludeBtnClick(Sender: TObject);
var
Index: Integer;
begin
    QCfop.Close;
...
procedure TFrmRelatorio_Vendas.LocalizacaoChange(Sender: TObject);
begin
    DetailSearch('Localização');
end;

procedure TFrmRelatorio_Vendas.LocalizacaoExit(Sender: TObject);
begin
    DetailSearch('Localização');
end;

procedure TFrmRelatorio_Vendas.LocalizacaoKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
     if (Key = Vk_F7) and (Sender = FAMILIA_ID) then
    btnGrupoClick(Self);
...
function TFrmRelatorio_Vendas.GetFirstSelection(List: TCustomListBox): Integer;
begin
...
procedure TFrmRelatorio_Vendas.GRUPO_IDChange(Sender: TObject);
begin
   DetailSearch('Grupo');
end;

procedure TFrmRelatorio_Vendas.GRUPO_IDExit(Sender: TObject);
begin
       DetailSearch('Grupo');
end;

procedure TFrmRelatorio_Vendas.GRUPO_IDKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if (Key = Vk_F7) and (Sender = GRUPO_ID) then
     btnGrupoClick(Self);
...
procedure TFrmRelatorio_Vendas.MoveSelected(List: TCustomListBox; Items: TStrings);
var
I: Integer;
begin
...
procedure TFrmRelatorio_Vendas.SetButtons;
var
...
procedure TFrmRelatorio_Vendas.SetItem(List: TListBox; Index: Integer);
var
MaxIndex: Integer;
begin
  with List do
...
procedure TFrmRelatorio_Vendas.SpeedButton1Click(Sender: TObject);
begin
try
...
procedure TFrmRelatorio_Vendas.SrcListClick(Sender: TObject);
var
I: Integer;
begin
  for I := 0 to (SrcList.Items.Count - 1) do
...
procedure TFrmRelatorio_Vendas.SubTipoChange(Sender: TObject);
begin
     DetailSearch('Subtipo');
end;

procedure TFrmRelatorio_Vendas.SubTipoExit(Sender: TObject);
begin
     DetailSearch('Subtipo');
end;

procedure TFrmRelatorio_Vendas.SubTipoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if (Key = Vk_F7) and (Sender = FAMILIA_ID) then
    btnGrupoClick(Self);
...
procedure TFrmRelatorio_Vendas.TributacaoChange(Sender: TObject);
begin
  DetailSearch('Tributação');
end;

procedure TFrmRelatorio_Vendas.TributacaoExit(Sender: TObject);
begin
   DetailSearch('Tributação');
end;

procedure TFrmRelatorio_Vendas.OrdemKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = Vk_Return then
    Perform(Wm_NextDlgctl, 0, 0);
...
procedure TFrmRelatorio_Vendas.Produto_idChange(Sender: TObject);
begin
  DetailSearch('Produtos');
end;

procedure TFrmRelatorio_Vendas.Prod_FinalKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
            if (Key = Vk_F7) and (Sender = FAMILIA_ID) then
    btnGrupoClick(Self);
...
procedure TFrmRelatorio_Vendas.Prod_InicialExit(Sender: TObject);
begin
     DetailSearch('Produto Inicial');
end;

procedure TFrmRelatorio_Vendas.Prod_InicialKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
   if (Key = Vk_F7) and (Sender = FAMILIA_ID) then
    btnGrupoClick(Self);
...
procedure TFrmRelatorio_Vendas.RDprint1NewPage(Sender: TObject;
  Pagina: Integer);
begin
            ...
            RDprint1.impF(03, 26, '*** RELATÓRIO DE VENDAS ***',[negrito]);
