...
type
  TFrmCTe_Canceladas = class(TForm)
    Panel1: TPanel;
    btnRetorna: TBitBtn;
...
var
  FrmCTe_Canceladas: TFrmCTe_Canceladas;
...
procedure TFrmCTe_Canceladas.btnEstornaClick(Sender: TObject);
var
Id_Trans: Integer;
Num_Doc: String;
Vl_Trans: Real;
...
begin
  ...
  QRel.ParamByName('HISTORICO').AsString := 'CANCELAMENTO CTe ' + StrZero(Num_Doc, 6, 0);
...
procedure TFrmCTe_Canceladas.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmCTe_Canceladas.btnSelecionarClick(Sender: TObject);
begin
  ...
  QNotas.SQL.Add('FROM(SELECT CTES_CANCELADAS.TRANSACAO_ID, NO_DOC_FISCAL,DT_SAIDA,HR_SAIDA, CLIENTES.NOME, TRANSACOES.CFOP, TRANSACOES.VALOR,CTES_CANCELADAS.CHAVE_NFE,PROTOCOLO,RECIBO,SINTEGRA, CTES_CANCELADAS.EMPRESA_ID FROM CTES_CANCELADAS ');
  QNotas.SQL.Add('INNER JOIN TRANSACOES ON TRANSACOES.TRANSACAO_ID = CTES_CANCELADAS.TRANSACAO_ID');
  QNotas.SQL.Add('INNER JOIN CLIENTES ON CLIENTES.CLIENTE_ID = TRANSACOES.CLIENTE_ID');
  QNotas.SQL.Add('UNION ALL');
  QNotas.SQL.Add('SELECT  CTES_CANCELADAS.TRANSACAO_ID , NO_DOC_FISCAL,DT_SAIDA,HR_SAIDA, FORNECEDORES.NOME, TRANSACOES.CFOP, TRANSACOES.VALOR,CTES_CANCELADAS.CHAVE_NFE,PROTOCOLO,RECIBO,SINTEGRA, CTES_CANCELADAS.EMPRESA_ID FROM CTES_CANCELADAS');
  QNotas.SQL.Add('INNER JOIN TRANSACOES ON TRANSACOES.TRANSACAO_ID = CTES_CANCELADAS.TRANSACAO_ID');
  ...
  Begin
  QNotas.Sql.Clear;
  QNotas.Sql.Add('SELECT TRANSACAO_ID, NO_DOC_FISCAL, DT_SAIDA,HR_SAIDA,CHAVE_NFE,PROTOCOLO,RECIBO,SINTEGRA,EMPRESA_ID, 0 VALOR ');
  QNotas.SQL.Add('FROM CTES_CANCELADAS');
...
procedure TFrmCTe_Canceladas.DtmaiKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    btnSelecionarClick(Sender);
end;

procedure TFrmCTe_Canceladas.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmCTe_Canceladas.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmCTe_Canceladas.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmCTe_Canceladas.FormCreate(Sender: TObject);
begin
...
procedure TFrmCTe_Canceladas.NotasDblClick(Sender: TObject);
begin
  QRel.Sql.Clear;
  QRel.Sql.Add('SELECT * FROM TRANSACOES');
  QRel.Sql.Add('WHERE');
...
procedure TFrmCTe_Canceladas.NotasKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  ...
  begin
    ...
    QRel.Sql.Clear;
    QRel.Sql.Add('UPDATE CTES_CANCELADAS SET SINTEGRA = :STATUS');
    QRel.Sql.Add('WHERE');
    QRel.Sql.Add('(TRANSACAO_ID = :TRANSACAO_ID)');
...
procedure TFrmCTe_Canceladas.NotasKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    QRel.Sql.Clear;
...
procedure TFrmCTe_Canceladas.QNotasAfterOpen(DataSet: TDataSet);
begin
