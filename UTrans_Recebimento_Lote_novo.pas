...
var
Valor, Valor_j, Percentual,Valor_Help,vr_quitacao,Valor_Log: Real;
ID : Integer;
...
begin
  ...
  while not DBGrid1.DataSource.DataSet.Eof do
  begin
    if (Vr_Quitar.Value > 0) AND (NOT Selecionar_Parcelas) then
    begin
      if Vr_Quitar.Value <= Vr_Total.Value then
      begin
        if Valor > 0 then
        begin
          ...
          QRel.Sql.Add('INSERT INTO TRANSACOES(TRANSACAO_ID, ' +
                       ...
                       'VALOR,          BALANCO,           TPCTB, ' +
                       'AUTORIZ_ID,     HISTORICO,         HORA, PAGAMENTO,COMPLEMENTO,SUB_CODIGO,FINALIZADORA_ID) VALUES(:TRANSACAO_ID,' +
                       ':DT_TRANS,      :DT_MOVIMENTO,     :CONDUTA, ' +
                       ':EMPRESA_ID,    :CLIENTE_ID,       :BANCO_ID, ' +
                       ':NUM_DOC,       :SERIE,            :FLUXO_CAIXA_ID, ' +
                       ':SAIDA_ID,      :PARCELA_ID,       :VALOR_PARCELA, ' +
                       ':VALOR,         :BALANCO,          :TPCTB, ' +
                       ':AUTORIZ_ID,    :HISTORICO,        :HORA, :PAGAMENTO,:COMPLEMENTO,:SUB_CODIGO,:FINALIZADORA_ID)');
          ...
          begin
            ...
            VaLOR_LOG :=  DBGrid1.DataSource.DataSet.FieldByName('VALOR_TOTAL').AsFloat;
         ...
         If (FrmPrincipal.Config.FieldByName('AUDITORIA').AsString = 'True') and (valor > 0) Then
          GravarLog('RECEBIMENTO','INSERIR',DateToStr(date),TimeToStr(time),'Transação_id ' , IntToStr(id), FloatToStr(valor_log) + '          Documento: ' + DBGrid1.DataSource.DataSet.FieldByName('DUPLICATA').AsString ,FrmData.QAcesso.FieldByName('FUNCIONARIO_ID').AsInteger,DBGrid1.DataSource.DataSet.FieldByName('CLIENTE_ID').AsInteger);
      ...
      begin
          ...
          QRel.Sql.Add('INSERT INTO TRANSACOES(' +
                       ...
                       'COMPLEMENTO,    HORA,              PAGAMENTO,FINALIZADORA_ID) VALUES(' +
                       ':DT_TRANS,      :DT_MOVIMENTO,     :CONDUTA, ' +
                       ':EMPRESA_ID,    :CLIENTE_ID,       :BANCO_ID, ' +
                       ':NUM_DOC,       :SERIE,            :FLUXO_CAIXA_ID, ' +
                       ':SAIDA_ID,      :PARCELA_ID,       :VALOR_PARCELA, ' +
                       ':VALOR,         :BALANCO,          :TPCTB, ' +
                       ':AUTORIZ_ID,    :HISTORICO,        :CONTA_ID, ' +
                       ':COMPLEMENTO,   :HORA,             :PAGAMENTO,:FINALIZADORA_ID)');
         ...
         If (FrmPrincipal.Config.FieldByName('AUDITORIA').AsString = 'True') Then
          GravarLog('RECEBIMENTO','INSERIR',DateToStr(date),TimeToStr(time),'Transação_id ' ,
          IntToStr(id), FloatToStr((DBGrid1.DataSource.DataSet.FieldByName('VALOR_TOTAL').AsFloat +
          ((DBGrid1.DataSource.DataSet.FieldByName('VALOR_TOTAL').AsFloat * Percentual) / 100))) +
          '         Documento: ' + DBGrid1.DataSource.DataSet.FieldByName('NUM_DOC').AsString  ,
          FrmData.QAcesso.FieldByName('FUNCIONARIO_ID').AsInteger,
          DBGrid1.DataSource.DataSet.FieldByName('CLIENTE_ID').AsInteger);
    ...
    begin
      ...
      begin
       ...
       begin
        ...
        If (FrmPrincipal.Config.FieldByName('AUDITORIA').AsString = 'True') and (DBGrid1.DataSource.DataSet.FieldByName('VALOR_TOTAL').AsFloat > 0) Then
          GravarLog('RECEBIMENTO','INSERIR',DateToStr(date),TimeToStr(time),'Transação_id ' , IntToStr(id),FloatToStr(DBGrid1.DataSource.DataSet.FieldByName('VALOR_TOTAL').AsFloat) + '         Documento: ' + DBGrid1.DataSource.DataSet.FieldByName('NUM_DOC').AsString  ,FrmData.QAcesso.FieldByName('FUNCIONARIO_ID').AsInteger,DBGrid1.DataSource.DataSet.FieldByName('CLIENTE_ID').AsInteger);


          QRel.Sql.Clear;
          QRel.Sql.Add('UPDATE TRANSPARCELAS SET  VALOR_ORI = :VALOR_ORI, DT_VENC_ORI = :DT_VENC_ORI, VALOR_JUROS = :VALOR_JUROS, DT_PAGAMENTO =:DT_PAGAMENTO');
          QRel.Sql.Add('WHERE');
          QRel.Sql.Add('(TRANSACAO_ID   = :TRANSACAO_ID)');
          QRel.Sql.Add('AND (PARCELA_ID = :PARCELA_ID)');
    ...
    Begin
     ...
     Begin
      ...
      begin
          ...
          Divisao_Parcelas(Id_Trans, 'T','1', True,FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime, soma_agrupar);
