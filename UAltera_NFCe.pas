...
var
  FrmAltera_NFCe: TFrmAltera_NFCe;
  ID: Integer;
  GeraXML: Boolean;
  Cod_CFOP: String;

  procedure Altera_NFCe(Transacao_Id: Integer; CFOP: String);
...
begin
  ...
  Application.CreateForm(TFrmAltera_NFCe, FrmAltera_NFCe);
  try
    FrmAltera_NFCe.ShowModal;
  finally
    FrmAltera_NFCe.Release;
  end;
end;

procedure TFrmAltera_NFCe.Set_Campos(Vazio: Boolean);
var
I: Integer;
Temp: TComponent;
begin
...
procedure TFrmAltera_NFCe.Insert;
var
I: Integer;
Temp: TComponent;
Sql, Par: String;
begin
  Sql := 'INSERT INTO COMPL_NFCEFISCAL(';
  Par := '';
  ...
  begin
    ...
    if Temp is TEdit then
    begin
      if Sql = 'INSERT INTO COMPL_NFCEFISCAL(' then
...
procedure TFrmAltera_NFCe.Edit;
var
I: Integer;
Temp: TComponent;
Sql: String;
begin
  Sql := 'UPDATE COMPL_NFCEFISCAL SET ';
  ...
  begin
    ...
    if Temp is TEdit then
    begin
      if Sql = 'UPDATE COMPL_NFCEFISCAL SET ' then
...
function TFrmAltera_NFCe.Validacao: Boolean;
begin
 Result := False;
 ...
 IF (QParceiro.IsEmpty) and (QTabela.FieldByName('VALOR').AsFloat > 3000)  then
 begin
 Application.MessageBox('NFC-e sem identificação de Cliente.', PChar(Msg_Title), mb_IconStop);
 Exit;
 end;
...
procedure TFrmAltera_NFCe.DetailSearch(Tabela: String);
begin

end;

function TFrmAltera_NFCe.Search_Campo(Txt: String): Integer;
begin
...
procedure TFrmAltera_NFCe.ProdutoSearch(ID: Integer);
begin
  QProdutos.Sql.Clear;
...
procedure TFrmAltera_NFCe.btnFornecedorClick(Sender: TObject);
var
Forne: Integer;
begin
...
procedure TFrmAltera_NFCe.btnGeraClick(Sender: TObject);
var
...
begin
  ...
  begin
    ...
    QTabela.Sql.Clear;
    QTabela.Sql.Add('SELECT * FROM COMPL_NFCEFISCAL');
    ...
    try
     ...
     End;
      ...
      FrmPrincipal.ACBrNFe1.Configuracoes.Geral.ModeloDF       := moNFCe;
...
// alteracao da linha anterior a pedido de comercial bras nao enviar pRedBc percentual e sim valor
      ...
      begin
        ...
        begin
         ...
         // IQuery.ParamByName('HISTORICO').AsString     := QTransacao.FieldByName('HISTORICO').AsString  + StrZero(NO_DOC_FISCAL.Text, 9, 0);
          ...
          IQuery.Sql.Clear;
          IQuery.Sql.Add('UPDATE COMPL_NFCEFISCAL SET RECIBO = :RECIBO, PROTOCOLO = :PROTOCOLO, CHAVE_NFE = :CHAVE_NFE');
          IQuery.Sql.Add('WHERE');
          IQuery.Sql.Add('(TRANSACAO_ID = :TRANSACAO_ID)');
        ...
        begin
          IQuery.Sql.Clear;
          IQuery.Sql.Add('DELETE FROM COMPL_NFCEFISCAL');
          IQuery.Sql.Add('WHERE');
          IQuery.Sql.Add('(TRANSACAO_ID = :TRANSACAO_ID)');
      ...
      begin
       ...
       // FrmPrincipal.ACBrNFe1.NotasFiscais.LoadFromFile(FrmPrincipal.ACBrNFe1.Configuracoes.Arquivos.GetPathNFe + Copy(FrmPrincipal.ACBrNFe1.NotasFiscais.Items[0].NFe.infNFe.ID, 4, 44)+'-nfe.xml');
        ...
        FrmPrincipal.ACBrNFe1.Configuracoes.Geral.ModeloDF       := moNFe;
        ...
        Begin
          ...
          IQuery.Sql.Clear;
          IQuery.Sql.Add('UPDATE COMPL_NFCEFISCAL SET CHAVE_NFE = :CHAVE_NFE, RECIBO = :RECIBO, PROTOCOLO = :PROTOCOLO');
          IQuery.Sql.Add('WHERE');
          IQuery.Sql.Add('(TRANSACAO_ID = :TRANSACAO_ID)');
...
procedure TFrmAltera_NFCe.CFOP_COMPLExit(Sender: TObject);
begin
  DetailSearch('CFOP');
end;

procedure TFrmAltera_NFCe.CNPJ_TRANSPORTADORKeyDown(Sender: TObject;
...
procedure TFrmAltera_NFCe.DT_SAIDAEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmAltera_NFCe.DT_SAIDAKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmAltera_NFCe.FormCreate(Sender: TObject);
begin
  GeraXML := True;
end;

procedure TFrmAltera_NFCe.FormShow(Sender: TObject);
var
IniFile: String;
Ini: TIniFile;
StreamMemo: TMemoryStream;
...
procedure TFrmAltera_NFCe.GerarXMLClick(Sender: TObject);
begin
  btnGeraClick(Sender);
end;
