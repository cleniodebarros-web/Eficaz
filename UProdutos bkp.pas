...
uses
  ...
  UConsulta_NCM, UConsulta_Contribuicao,UEtiquetas_novos,UDetalhes_Produto,UConsulta_CEST,UTransf_Loc_Estoque;
...
begin
  ...
  {
  QSearch.Sql.Clear;
  QSearch.Sql.Add('SELECT DT_TRANS, DT_ENT_SAI, TRANSITENS.VR_UNITARIO VALOR_COMPRA, ');
  QSearch.Sql.Add('(TRANSITENS.VR_UNITARIO - TRANSITENS.VR_DESCONTO) CUSTO_COMPRA, TRANSACOES.FORNECEDOR_ID');
  QSearch.Sql.Add('FROM TRANSITENS');
  QSearch.Sql.Add('INNER JOIN TRANSACOES ON TRANSACOES.TRANSACAO_ID = TRANSITENS.TRANSACAO_ID');
  QSearch.Sql.Add('INNER JOIN FORNECEDORES ON TRANSACOES.FORNECEDOR_ID = FORNECEDORES.FORNECEDOR_ID');
  QSearch.Sql.Add('WHERE CONDUTA = ''02'' AND DEPTO = ''07'' AND DT_ENT_SAI >= :DT_ENT_SAI');
  ...
  csto_compra   := QSearch.FieldByName('CUSTO_COMPRA').AsFloat + Vr_IcmsSt.Value + Vr_Frete.value + Vr_Ipi.Value + Vr_Despesas.Value + Dif_Icms.Value;
