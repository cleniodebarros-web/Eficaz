...
type
  TFrmTrans_DebitoParceiro = class(TForm)
    ...
    procedure PARCEIRO_IDExit(Sender: TObject);
...
var
  FrmTrans_DebitoParceiro: TFrmTrans_DebitoParceiro;
...
procedure TFrmTrans_DebitoParceiro.PARCEIRO_IDExit(Sender: TObject);
begin
  DetailSearch('Parceiro');
end;

procedure TFrmTrans_DebitoParceiro.CFOPExit(Sender: TObject);
begin
  DetailSearch('CFOP');
end;

procedure TFrmTrans_DebitoParceiro.CONTA_IDExit(Sender: TObject);
begin
  DetailSearch('Conta');
end;

procedure TFrmTrans_DebitoParceiro.CST_COFINSChange(Sender: TObject);
begin
  //CST_PIS.Text := CST_COFINS.Text;
end;

procedure TFrmTrans_DebitoParceiro.CST_PISChange(Sender: TObject);
begin
  //CST_COFINS.Text := CST_PIS.Text;
end;

procedure TFrmTrans_DebitoParceiro.C_CUSTO_IDExit(Sender: TObject);
begin
  DetailSearch('C. Custo');
end;

procedure TFrmTrans_DebitoParceiro.BANCO_IDExit(Sender: TObject);
begin
  DetailSearch('Banco');
end;

procedure TFrmTrans_DebitoParceiro.BASE_CALC_COFINSExit(Sender: TObject);
begin
//Base_Calc_pis.Value := Base_Calc_Cofins.Value;
end;

procedure TFrmTrans_DebitoParceiro.BASE_CALC_PISExit(Sender: TObject);
begin
//Base_Calc_Cofins.Value := Base_calc_pis.Value;
end;

procedure TFrmTrans_DebitoParceiro.Botoes_Editing;
begin
...
procedure TFrmTrans_DebitoParceiro.Botoes_Normal;
begin
...
procedure TFrmTrans_DebitoParceiro.DetailSearch(Tabela: String);
begin
  ...
  if ((Tabela = '') or (Tabela = 'Parceiro')) and (PARCEIRO_ID.Text <> '') then
  begin
    QParceiro.Close;

    QParceiro.ParamByName('PARCEIRO_ID').AsInteger := StrToInt(PARCEIRO_ID.Text);
  ...
  if ((Tabela = '') or (Tabela = 'Funcionário')) and (FUNCIONARIO_ID.Text <> '') then
  begin
    QFuncionario.Close;
...
procedure TFrmTrans_DebitoParceiro.DiasClick(Sender: TObject);
begin
  ...
  begin
    ...
    QTabela.ParamByName('CONDUTA').AsString        := '05';
    QTabela.ParamByName('DEPTO').AsString          := '05';
...
procedure TFrmTrans_DebitoParceiro.DT_TRANSEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmTrans_DebitoParceiro.DT_TRANSKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {
...
procedure TFrmTrans_DebitoParceiro.Habilitar(Status: Boolean);
var
I: Integer;
Temp: TComponent;
begin
...
procedure TFrmTrans_DebitoParceiro.Insert;
var
I: Integer;
Temp: TComponent;
Sql, Par: String;
...
procedure TFrmTrans_DebitoParceiro.Edit;
var
I: Integer;
Temp: TComponent;
Sql: String;
...
procedure TFrmTrans_DebitoParceiro.EMPRESA_IDExit(Sender: TObject);
begin
  DetailSearch('Empresa');
end;

procedure TFrmTrans_DebitoParceiro.EMPRESA_IDKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (Key = Vk_F7) and (Sender = EMPRESA_ID) then
    btnEmpresaClick(Self);

  if (Key = Vk_F7) and (Sender = PARCEIRO_ID) then
...
function TFrmTrans_DebitoParceiro.Validacao: Boolean;
...
begin
 ...
 // BANCO_ID.Color         := clWindow;
  PARCEIRO_ID.Color      := clWindow;
  FUNCIONARIO_ID.Color   := clWindow;
  ...
  begin
    ...
    if (PARCEIRO_ID.Value >= 0) and (QParceiro.IsEmpty) then
    begin
      Application.MessageBox('Parceiro inexistente', PChar(Msg_Title), mb_IconStop);
      PARCEIRO_ID.Color := clYellow;
      PARCEIRO_ID.SetFocus;
      exit;
    end;
 { end
  else
  ...
  begin
    ...
    begin
      Application.MessageBox('Funcionario inexistente', PChar(Msg_Title), mb_IconStop);
  ...
  if PARCEIRO_ID.Value > 0 then
  begin
    if QParceiro.IsEmpty then
    begin
      Application.MessageBox('Parceiro inexistente', PChar(Msg_Title), mb_IconStop);
      PARCEIRO_ID.Color := clYellow;
      PARCEIRO_ID.SetFocus;
...
procedure TFrmTrans_DebitoParceiro.FUNCIONARIO_IDExit(Sender: TObject);
begin
  DetailSearch('Funcionário');
end;

procedure TFrmTrans_DebitoParceiro.ManutencaoShow(Sender: TObject);
begin
  Set_Campos(False);
  Botoes_Normal;
end;

procedure TFrmTrans_DebitoParceiro.NUM_DOCKeyPress(Sender: TObject; var Key: Char);
begin
 If not( key in['0'..'9',#08] ) then
  key:=#0;
end;

procedure TFrmTrans_DebitoParceiro.PATRIMONIO_IDExit(Sender: TObject);
begin
  DetailSearch('Patrimônio');
end;

procedure TFrmTrans_DebitoParceiro.QTabelaAfterOpen(DataSet: TDataSet);
begin
TFloatField(QTabela.FieldByName('Valor')).DisplayFormat  := '#,##0.00';
end;

procedure TFrmTrans_DebitoParceiro.Set_Campos(Vazio: Boolean);
var
I: Integer;
Temp: TComponent;
begin
...
procedure TFrmTrans_DebitoParceiro.btnBancoClick(Sender: TObject);
begin
  try
...
procedure TFrmTrans_DebitoParceiro.btnCentro_CustoClick(Sender: TObject);
begin
  try
...
procedure TFrmTrans_DebitoParceiro.btnCFOPClick(Sender: TObject);
begin
  CFOP.Text := GetConsulta_CFOP(CFOP.Text);
end;

procedure TFrmTrans_DebitoParceiro.btnContaClick(Sender: TObject);
begin
...
procedure TFrmTrans_DebitoParceiro.btnCst_CofinsClick(Sender: TObject);
begin
  //CST_COFINS.Text := GetConsulta_CST('CST_COFINS', CST_COFINS.Text);
end;

procedure TFrmTrans_DebitoParceiro.btnCst_IcmsClick(Sender: TObject);
begin
  //CST_ICMS.Text := GetConsulta_CST('CST_ICMS', CST_ICMS.Text);
end;

procedure TFrmTrans_DebitoParceiro.btnCst_PisClick(Sender: TObject);
begin
  //CST_PIS.Text := GetConsulta_CST('CST_PIS', CST_PIS.Text);
end;

procedure TFrmTrans_DebitoParceiro.btnDeleteClick(Sender: TObject);
begin
...
procedure TFrmTrans_DebitoParceiro.btnDiscardClick(Sender: TObject);
begin
  Botoes_Normal;
  Set_Campos(False);
...
procedure TFrmTrans_DebitoParceiro.btnEditClick(Sender: TObject);
begin
...
procedure TFrmTrans_DebitoParceiro.btnEmpresaClick(Sender: TObject);
begin
  try
...
procedure TFrmTrans_DebitoParceiro.btnFluxoCaixaClick(Sender: TObject);
begin
  try
...
procedure TFrmTrans_DebitoParceiro.btnFornecedorClick(Sender: TObject);
begin
  try
    PARCEIRO_ID.Value := GetConsulta('PARCEIRO', 0, 0, StrToInt(PARCEIRO_ID.Text));
  except
    PARCEIRO_ID.Value := GetConsulta('PARCEIRO', 0, 0, 0);
  end;
end;

procedure TFrmTrans_DebitoParceiro.btnInsertClick(Sender: TObject);
begin
  ...
  Num_Doc.Text      := copy(DatetimeToStr(now),9,2) + copy(DatetimeToStr(now),12,2) + copy(DatetimeToStr(now),15,2) + copy(DatetimeToStr(now),18,2) ;
...
procedure TFrmTrans_DebitoParceiro.btnNextClick(Sender: TObject);
begin
  if not QTabela.Eof then
  begin
    QTabela.Next;
...
procedure TFrmTrans_DebitoParceiro.btnParcelasClick(Sender: TObject);
begin
...
procedure TFrmTrans_DebitoParceiro.btnPatrimonioClick(Sender: TObject);
begin
  {
  try
...
procedure TFrmTrans_DebitoParceiro.btnPesquisaClick(Sender: TObject);
var
Condicao: String;
begin
...
procedure TFrmTrans_DebitoParceiro.btnPriorClick(Sender: TObject);
begin
  if not QTabela.Bof then
  begin
    QTabela.Prior;
...
procedure TFrmTrans_DebitoParceiro.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmTrans_DebitoParceiro.btnSaveClick(Sender: TObject);
begin
  DetailSearch('');
  ...
  begin
    ...
    if HISTORICO.Text = '' then
      HISTORICO.Text := 'Débito de Parceiro nº ' + NUM_DOC.Text;
    {
...
procedure TFrmTrans_DebitoParceiro.btnVendedorClick(Sender: TObject);
begin
  try
...
procedure TFrmTrans_DebitoParceiro.Btn_ChequeClick(Sender: TObject);
begin
 try
...
procedure TFrmTrans_DebitoParceiro.DBGrid1DblClick(Sender: TObject);
begin
  Manutencao.Show;
end;

procedure TFrmTrans_DebitoParceiro.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    Manutencao.Show;
end;

procedure TFrmTrans_DebitoParceiro.DBGrid1TitleClick(Column: TColumn);
begin
...
procedure TFrmTrans_DebitoParceiro.FLUXO_CAIXA_IDExit(Sender: TObject);
begin
  DetailSearch('Fluxo Caixa');
end;

procedure TFrmTrans_DebitoParceiro.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmTrans_DebitoParceiro.FormCloseQuery(Sender: TObject;
...
procedure TFrmTrans_DebitoParceiro.FormCreate(Sender: TObject);
var
X, Ult_Dia: Integer;
begin
...
procedure TFrmTrans_DebitoParceiro.FormShow(Sender: TObject);
begin
  DBGrid1.SetFocus;
end;
