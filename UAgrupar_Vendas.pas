...
type
  TFrmAgrupar_Vendas = class(TForm)
    SrcList: TListBox;
    DstList: TListBox;
    SrcLabel: TLabel;
    DstLabel: TLabel;
...
var
  FrmAgrupar_Vendas: TFrmAgrupar_Vendas;
...
procedure TFrmAgrupar_Vendas.Search_Cliente;
begin
  QCliente.Sql.Clear;
...
procedure TFrmAgrupar_Vendas.Soma_Agrupamento;
var
I: Integer;
begin
  Vr_Quitar.Value    := 0;
...
procedure TFrmAgrupar_Vendas.IncludeBtnClick(Sender: TObject);
var
Index: Integer;
begin
  QItens.Close;
...
procedure TFrmAgrupar_Vendas.Label6Click(Sender: TObject);
begin
  Cupons.Visible := True;
  Cupons.SetFocus;
end;

procedure TFrmAgrupar_Vendas.ExcludeBtnClick(Sender: TObject);
var
Index: Integer;
begin
  QItens.Close;
...
procedure TFrmAgrupar_Vendas.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmAgrupar_Vendas.FormCreate(Sender: TObject);
begin
  ...
  //Dtmen.Date := StrToDate('01/01/2022');
...
procedure TFrmAgrupar_Vendas.IncAllBtnClick(Sender: TObject);
var
I: Integer;
begin
  QItens.Close;
...
procedure TFrmAgrupar_Vendas.btnClienteClick(Sender: TObject);
begin
  try
...
procedure TFrmAgrupar_Vendas.btnExecutaClick(Sender: TObject);
var
Id_Trans, I, Itens, Trans_Agrp: Integer;
Unidades, Desconto, Valor_Produtos, Base_ICMS_Normal, Vr_ICMS_Normal, Base_ICMS_ST, Vr_ICMS_ST, Vr_Isento, Vr_Unit, Vr_Desc, Vr_Item, Base_ICMS_Normal_T, Vr_ICMS_Normal_T, Base_ICMS_ST_T, Vr_ICMS_ST_T, Aliquota_Inter_Estadual, Base_Pis_T, Base_Cofins_T, Vr_Pis_T, Vr_Cofins_T: Real;
Cst, CFOP, Condicao,List_cupom, InputSerial, CFOP_est: String;
Gerencial: Boolean;
begin
  try
    btnExecuta.Enabled := False;
    ...
    InputSerial  := InputBox(PChar(Msg_Title), 'Informe a Serie da NFe' , '001');
    ...
    QRel.Sql.Add('INSERT INTO TRANSACOES( ' +
                 ...
                 'CLIENTE_ID,    MODELO,      SUB_CODIGO,HORA,DT_SPED,FINALIZADORA_ID) VALUES(' +
                 ':TRANSACAO_ID, :DT_TRANS,   :DT_MOVIMENTO,   :CONDUTA, ' +
                 ':DEPTO,        :EMPRESA_ID, :CONTA_ID,       :C_CUSTO_ID, ' +
                 ':VALOR,        :TPCTB,      :AUTORIZ_ID,     :HISTORICO, ' +
                 ':CONTAAUX_ID,  :NUM_DOC,    :BANCO_ID,       :BALANCO, ' +
                 ':COND_PAGTO,   :SERIE,      :FLUXO_CAIXA_ID, :TIPO_VENDA, ' +
                 ':CLIENTE_ID,   :MODELO,     :SUB_CODIGO,:HORA,:DT_SPED, 1)');
    ...
    QRel.ParamByName('CONTA_ID').AsInteger       := 0; //FrmPrincipal.Config.FieldByName('CONTA_ESTOQUE').AsInteger;
    QRel.ParamByName('C_CUSTO_ID').AsInteger     := 0; //1;
    QRel.ParamByName('VALOR').AsFloat            := 0;
    QRel.ParamByName('TPCTB').AsString           := '2';
    QRel.ParamByName('AUTORIZ_ID').AsInteger     := 0;
    QRel.ParamByName('HISTORICO').AsString       := 'AGRUPAMENTO ORÇAMENTOS PERÍODO ' + Dtmen.Text + ' A ' + Dtmai.Text + ' ' + StrZero(IQuery.FieldByName('NO_DOC_FISCAL').AsString,9,0);
    ...
    QRel.ParamByName('FLUXO_CAIXA_ID').AsInteger := 0; //2;

    QRel.ParamByName('TIPO_VENDA').AsString      := 'REMESSA'; //'VENDA';
    ...
    QRel.Sql.Clear;
    QRel.Sql.Add('SELECT PRODUTO_ID, CFOP, SUM(VR_TOTAL) VALOR, SUM(QUANTIDADE) QUANTIDADE, SUM(DESC_RODAPE) DESCONTO');
    QRel.Sql.Add('FROM TRANSITENS');
    QRel.Sql.Add('WHERE');
    QRel.Sql.Add(Condicao);
    QRel.Sql.Add('GROUP BY PRODUTO_ID, CFOP');
    ...
    begin
      ...
      CFOP := CFOP_est + Copy(QTributo.FieldByName('CFOP').AsString,2,3);
      if CFOP = '6405' then
        CFOP := '6403';
      ...
      begin
        Base_ICMS_Normal := Vr_Item - Vr_Desc;
        Vr_ICMS_Normal   := Base_ICMS_Normal * QTributo.FieldByName('ALIQUOTA_ICMS').AsFloat / 100;
      end
      else
      begin
        Base_ICMS_Normal := 0;
      ...
      QUpdate.Sql.Add('INSERT INTO TRANSITENS( ' +
                      ...
                      'VR_TOTAL,       CST_PIS,         CST_COFINS,     BASE_CALC_PIS, ' +
                      'BASE_CALC_COFINS, ALIQUOTA_PIS,  ALIQUOTA_COFINS, VR_PIS, ' +
                      'VR_COFINS,      DESC_RODAPE,     SEQUENCIA) VALUES(' +
                      ':TRANSACAO_ID,  :PRODUTO_ID,     :TP_PROD_SERV,  :CFOP, ' +
                      ':DESCRICAO,     :CONTA_ID,       :TRIBUTO_ID,    :CST, ' +
                      ':MVA,           :BASE_CALC_ICMS, :ALIQUOTA_ICMS, :VALOR_ICMS, ' +
                      ':BASE_CALC_ST,  :VALOR_ICMS_ST,  :VALOR_ISENTO,  :VALOR_OUTROS, ' +
                      ...
                      ':VR_TOTAL,      :CST_PIS,        :CST_COFINS,    :BASE_CALC_PIS, ' +
                      ':BASE_CALC_COFINS,:ALIQUOTA_PIS, :ALIQUOTA_COFINS,:VR_PIS, ' +
                      ':VR_COFINS,     :DESC_RODAPE,    :SEQUENCIA)');
      ...
      if FrmPrincipal.QEmpresa.FieldByName('CRT').AsString <> '1 - SIMPLES NACIONAL' then
        QUpdate.ParamByName('CST').AsString          := Cst//Copy(QTributo.FieldByName('ORIGEM').AsString, 1, 1) + Copy(QTributo.FieldByName('TRIBUTACAO').AsString, 1, 2)
      Else
      ...
      QUpdate.ParamByName('CST_PIS').AsString        := QProduto.FieldByName('CST_PIS').AsString;
      QUpdate.ParamByName('CST_COFINS').AsString     := QProduto.FieldByName('CST_COFINS').AsString;
      QUpdate.ParamByName('BASE_CALC_PIS').AsFloat   := Vr_Item - Vr_Desc;
      QUpdate.ParamByName('BASE_CALC_COFINS').AsFloat:= Vr_Item - Vr_Desc;
      QUpdate.ParamByName('ALIQUOTA_PIS').AsFloat    := QProduto.FieldByName('PIS').AsFloat;
      QUpdate.ParamByName('ALIQUOTA_COFINS').AsFloat := QProduto.FieldByName('COFINS').AsFloat;
      QUpdate.ParamByName('VR_PIS').AsFloat          := RoundTo((Vr_Item - Vr_Desc) * QProduto.FieldByName('PIS').AsFloat / 100,-2);
      QUpdate.ParamByName('VR_COFINS').AsFloat       := RoundTo((Vr_Item - Vr_Desc) * QProduto.FieldByName('COFINS').AsFloat / 100,-2);
      ...
      Base_Pis_T         := Base_Pis_T + Vr_Item - Vr_Desc;
      Base_Cofins_T      := Base_Cofins_T + Vr_Item - Vr_Desc;
      Vr_Pis_T           := Vr_Pis_T + ((Vr_Item - Vr_Desc) * QProduto.FieldByName('PIS').AsFloat / 100);
      Vr_Cofins_T        := Vr_Cofins_T + ((Vr_Item - Vr_Desc) * QProduto.FieldByName('COFINS').AsFloat / 100);
    ...
    QUpdate.Sql.Add('UPDATE TRANSACOES SET ' +
                                           ...
                                           'BASE_CALC_PIS    = :BASE_CALC_PIS,    BASE_CALC_COFINS = :BASE_CALC_COFINS, ' +
                                           'VR_PIS           = :VR_PIS,           VR_COFINS      = :VR_COFINS, ' +
    ...
    QUpdate.ParamByName('BASE_CALC_PIS').AsFloat    := RoundTo(Base_Pis_T, -2);
    QUpdate.ParamByName('BASE_CALC_COFINS').AsFloat := RoundTo(Base_Cofins_T, -2);
    QUpdate.ParamByName('VR_PIS').AsFloat           := RoundTo(Vr_Pis_T, -2);
    QUpdate.ParamByName('VR_COFINS').AsFloat        := RoundTo(Vr_Cofins_T, -2);
    ...
    begin
      ...
      //QUpdate.ParamByName('CFOP').AsString          := '5929';
...
procedure TFrmAgrupar_Vendas.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmAgrupar_Vendas.btnSelecaoClick(Sender: TObject);
var
X, Lin: Integer;
Txt: String;
begin
  ...
  QRel.Sql.Clear;
  QRel.Sql.Add('SELECT TRANSACOES.TRANSACAO_ID, TRANSACOES.DT_TRANS, TRANSACOES.NUM_DOC, TRANSACOES.COND_PAGTO, TRANSACOES.BANCO_ID, ' +
  ...
  begin
    ...
    begin
      ...
      Txt := Txt + 'LPAD(' + #39 + Cupons.Lines.Strings[X] + #39 + ',7,''0'')';
      Inc(Lin);
    end;
  end;

  if Txt <> 'AND TRANSACOES.NUM_DOC IN (' then
  begin
    Txt := Txt + ')';
    QRel.Sql.Add(Txt);
  end;
  ...
  QRel.ParamByName('EMPRESA_ID').AsInteger := FrmPrincipal.QEmpresa.FieldByName('EMPRESA_ID').AsInteger;
  QRel.ParamByName('TPCTB').AsString       := '3'; //FrmData.QAcesso.FieldByName('TPCTB').AsString;
  QRel.ParamByName('CONDUTA').AsString     := '01';
  QRel.ParamByName('DEPTO').AsString       := '07';
...
procedure TFrmAgrupar_Vendas.ClienteChange(Sender: TObject);
begin
  if Cliente.Value > 0 then
    Search_Cliente;
end;

procedure TFrmAgrupar_Vendas.ClienteEnter(Sender: TObject);
begin
  btnSelecao.Enabled := True;
...
procedure TFrmAgrupar_Vendas.ClienteKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmAgrupar_Vendas.CuponsExit(Sender: TObject);
begin
  Cupons.Visible := False;
end;

procedure TFrmAgrupar_Vendas.DstListClick(Sender: TObject);
var
I: Integer;
begin
  for I := 0 to (DstList.Items.Count - 1) do
...
procedure TFrmAgrupar_Vendas.DstListDrawItem(Control: TWinControl;
...
procedure TFrmAgrupar_Vendas.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmAgrupar_Vendas.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmAgrupar_Vendas.ExcAllBtnClick(Sender: TObject);
var
I: Integer;
begin
  QItens.Close;
...
procedure TFrmAgrupar_Vendas.MoveSelected(List: TCustomListBox; Items: TStrings);
var
I: Integer;
begin
...
procedure TFrmAgrupar_Vendas.SetButtons;
var
...
function TFrmAgrupar_Vendas.GetFirstSelection(List: TCustomListBox): Integer;
begin
...
procedure TFrmAgrupar_Vendas.SetItem(List: TListBox; Index: Integer);
var
MaxIndex: Integer;
begin
  with List do
...
procedure TFrmAgrupar_Vendas.SrcListClick(Sender: TObject);
var
I: Integer;
begin
  for I := 0 to (SrcList.Items.Count - 1) do
...
procedure TFrmAgrupar_Vendas.SrcListDrawItem(Control: TWinControl;
  Index: Integer; Rect: TRect; State: TOwnerDrawState);
var
X: Integer;
Txt: String;
