...
type
  TFrmVendas_Vendedor2 = class(TForm)
    Dtmen: TDateEdit;
    Label1: TLabel;
    Dtmai: TDateEdit;
    Label2: TLabel;
...
var
  FrmVendas_Vendedor2: TFrmVendas_Vendedor2;
...
procedure TFrmVendas_Vendedor2.btnExecutaClick(Sender: TObject);
begin
// 125, 327
 QRel.Sql.Clear;
  ...
  QRel.Sql.Add('ORDER BY FUNCIONARIOS.NOME, FINALIZADORAS.LEGENDA, TRANSACOES.DT_TRANS'); }
    ...
    QRel.Sql.Add('PEDIDOS.PEDIDO_ID, PEDIDOS.DT_PEDIDO,PEDIDOS.STATUS, FUNCIONARIOS.NOME VENDEDOR,TRANSACOES.NUM_DOC DOCUMENTO,PEDIDOS.NOME CLIENTES,PEDIDOS.TRANSACAO_ID,TRANSACOES.DT_ENT_SAI');
    ...
    QRel.Sql.Add('GROUP BY PEDIDOS.PEDIDO_ID,TRANSACOES.DT_ENT_SAI ,PEDIDOS.STATUS,FUNCIONARIOS.NOME,TRANSACOES.NUM_DOC,PEDIDOS.NOME,PEDIDOS.TRANSACAO_ID,PEDIDOS.DT_PEDIDO');
    QRel.Sql.Add('ORDER BY FUNCIONARIOS.NOME, TRANSACOES.DT_ENT_SAI,PEDIDOS.TRANSACAO_ID');
    }

    QRel.Sql.Add('SELECT (TRANSACOES.VALOR_PRODUTOS + SUM(TRANSITENS.VR_DESCONTO * TRANSITENS.QUANTIDADE)) VALOR_PRODUTOS, ');
    QRel.Sql.Add('(TRANSACOES.VR_ACRESCIMO + TRANSACOES.VR_FRETE)  ACRESCIMO,');
    QRel.Sql.Add('TRANSACOES.VR_DESCONTO DESCONTO , SUM(TRANSITENS.VR_DESCONTO * TRANSITENS.QUANTIDADE) DESCONTO_ITEM, TRANSACOES.VALOR ,');
    QRel.Sql.Add('TRANSACOES.TRANSACAO_ID, TRANSACOES.DT_ENT_SAI, FUNCIONARIOS.NOME VENDEDOR,TRANSACOES.NUM_DOC DOCUMENTO,CLIENTES.NOME CLIENTES');
    QRel.Sql.Add('FROM TRANSITENS');
    QRel.Sql.Add('INNER JOIN TRANSACOES ON TRANSACOES.TRANSACAO_ID = TRANSITENS.TRANSACAO_ID');
    QRel.Sql.Add('INNER JOIN FUNCIONARIOS ON (FUNCIONARIOS.FUNCIONARIO_ID = TRANSACOES.VENDEDOR_ID) AND (TRANSACOES.VENDEDOR_ID > 1)');
    ...
    QRel.Sql.Add('AND TRANSITENS.TP_PROD_SERV = ''P'' ');
    ...
    QRel.Sql.Add('GROUP BY TRANSACOES.TRANSACAO_ID,TRANSACOES.DT_ENT_SAI ,FUNCIONARIOS.NOME,TRANSACOES.NUM_DOC,CLIENTES.NOME');
    QRel.Sql.Add('ORDER BY FUNCIONARIOS.NOME, TRANSACOES.DT_ENT_SAI,TRANSACOES.TRANSACAO_ID');
...
procedure TFrmVendas_Vendedor2.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmVendas_Vendedor2.btnVendedorClick(Sender: TObject);
begin
  try
...
procedure TFrmVendas_Vendedor2.DetailBand1BeforePrint(
...
procedure TFrmVendas_Vendedor2.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmVendas_Vendedor2.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmVendas_Vendedor2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmVendas_Vendedor2.FormCreate(Sender: TObject);
begin
  Dtmen.Date := StrToDate('01/' + Copy(DateToStr(date), 4, 7));
  Dtmai.Date := Ult_Dia_Mes(date);
end;

procedure TFrmVendas_Vendedor2.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TFrmVendas_Vendedor2.QRGroup1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
 Qdt_Pedidos := 0;
 Total_pedido := 0;
...
procedure TFrmVendas_Vendedor2.VendedorKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
