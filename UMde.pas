...
type
  TFrmMde = class(TForm)
    ...
    Label1: TLabel;
    TabSheet3: TTabSheet;
    RESPONSAVEL_SEGURO: TComboBox;
    Label18: TLabel;
    GroupBox1: TGroupBox;
    tp_cnpj: TRadioGroup;
    cnpj_responsavel: TMaskEdit;
    GroupBox2: TGroupBox;
    Nome_seguro: TEdit;
    Label19: TLabel;
    cnpj_seguro: TMaskEdit;
    Label20: TLabel;
    Apolice_Seguro: TEdit;
    Label21: TLabel;
    TabSheet4: TTabSheet;
    Grid_averbacao: TDBGrid;
    QAverbacao: TFDQuery;
    DataAverbacao: TDataSource;
    StatusBar4: TStatusBar;
    GroupBox3: TGroupBox;
    Label6: TLabel;
    Label16: TLabel;
    Label14: TLabel;
    Tara_Reboque: TRxCalcEdit;
    UF_Veiculo_Reboque: TComboBox;
    Placa_Veiculo_Reboque: TEdit;
    GroupBox4: TGroupBox;
    Label31: TLabel;
    Label32: TLabel;
    Label37: TLabel;
    Tara_reboque_02: TRxCalcEdit;
    uf_reboque_02: TComboBox;
    veiculo_reboque_02: TEdit;
    GroupBox5: TGroupBox;
    Label38: TLabel;
    Label41: TLabel;
    Label42: TLabel;
    Tara_Reboque_03: TRxCalcEdit;
    uf_reboque_03: TComboBox;
    veiculo_reboque_03: TEdit;
    Btnveiculo_reboque: TSpeedButton;
    btnveiculo_reboque02: TSpeedButton;
    btnveiculo_reboque03: TSpeedButton;
    ListBox3: TListBox;
    ListBox4: TListBox;
    ListBox5: TListBox;
    ...
    Label43: TLabel;
    CEP_CARREGA: TMaskEdit;
    TabSheet6: TTabSheet;
    Label28: TLabel;
    UF_DESCARREGA: TComboBox;
    Label44: TLabel;
    MUNICIPIO_DESCARREGA: TComboBox;
    Label45: TLabel;
    CEP_DESCARREGA: TMaskEdit;
    TabSheet7: TTabSheet;
    Label47: TLabel;
    prodpred_id: TCurrencyEdit;
    ...
    procedure tp_cnpjClick(Sender: TObject);
    procedure QAverbacaoBeforeEdit(DataSet: TDataSet);
    procedure QAverbacaoBeforePost(DataSet: TDataSet);
    procedure QAverbacaoNewRecord(DataSet: TDataSet);
    procedure QAverbacaoPostError(DataSet: TDataSet; E: EDatabaseError;
      var Action: TDataAction);
    procedure Grid_averbacaoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure Grid_averbacaoEnter(Sender: TObject);
    procedure Grid_averbacaoExit(Sender: TObject);
    procedure Grid_averbacaoKeyPress(Sender: TObject; var Key: Char);
    procedure btnVeiculoClick(Sender: TObject);
    procedure PLACA_VEICULOExit(Sender: TObject);
    procedure Btnveiculo_reboqueClick(Sender: TObject);
    procedure btnveiculo_reboque02Click(Sender: TObject);
    procedure btnveiculo_reboque03Click(Sender: TObject);
    procedure Placa_Veiculo_ReboqueExit(Sender: TObject);
    procedure veiculo_reboque_02Exit(Sender: TObject);
    procedure veiculo_reboque_03Exit(Sender: TObject);
    procedure btnProdutoClick(Sender: TObject);
    procedure prodpred_idChange(Sender: TObject);
    procedure UF_DESCARREGAExit(Sender: TObject);
    procedure chk_TerceirosClick(Sender: TObject);
  private
...
uses
  UPrincipal, UData, pcnConversao, UConsulta,UPesquisa,
  UConsulta_CFOP,UPasswordDlg,UAcesso,UConsulta_CST,UConsulta_Veiculos;

{$R *.dfm}
procedure TFrmMde.SearchProduto;
begin
  QProduto.Close;
  QProduto.ParamByName('PRODUTO_ID').AsInteger := StrToInt(prodpred_id.Text);
...
begin
  ...
  begin
    ...
    if Temp is TSpeedButton then
      TSpeedButton(Temp).Enabled := Status;
...
begin
  ...
  begin
    ...
    begin
      ...
      QAverbacao.Close;
      QAverbacao.ParamByName('MDEFISCAL_ID').AsInteger := QTabela.FieldByName('MDEFISCAL_ID').AsInteger;
      QAverbacao.Prepare;
      QAverbacao.Open;
  ...
  Begin
   ...
   Grid_averbacao.Enabled       := True;
  End;

  IF QTabela.FieldByName('CNPJ_CPF_TERCEIROS').AsString <> '' Then
  chk_Terceiros.Checked := True;

end;

procedure TFrmMde.btnVeiculoClick(Sender: TObject);
begin
 try
    PLACA_VEICULO.Text := GetConsulta_Veiculos(PLACA_VEICULO.Text);
  except

  end;
end;

procedure TFrmMde.btnveiculo_reboque02Click(Sender: TObject);
begin
try
    VEICULO_REBOQUE_02.Text := GetConsulta_Veiculos(VEICULO_REBOQUE_02.Text);
  except

  end;
end;

procedure TFrmMde.btnveiculo_reboque03Click(Sender: TObject);
begin
try
    VEICULO_REBOQUE_03.Text := GetConsulta_Veiculos(VEICULO_REBOQUE_03.Text);
  except
...
procedure TFrmMde.UF_DESCARREGAExit(Sender: TObject);
begin
QSearch.SQL.Clear;
QSearch.sql.Add('SELECT * FROM IBGE WHERE UF_SIGLA =:UF ORDER BY DESCRICAO');
QSearch.ParamByName('UF').AsString := UF_DESCARREGA.Text;
QSearch.Prepare;
QSearch.Open;
QSearch.First;

municipio_DEScarrega.Items.Clear;

  while not QSearch.Eof do
  begin
   municipio_descarrega.Items.Add(QSearch.FieldByName('DESCRICAO').AsString);
   QSearch.Next;
  end;
...
Set_Campos(False);
  Botoes_Normal;
   ...
   ListBox1.Clear;
     ...
     IQuery.Sql.Clear;
     IQuery.Sql.Add('SELECT CHAVE_NFE FROM COMPL_CTEFISCAL WHERE CHAVE_NFE <> :CHAVE_NFE ORDER BY DT_SAIDA ');
     IQuery.ParamByName('CHAVE_NFE').AsString := '';
     IQuery.Prepare;
     ...
     while not IQuery.Eof do
     Begin
      ListBox3.Items.Add(IQuery.FieldByName('CHAVE_NFE').AsString);
      IQuery.Next;
     End;

     Grid_Notas.Columns[2].PickList:= ListBox3.Items;

     ListBox4.Clear;

     IQuery.Sql.Clear;
     IQuery.Sql.Add('SELECT CHAVE_NFE FROM COMPL_NFISCAL WHERE CHAVE_NFE <> :CHAVE_NFE ORDER BY DT_SAIDA ');
     IQuery.ParamByName('CHAVE_NFE').AsString := '';
     IQuery.Prepare;
     ...
     while not IQuery.Eof do
     Begin
      ListBox4.Items.Add(IQuery.FieldByName('CHAVE_NFE').AsString);
      IQuery.Next;
     End;

     Grid_Notas.Columns[3].PickList:= ListBox4.Items;

     ListBox5.Clear;

     IQuery.Sql.Clear;
     IQuery.Sql.Add('SELECT DISTINCT NOME_MOTORISTA FROM COMPL_MDEFISCAL_MOTORISTA  ORDER BY NOME_MOTORISTA ');
     IQuery.Prepare;
     ...
     while not IQuery.Eof do
     Begin
      ListBox5.Items.Add(IQuery.FieldByName('NOME_MOTORISTA').AsString);
      IQuery.Next;
     End;

     Grid_Notas_Motorista.Columns[0].PickList:= ListBox5.Items;
...
procedure TFrmMde.veiculo_reboque_02Exit(Sender: TObject);
begin
QSearch.SQL.Clear;
QSearch.SQL.Add('SELECT * FROM VEICULOS WHERE PLACA = :PLACA');
QSearch.ParamByName('PLACA').AsString := Veiculo_Reboque_02.Text;
QSearch.Prepare;
QSearch.Open();

  if not QSearch.IsEmpty then
  Begin
  Tara_Reboque_02.Value      := QSearch.FieldByName('TARA').AsFloat;
  UF_REBOQUE_02.Text := QSearch.FieldByName('UF').AsString;
  End;
end;

procedure TFrmMde.veiculo_reboque_03Exit(Sender: TObject);
begin
QSearch.SQL.Clear;
QSearch.SQL.Add('SELECT * FROM VEICULOS WHERE PLACA = :PLACA');
QSearch.ParamByName('PLACA').AsString := Veiculo_Reboque_03.Text;
QSearch.Prepare;
QSearch.Open();

  if not QSearch.IsEmpty then
  Begin
  Tara_Reboque_03.Value      := QSearch.FieldByName('TARA').AsFloat;
  UF_REBOQUE_03.Text := QSearch.FieldByName('UF').AsString;
  End;
...
if QNotas_motorista.State in [dsInsert, dsEdit] then
  begin
   try
    if Grid_notas_motorista.SelectedField.FieldName = 'nome_motorista' then
    begin
     QSearch.SQL.Clear;
     QSearch.SQL.Add('SELECT CPF_MOTORISTA FROM COMPL_MDEFISCAL_MOTORISTA WHERE NOME_MOTORISTA = :NOME');
     QSearch.ParamByName('NOME').AsString :=  QNotas_Motorista.FieldByName('NOME_MOTORISTA').AsString;
     QSearch.Prepare;
     QSearch.Open();

     QNotas_Motorista.FieldByName('CPF_MOTORISTA').AsString  := QSearch.FieldByName('CPF_MOTORISTA').AsString;
...
procedure TFrmMde.PLACA_VEICULOExit(Sender: TObject);
begin
QSearch.SQL.Clear;
QSearch.SQL.Add('SELECT * FROM VEICULOS WHERE PLACA = :PLACA');
QSearch.ParamByName('PLACA').AsString := PLACA_VEICULO.Text;
QSearch.Prepare;
QSearch.Open();

  if not QSearch.IsEmpty then
  Begin
  TARA.Value      := QSearch.FieldByName('TARA').AsFloat;
  RNTRC.Text      := QSearch.FieldByName('RNTRC').AsString;
  UF_VEICULO.Text := QSearch.FieldByName('UF').AsString;
  End;

end;


procedure TFrmMde.Placa_Veiculo_ReboqueExit(Sender: TObject);
begin
QSearch.SQL.Clear;
QSearch.SQL.Add('SELECT * FROM VEICULOS WHERE PLACA = :PLACA');
QSearch.ParamByName('PLACA').AsString := Placa_Veiculo_Reboque.Text;
QSearch.Prepare;
QSearch.Open();

  if not QSearch.IsEmpty then
  Begin
  Tara_Reboque.Value      := QSearch.FieldByName('TARA').AsFloat;
  UF_VEICULO_REBOQUE.Text := QSearch.FieldByName('UF').AsString;
  End;
end;

procedure TFrmMde.prodpred_idChange(Sender: TObject);
begin
if prodpred_id.Value > 0 then
    SearchProduto;
...
procedure TFrmMde.QAverbacaoBeforeEdit(DataSet: TDataSet);
begin
IQuery.Sql.Clear;
    IQuery.Sql.Add('SELECT * FROM COMPL_MDEFISCAL_AVERBACAO');
    IQuery.Sql.Add('WHERE');
    IQuery.Sql.Add('(MDEFISCAL_ID = :MDEFISCAL_ID)');
    IQuery.Sql.Add('AND (NUMERO_AVERBACAO = :NUMERO_AVERBACAO)');

    IQuery.ParamByName('MDEFISCAL_ID').AsInteger      := QTabela.FieldByName('MDEFISCAL_ID').AsInteger;
    IQuery.ParamByName('NUMERO_AVERBACAO').AsString   := QAverbacao.FieldByName('NUMERO_AVERBACAO').AsString;
    ...
    begin
      Application.MessageBox('Número de Averbação  já lançado.', PChar(Msg_Title), mb_IconStop);
      Grid_averbacao.SelectedIndex := 0;
      abort;
    end;
...
procedure TFrmMde.QAverbacaoBeforePost(DataSet: TDataSet);
begin
    IQuery.Sql.Clear;
    IQuery.Sql.Add('SELECT * FROM COMPL_MDEFISCAL_AVERBACAO');
    IQuery.Sql.Add('WHERE');
...
procedure TFrmMde.QAverbacaoNewRecord(DataSet: TDataSet);
begin
DataSet['MDEFISCAL_ID']         := QTabela.FieldByName('MDEFISCAL_ID').AsInteger;
end;

procedure TFrmMde.QAverbacaoPostError(DataSet: TDataSet; E: EDatabaseError;
  var Action: TDataAction);
begin
try
...
begin
  ...
  End;
    ...
    IQuery.Sql.Add('WHERE');
    IQuery.Sql.Add('(MDEFISCAL_ID = :MDEFISCAL_ID)');
    IQuery.Sql.Add('AND (IBGE = :IBGE) AND (LACRE = :LACRE)');
    IQuery.Sql.Add('AND (CTE = :CTE) AND (NFE = :NFE)');

    IQuery.ParamByName('MDEFISCAL_ID').AsInteger := QTabela.FieldByName('MDEFISCAL_ID').AsInteger;
    IQuery.ParamByName('IBGE').AsString          := QNotas.FieldByName('IBGE').AsString;
    IQuery.ParamByName('LACRE').AsString         := QNotas.FieldByName('LACRE').AsString;
    IQuery.ParamByName('CTE').AsString           := QNotas.FieldByName('CTE').AsString;
    IQuery.ParamByName('NFE').AsString           := QNotas.FieldByName('NFE').AsString;
...
procedure TFrmMde.tp_cnpjClick(Sender: TObject);
begin
 if tp_cnpj.ItemIndex = 1 then
 cnpj_responsavel.EditMask := '99.999.999/9999-99;1;_'
 Else
 cnpj_responsavel.EditMask := '999.999.999-99;1;_';
end;
...
begin
  ...
  begin
   ...
   // Dados do Emitente
   //
   Emit.CNPJCPF  := CNPJ_TRANSPORTADOR.Text;
   ...
   With prodPred do
   Begin
   xProd := QProduto.FieldByName('DESCRICAO').AsString;
   cEAN  := QProduto.FieldByName('COD_BARRA').AsString;
   NCM   := QProduto.FieldByName('NCM').AsString;
   tpCarga := tcCargaGeral;
   infLocalCarrega.CEP    := StrToInt(SemMascara(CEP_CARREGA.Text));
   infLocalDescarrega.CEP := StrToInt(SemMascara(CEP_DESCARREGA.Text));
   End;
   ...
   bEGIN
     ...
     IF not chk_Terceiros.Checked Then
     Begin
     ...
     end;
   {
    ...
    // bEGIN
      ...
      bEGIN
       ...
       rodo.veicTracao.prop.CNPJCPF := cnpj_cpf_terceiros.Text;
       rodo.veicTracao.prop.RNTRC   := RNTRC.Text;
       rodo.veicTracao.prop.xNome   := iquery.FieldByName('NOME_MOTORISTA').AsString;
       rodo.veicTracao.prop.IE      := 'ISENTO';
       rodo.veicTracao.prop.UF      := UF_VEICULO.Text;
   ...
   //  end;
     ...
     if PLACA_VEICULO_Reboque.Text <>  ''   then
     bEGIN
      with rodo.veicReboque.Add do
      begin
       cInt    := '002';
       placa   := PLACA_VEICULO_Reboque.Text;
       RENAVAM := '';
       capKG   := 0;
       capM3   := 0;
       tara    := StrToInt(TARA_Reboque.text);
       // TpcteTipoCarroceria = (tcNaoAplicavel, tcAberta, tcFechada, tcGraneleira, tcPortaContainer, tcSider);
       tpCar := tcFechada;
       UF := UF_VEICULO_Reboque.Text;

      end;
     end;

     if VEICULO_Reboque_02.Text <>  ''   then
     bEGIN
      with rodo.veicReboque.Add do
      begin
       cInt    := '003';
       placa   := VEICULO_Reboque_02.Text;
       RENAVAM := '';
       capKG   := 0;
       capM3   := 0;
       tara    := StrToInt(TARA_Reboque_02.text);
       // TpcteTipoCarroceria = (tcNaoAplicavel, tcAberta, tcFechada, tcGraneleira, tcPortaContainer, tcSider);
       tpCar := tcFechada;
       UF := UF_Reboque_02.Text;

      end;
     end;

      if VEICULO_Reboque_03.Text <>  ''   then
     bEGIN
      with rodo.veicReboque.Add do
      begin
       cInt    := '004';
       placa   := VEICULO_Reboque_03.Text;
       RENAVAM := '';
       capKG   := 0;
       capM3   := 0;
       tara    := StrToInt(TARA_Reboque_03.text);
       // TpcteTipoCarroceria = (tcNaoAplicavel, tcAberta, tcFechada, tcGraneleira, tcPortaContainer, tcSider);
       tpCar := tcFechada;
       UF := UF_Reboque_03.Text;
     ...
     Begin
       ...
       Begin
        ...
        begin
          ...
          if QDetalhe.FieldByName('LACRE').AsString = 'C' then
          SegCodBarra := Copy(QDetalhe.FieldByName('CTE').AsString,1,36);
   ...
   if RESPONSAVEL_SEGURO.Text <> '' then
   Begin
     with seg.Add do
     begin

      if RESPONSAVEL_SEGURO.Text = 'EMITENTE DO MDF-E' then
      respSeg := rsEmitente
      Else
      respSeg := rsTomadorServico;

      CNPJCPF := SemMascara(cnpj_responsavel.Text);
      CNPJ    := SemMascara(cnpj_seguro.Text);
      xSeg := Nome_seguro.Text;
      nApol :=Apolice_Seguro.Text;

      QSearch.SQL.Clear;
      QSearch.SQL.Add('SELECT * FROM COMPL_MDEFISCAL_AVERBACAO WHERE MDEFISCAL_ID = :MDEFISCAL_ID');
      ...
      Begin
        with aver.Add do
        begin
         nAver := QSearch.FieldByName('NUMERO_AVERBACAO').AsString;
        end;
        QSearch.Next;
      End;
     end;
  ...
  if FrmPrincipal.ACBrMDFe1.Enviar(0, True,True) then
  begin
    IQuery.Sql.Clear;
    IQuery.Sql.Add('UPDATE COMPL_MDEFISCAL SET RECIBO = :RECIBO, PROTOCOLO = :PROTOCOLO, CHAVE_MDF = :CHAVE_MDF,ARQUIVO_XML = :ARQUIVO_XML');
    IQuery.Sql.Add('WHERE');
    IQuery.Sql.Add('(MDEFISCAL_ID = :MDEFISCAL_ID)');
    ...
    IQuery.ParamByName('MDEFISCAL_ID').AsInteger := QTabela.FieldByName('MDEFISCAL_ID').AsInteger;
    IQuery.ParamByName('ARQUIVO_XML').AsByteStr   := FrmPrincipal.ACBrMDFe1.Manifestos.Items[0].XML;
...
procedure TFrmMde.btnProdutoClick(Sender: TObject);
begin
  try
    ProdPred_id.Value := GetConsulta('ESTOQUE', FrmData.QAcesso.FieldByName('EMPRESA_ID').AsInteger, 0 , StrToInt(ProdPred_id.Text));
  except
    ProdPred_id.Value := GetConsulta('ESTOQUE', 0, 0, 0);
  end;
...
procedure TFrmMde.Btnveiculo_reboqueClick(Sender: TObject);
begin
try
    PLACA_VEICULO_REBOQUE.Text := GetConsulta_Veiculos(PLACA_VEICULO_REBOQUE.Text);
  except
...
procedure TFrmMde.chk_TerceirosClick(Sender: TObject);
begin

  if chk_Terceiros.Checked Then
  Cnpj_cpf_terceiros.Enabled := True
  Else
  Cnpj_cpf_terceiros.Enabled := False;
...
begin
  ...
  if (Key = Vk_F7) and (Sender = PLACA_VEICULO) then
    btnVeiculoClick(Sender);

  if (Key = Vk_F7) and (Sender = Placa_Veiculo_Reboque) then
    Btnveiculo_reboqueClick(Sender);

  if (Key = Vk_F7) and (Sender = Veiculo_Reboque_02) then
    Btnveiculo_reboque02Click(Sender);

  if (Key = Vk_F7) and (Sender = Veiculo_Reboque_03) then
    Btnveiculo_reboque03Click(Sender);
...
begin
  ...
  if (not QNotas.FieldByName('LACRE').IsNull) OR (QNotas.FieldByName('LACRE').AsString = '') then
  Begin
  Inc(id_lacre);
  QNotas.FieldByName('LACRE').AsString := IntToStr(id_lacre);
  End;
...
procedure TFrmMde.Grid_averbacaoEnter(Sender: TObject);
begin
Botoes_Disable;
Consulta.TabVisible := False;
end;

procedure TFrmMde.Grid_averbacaoExit(Sender: TObject);
begin
  if FrmData.QAcesso.FieldByName('ALTERACAO').AsString = 'NÃO' then
  begin
    Application.MessageBox('Usuário sem permissão para alteração', PChar(Msg_Title), mb_IconStop);
...
procedure TFrmMde.Grid_averbacaoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin

 if (Key = Vk_F4) and (QAverbacao.State = dsBrowse) and (not QAverbacao.IsEmpty) then
    QAverbacao.Delete;

  if Key = VK_F9 then
    Perform(Wm_NextDlgCtl, 0, 0);

  if ((Key = Vk_Up) or (Key = Vk_Down)) and (QAverbacao.State in [dsInsert, dsEdit]) then
    Key := 0;
end;

procedure TFrmMde.Grid_averbacaoKeyPress(Sender: TObject; var Key: Char);
begin
 if Key = #46 then
    Key := #44;
