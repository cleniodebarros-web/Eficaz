...
type
  TFrmHistorico_Clientes = class(TForm)
    ...
    table_historicovalor_juros: TFloatField;
    table_historicovalor_despesa: TFloatField;
   ...
   if not Caso(Copy(FrmData.QAcesso.FieldByName('OPCOES').AsString, 66, 1))  = True Then
    ...
    begin
      ...
      begin
        ...
        while not QParcelas.Eof do
        begin
          if QParcelas.FieldByName('VALOR_ORI').AsFloat > 0 then
          begin

            QPagamentos.Sql.Clear;
            QPagamentos.Sql.Add('SELECT * FROM TRANSACOES');
            QPagamentos.Sql.Add('WHERE');
            QPagamentos.Sql.Add('(SAIDA_ID = :SAIDA_ID) and (PARCELA_ID = :PARCELA_ID)');

            QPagamentos.ParamByName('SAIDA_ID').AsInteger  := QCompras.FieldByName('TRANSACAO_ID').AsInteger;
            QPagamentos.ParamByName('PARCELA_ID').AsString := QParcelas.FieldByName('PARCELA_ID').AsString;
            ...
            Begin
              ...
              try
                ...
                Table_Historico.FieldByName('VALOR_PARCELA').AsFloat    := QPagamentos.FieldByName('VALOR_PARCELA').AsFloat;
                Table_Historico.FieldByName('NOME').AsString            := QCliente.FieldByName('NOME').AsString;
                Table_Historico.FieldByName('VALOR_JUROS').AsFloat      := QParcelas.FieldByName('VALOR_JUROS').AsFloat;
                Table_Historico.FieldByName('VALOR_DESPESA').AsFloat    := QParcelas.FieldByName('VALOR_DESPESAS').AsFloat;
                ...
                if QPagamentos.FieldByName('DT_TRANS').AsDateTime - QParcelas.FieldByName('DT_VENCIMENTO').AsDateTime > 0 Then
                Table_Historico.FieldByName('ATRASO').AsFloat           :=  QPagamentos.FieldByName('DT_TRANS').AsDateTime - QParcelas.FieldByName('DT_VENCIMENTO').AsDateTime  //MEDIA_ATR;
                Else
          ...
          Begin
            ...
            try
              ...
              if StrToDate(FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsString) - QParcelas.FieldByName('DT_VENCIMENTO').AsDateTime > 0 Then
              Table_Historico.FieldByName('ATRASO').AsFloat           :=  StrToDate(FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsString) - QParcelas.FieldByName('DT_VENCIMENTO').AsDateTime //MEDIA_ATR;
              Else
              Table_Historico.FieldByName('ATRASO').AsFloat           := 0;
...
Begin
  ...
  Table_Historico.Open;
  if not Table_historico.IsEmpty   Then
