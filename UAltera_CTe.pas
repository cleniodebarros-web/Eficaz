...
var
  FrmAltera_CTe: TFrmAltera_CTe;
  ID: Integer;
  GeraXML: Boolean;
  Cod_CFOP: String;

  procedure Altera_CTe(Transacao_Id: Integer; CFOP: String);

implementation

uses
  UPrincipal, UData, pcnConversao, pcteConversaocte, UConsulta, UConsulta_CFOP,UPasswordDlg,UAcesso;

{$R *.dfm}

procedure TFrmAltera_CTe.Set_Campos(Vazio: Boolean);
var
I: Integer;
Temp: TComponent;
begin
...
procedure TFrmAltera_CTe.Insert;
var
I: Integer;
Temp: TComponent;
Sql, Par: String;
...
function TFrmAltera_CTe.Validacao: Boolean;
begin
  Result := False;
  ...
  if (TOMADOR_SERVICO.Text <> 'REMETENTE') and (TOMADOR_SERVICO.Text <> 'DESTINATARIO') and (TOMADOR_SERVICO.Text <> 'RECEBEDOR') and (TOMADOR_SERVICO.Text <> 'EXPEDIDOR') and (TOMADOR_SERVICO.Text <> 'OURTOS') then
  begin
...
procedure TFrmAltera_CTe.DetailSearch(Tabela: String);
begin
...
function TFrmAltera_CTe.Search_Campo(Txt: String): Integer;
begin
...
begin
  ...
  Application.CreateForm(TFrmAltera_CTe, FrmAltera_CTe);
  try
    FrmAltera_CTe.ShowModal;
  finally
    FrmAltera_CTe.Release;
  end;
end;

procedure TFrmAltera_CTe.ProdutoSearch(ID: Integer);
begin
  QProdutos.Sql.Clear;
...
procedure TFrmAltera_CTe.QNotasAfterOpen(DataSet: TDataSet);
begin
...
procedure TFrmAltera_CTe.QNotasBeforeEdit(DataSet: TDataSet);
begin
...
procedure TFrmAltera_CTe.QNotasBeforePost(DataSet: TDataSet);
begin
...
procedure TFrmAltera_CTe.QNotasNewRecord(DataSet: TDataSet);
begin
...
procedure TFrmAltera_CTe.btnFornecedorClick(Sender: TObject);
var
Forne: Integer;
begin
...
procedure TFrmAltera_CTe.btnGeraClick(Sender: TObject);
var
...
//
       { if (FORMA_EMISSAO.Text <> 'NORMAL')  then
        ...
        // Origem da Prestação
         Ide.cMunIni  := FrmPrincipal.QEmpresa.FieldByName('IBGE').AsInteger;
         Ide.xMunIni  := FrmPrincipal.QEmpresa.FieldByName('MUNICIPIO').AsString;
         Ide.UFIni    := FrmPrincipal.QEmpresa.FieldByName('ESTADO').AsString;
        ...
        Ide.xdetretira:='';
         ...
         Else if TOMADOR_SERVICO.Text = 'DESTINATARIO' then
     ...
     //  Local de Entrega
       ...
       Begin
         ...
         begin
          ...
          nPeso := RoundTo(Peso_liquido.Value, -2);
      ...
      //      end;
       ...
       //  Informações da Carga
        ...
        infCTeNorm.infCarga.vCarga   := RoundTo(QTransacao.FieldByName('VALOR').AsFloat , -2);
        infCTeNorm.infCarga.proPred  := Prod_Principal.Text;
        infCTeNorm.infCarga.xOutCat  := '';
      ...
      //Informações do Seguro
        ...
        begin
          ...
          begin
           ...
           // IQuery.ParamByName('HISTORICO').AsString     := StrZero(NO_DOC_FISCAL.Text, 9, 0);
        ...
        begin
          ...
          FrmPrincipal.ACBrCTe1.Conhecimentos.LoadFromFile(FrmPrincipal.ACBrcte1.Configuracoes.Arquivos.GetPathCTe + Copy(FrmPrincipal.ACBrCTe1.Conhecimentos.Items[0].CTe.infCTe.ID, 4, 44)+'-cte.xml');

          FrmPrincipal.ACBrCTe1.Consultar;

          if FrmPrincipal.ACBrCTe1.WebServices.Consulta.Protocolo <> '' then
          Begin
            ...
            IQuery.Sql.Clear;
            IQuery.Sql.Add('UPDATE COMPL_CTEFISCAL SET CHAVE_NFE = :CHAVE_NFE, RECIBO = :RECIBO, PROTOCOLO = :PROTOCOLO');
            IQuery.Sql.Add('WHERE');
            IQuery.Sql.Add('(TRANSACAO_ID = :TRANSACAO_ID)');

            IQuery.ParamByName('CHAVE_NFE').AsString     := Copy(FrmPrincipal.ACBrCTe1.Conhecimentos.Items[0].CTe.infCTe.ID, 4, 44);
            IQuery.ParamByName('RECIBO').AsString        := DateToStr(FrmPrincipal.ACBrCTe1.WebServices.Consulta.DhRecbto);
            IQuery.ParamByName('PROTOCOLO').AsString     := FrmPrincipal.ACBrCTe1.WebServices.Consulta.Protocolo;
          ...
          Else
          Begin
          Application.MessageBox('Cte não consta na base de dados do Sefaz ou não pode ser efetuada!', PChar(Msg_Title), mb_IconInformation);
          Close;
          End;
        ...
        Begin
          ...
          IQuery.Sql.Clear;
          IQuery.Sql.Add('INSERT INTO CTES_CANCELADAS(TRANSACAO_ID, EMPRESA_ID, NO_DOC_FISCAL, DT_SAIDA, HR_SAIDA, CFOP_COMPL, RESP_FRETE, NOME_TRANSPORTADOR, CNPJ_TRANSPORTADOR, ' +
...
procedure TFrmAltera_CTe.Button1Click(Sender: TObject);
var
MyFile: TextFile;
...
procedure TFrmAltera_CTe.CFOP_COMPLExit(Sender: TObject);
begin
  DetailSearch('CFOP');
end;

procedure TFrmAltera_CTe.CNPJ_TRANSPORTADORKeyDown(Sender: TObject;
...
procedure TFrmAltera_CTe.COND_PAGTOExit(Sender: TObject);
begin
 DetailSearch('');
...
procedure TFrmAltera_CTe.DT_SAIDAEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmAltera_CTe.DT_SAIDAKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmAltera_CTe.FormCreate(Sender: TObject);
begin
  GeraXML          := False;
  DT_PREVISAO.Date := Date;
end;

procedure TFrmAltera_CTe.FormShow(Sender: TObject);
var
IniFile: String;
Ini: TIniFile;
StreamMemo: TMemoryStream;
...
procedure TFrmAltera_CTe.GerarXMLClick(Sender: TObject);
begin
 GeraXML := True;
 btnGeraClick(Sender);
 GeraXML := False;
...
procedure TFrmAltera_CTe.Grid_NotasKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
InputString: String;
begin
...
procedure TFrmAltera_CTe.Grid_NotasKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
