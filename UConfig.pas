...
type
  TFrmConfig = class(TForm)
    ...
    CheckBox1: TCheckBox;
    Label81: TLabel;
    FonteObs: TCurrencyEdit;
    Chk_FecharStreaming: TCheckBox;
    Chk_VeiculoNovo: TCheckBox;
    Label82: TLabel;
    ModeloPadrao: TEdit;
    QConfig: TFDQuery;
    Hora_Consiste: TMaskEdit;
    Label83: TLabel;
    H_Inicial: TMaskEdit;
    chk_RemarcaNovo: TCheckBox;
    Estoque_Negativo_Orc_Interno: TCheckBox;
    Libera_Valor_Unitario_OS: TCheckBox;
    chk_desabilitarimpostos: TCheckBox;
    Label84: TLabel;
    VALOR_SANGRIA: TRxCalcEdit;
...
begin
  ...
  QConfig.Close;
  QConfig.ParamByName('EMPRESA_ID').AsInteger := FrmData.QAcesso.FieldByName('EMPRESA_ID').AsInteger;
  QConfig.Prepare;
  QConfig.Open;

  Conta_Estoque.Value     := QConfig.FieldByName('CONTA_ESTOQUE').AsInteger;
  Conta_Estoque_Ent.Value := QConfig.FieldByName('CONTA_ESTOQUE_ENT').AsInteger;
  Conta_Consumo.Value     := QConfig.FieldByName('CONTA_CONSUMO').AsInteger;
  Cod_Cta.Value           := QConfig.FieldByName('COD_CTA').AsInteger;

  if LeIni(Arq_Ini, 'Sistema', 'Caixa') <> '' then
    Caixa.Value := StrToInt(LeIni(Arq_Ini, 'Sistema', 'Caixa'));

  Tam_Barra.Value        := QConfig.FieldByName('TAM_BARRAS').AsInteger;
  Tam_Pesavel.Value      := QConfig.FieldByName('TAM_PESAVEIS').AsInteger;
  Desc_Maximo.Value      := QConfig.FieldByName('DESC_MAXIMO').AsFloat;
  Simples_Nacional.Value := QConfig.FieldByName('ALIQ_SIMPLES').AsFloat;
  Dt_Alteracao.Date      := QConfig.FieldByName('DT_ALTERACAO').AsDateTime;
  DT_CONSISTIR.Date      := QConfig.FieldByName('DT_CONSISTIR').AsDateTime;
  dias_consistir.Value   := QConfig.FieldByName('DIAS_CONSISTIR').AsInteger;
  versao_sync.Text       := QConfig.FieldByName('VERSAO_SYNC').AsString;
  versao_streaming.Text  := QConfig.FieldByName('VERSAO_STREAMING').AsString;
  Localizacao_descricao.Text := QConfig.FieldByName('LOCALIZACAO_DESCRICAO').AsString;
  Localizacao_id.Value   := QConfig.FieldByName('LOCALIZACAO_ID').AsInteger;
  Localizacao_descricao_PDV.Text := QConfig.FieldByName('LOCALIZACAO_DESCRICAO_PDV').AsString;
  Localizacao_id_PDV.Value       := QConfig.FieldByName('LOCALIZACAO_ID_PDV').AsInteger;
  Fluxo_Caixa_id.Value           := QConfig.FieldByName('FLUXO_CAIXA_ID').AsInteger;
  SerieNf.Text                   := QConfig.FieldByName('SERIE').AsString;
  ModeloPadrao.Text              := QConfig.FieldByName('MODELOPADRAO').AsString;
  hora_consiste.Text             := QConfig.FieldByName('HORA_CONSISTE').AsString;

  if QConfig.FieldByName('GERA_DIGITO').AsString = 'True' then
    Gera_Digito.Checked := True
  else
    Gera_Digito.Checked := False;

  if QConfig.FieldByName('TEM_BALANCA').AsString = 'True' then
    Balanca.Checked := True
  else
    Balanca.Checked := False;

  if QConfig.FieldByName('PLANO_PROPRIO').AsString = 'True' then
    Plano_Proprio.Checked := True
  else
    Plano_Proprio.Checked := False;

  if QConfig.FieldByName('TURNO').AsString = 'True' then
    Turno.Checked := True
  else
    Turno.Checked := False;

  if QConfig.FieldByName('SPED').AsString = 'True' then
    Sped_Sintegra.Checked := True
  else
    Sped_Sintegra.Checked := False;

  if QConfig.FieldByName('SPED_DESCONTO').AsString = 'True' then
    SPED_CONTRIB_DESCONTO.Checked := True
  else
    SPED_CONTRIB_DESCONTO.Checked := False;

  if QConfig.FieldByName('EXIBE_FOTO').AsString = 'True' then
    Exibir_foto_cliente_produto.Checked := True
  else
    Exibir_foto_cliente_produto.Checked := False;

  if QConfig.FieldByName('SPED_SERVICO').AsString = 'True' then
 ...
 { if QConfig.FieldByName('VER_DESATIVADOS').AsString = 'True' then
  ...
  Cta_Juros.Value        := QConfig.FieldByName('CONTA_JUROS').AsInteger;
  Cta_Desc.Value         := QConfig.FieldByName('CONTA_DESCONTO').AsInteger;
  desc_funrural.Value    := QConfig.FieldByName('DESC_FUNRURAL').AsFloat;
  desc_cooperativa.Value := QConfig.FieldByName('DESC_COOPERATIVA').AsFloat;

  if QConfig.FieldByName('MOSTRAR_CADASTROS').AsString = 'True' then
    Iniciar.Checked := True
  else
    Iniciar.Checked := False;

  if QConfig.FieldByName('BLOQ_DEVEDORES').AsString = 'True' then
    Bloq_Devedores.Checked := True
  else
    Bloq_Devedores.Checked := False;

  if QConfig.FieldByName('CONSULTA_SIMPLES').AsString = 'True' then
    Consulta_simples.Checked := True
  else
    Consulta_simples.Checked := False;

   if QConfig.FieldByName('DEV_PAGTO').AsString = 'True' then
    Chk_Pgto_Devolucao.Checked := True
  else
    Chk_Pgto_Devolucao.Checked := False;

    if QConfig.FieldByName('DEV_RECEB').AsString = 'True' then
    Chk_Pgto_Devolucao_V.Checked := True
  else
    Chk_Pgto_Devolucao_V.Checked := False;

  if QConfig.FieldByName('VDA_PEDV').AsString = 'True' then
    Chk_Vda_Ped.Checked := True
  else
    Chk_Vda_Ped.Checked := False;

  if QConfig.FieldByName('CPR_PEDV').AsString = 'True' then
    Chk_CPR_Ped.Checked := True
  else
    Chk_CPR_Ped.Checked := False;

  if QConfig.FieldByName('DESC_ORC_USU').AsString = 'True' then
    Desc_Orc_Usu.Checked := True
  else
    Desc_Orc_Usu.Checked := False;

  if QConfig.FieldByName('CALC_EST_PED_VDA').AsString = 'True' then
    Calc_Est_Ped_vda.Checked := True
  else
    Calc_Est_Ped_vda.Checked := False;

  if QConfig.FieldByName('CONS_SIMILAR').AsString = 'True' then
    Chk_Similar.Checked := True
  else
    Chk_Similar.Checked := False;

  if QConfig.FieldByName('SPED_ICMS_ST_OUTROS').AsString = 'True' then
    icms_st_sped.Checked := True
  else
    icms_st_sped.Checked := False;

  if QConfig.FieldByName('SPED_ICMS_ST_IPI_MERC').AsString = 'True' then
    icms_st_ipi_merc.Checked := True
  else
    icms_st_ipi_merc.Checked := False;

  if QConfig.FieldByName('ESTOQUE_NEGATIVO').AsString = 'True' then
    Estoque_Negativo.Checked := True
  else
    Estoque_Negativo.Checked := False;

  if QConfig.FieldByName('CARGA_MANUAL_PROD').AsString = 'True' then
    Sync_Produtos_Manual.Checked := True
  else
    Sync_Produtos_Manual.Checked := False;

  if QConfig.FieldByName('CONTROLE_VALIDADE').AsString = 'True' then
    Chk_Controle_Validade.Checked := True
  else
    Chk_Controle_Validade.Checked := False;

  if QConfig.FieldByName('AUDITORIA').AsString = 'True' then
    chk_Auditoria.Checked := True
  else
    chk_Auditoria.Checked := False;

  if QConfig.FieldByName('CONSISTIR_PDV').AsString = 'True' then
  Begin
 ...
 { if LeIni(Arq_Ini, 'Sistema', 'Permitir Estoque Negativo') = 'True' then
  ...
  if QConfig.FieldByName('ALTERAR_PRECO_CUSTO').AsString = 'True' then
  Chk_CustoProdutos.Checked := True
  else
  Chk_CustoProdutos.Checked := False;

  if QConfig.FieldByName('NAO_EXIBIR_F_NO_G').AsString = 'True' then
  Chk_Nao_Exibir_Estoque_Fiscal_No_Gerencial.Checked := True
  else
  Chk_Nao_Exibir_Estoque_Fiscal_No_Gerencial.Checked := False;
  ...
  FonteObs.Text := LeIni(Arq_Ini, 'Sistema', 'Fonte Observação Orçamento');



  if  QConfig.FieldByName('CARGA').AsInteger = 1 then
  Chk_Atualizar_Ecfs.Checked := True
  Else
  Chk_Atualizar_Ecfs.Checked := False;

  if  QConfig.FieldByName('ATUALIZAR_NCM_NF').AsString = 'True' then
  Chk_AtualizarNcm.Checked := True
  Else
  Chk_AtualizarNcm.Checked := False;

  if  QConfig.FieldByName('ATUALIZAR_NCM_NFCE').AsString = 'True' then
  chk_atualizar_ncm_streaming.Checked := True
  Else
  chk_atualizar_ncm_streaming.Checked := False;

  if  QConfig.FieldByName('REM_IMP_CODFAB_XML').AsString = 'True' then
  Chk_Remover_Cdfabri_xml.Checked := True
  Else
  Chk_Remover_Cdfabri_xml.Checked := False;

  if  QConfig.FieldByName('REM_IMP_CODBARRA_XML').AsString = 'True' then
  Chk_Remover_Cdbarras_xml.Checked := True
  Else
  Chk_Remover_Cdbarras_xml.Checked := False;

  if  QConfig.FieldByName('GERAR_BLOCOK').AsString = 'True' then
  Chk_BlocoK.Checked := True
  Else
  Chk_BlocoK.Checked := False;

  if  QConfig.FieldByName('DESATIVAR_CUSTOS').AsString = 'True' then
  chk_DesativarCustos.Checked := True
  Else
  chk_DesativarCustos.Checked := False;

  if  QConfig.FieldByName('VR_UNITARIO_X_QUANTIDADE').AsString = 'True' then
  Chk_unitarioxquantidade.Checked := True
  Else
  Chk_unitarioxquantidade.Checked := False;

  if  QConfig.FieldByName('IGNORAR_FRACIONAMENTO').AsString = 'True' then
  Chk_IgnorarFracionamento.Checked := True
  Else
  Chk_IgnorarFracionamento.Checked := False;

  if  QConfig.FieldByName('VENDA_VEICULOS_NOVOS').AsString = 'True' then
  Chk_VeiculoNovo.Checked := True
  Else
  Chk_VeiculoNovo.Checked := False;

  if  QConfig.FieldByName('CONTINGENCIA').AsInteger = 1 then
  chk_contingencia.Checked := True
  Else
  chk_contingencia.Checked := False;

  if  QConfig.FieldByName('REFORMA_TRIBUTARIA').AsInteger = 1 then
  Chk_ReformaTributaria.Checked := True
  Else
  Chk_ReformaTributaria.Checked := False;

  if  QConfig.FieldByName('CADASTRO_XML').AsString = 'True' then
  Chk_cadastro_xml.Checked := True
  Else
  Chk_cadastro_xml.Checked := False;

  if  QConfig.FieldByName('ESTOQUE_FILIAIS').AsString = 'True' then
  chk_estoque_Filiais.Checked := True
  Else
  chk_estoque_Filiais.Checked := False;

  if  QConfig.FieldByName('GERAR_BLOCO1400').AsString = 'True' then
  Chk_bl1400.Checked := True
  Else
  Chk_bl1400.Checked := False;

  if  QConfig.FieldByName('CONSISTIR_GERENCIAL').AsString = 'True' then
  chk_consisitr.Checked := True
  Else
  chk_consisitr.Checked := False;

  if  QConfig.FieldByName('DIFERENCA_ICMS').AsString = 'True' then
  CHK_DIF_ICMS.Checked := True
  Else
  CHK_DIF_ICMS.Checked := False;

  if  QConfig.FieldByName('TOTALIZADOR_G').AsString = 'True' then
  Chk_Totalizar_G.Checked := True
  Else
  Chk_Totalizar_G.Checked := False;

  if  QConfig.FieldByName('REPETIR_ITENS_ORC').AsString = 'True' then
  Chk_Repetir_Itens.Checked := True
  Else
  Chk_Repetir_Itens.Checked := False;

  if  QConfig.FieldByName('FILTRO_PLANO').AsString = 'True' then
  Chk_filtro_plano.Checked := True
  Else
  Chk_filtro_plano.Checked := False;

  if  QConfig.FieldByName('DESC_OPERACOES_CONTRIBUICOES').AsString = 'True' then
  chk_desc_operacao.Checked := True
  Else
  chk_desc_operacao.Checked := False;

  if  QConfig.FieldByName('POSICAO_FIM_MES').AsString = 'True' then
  Chk_PosicaoFim.Checked := True
  Else
  Chk_PosicaoFim.Checked := False;

  if  QConfig.FieldByName('CONSISTIR_ABERTURA').AsString = 'True' then
  Chk_Consistir.Checked := True
  Else
  Chk_Consistir.Checked := False;


  if  QConfig.FieldByName('IMPRESSAO_NORMAL_ORCAMENTO').AsString = 'True' then
  Chk_Impressao_Normal.Checked := True
  Else
  Chk_Impressao_Normal.Checked := False;


  if  QConfig.FieldByName('BLOQ_DESC_ORCAMENTO').AsString = 'True' then
  Chk_Bloq_Desc_Orcamento.Checked := True
  Else
  Chk_Bloq_Desc_Orcamento.Checked := False;


  if  QConfig.FieldByName('IMPORTAR_BOLETOS_XML').AsString = 'True' then
  chk_parcelas_Compras.Checked := True
  Else
  chk_parcelas_Compras.Checked := False;

  if  QConfig.FieldByName('DESCONTOS_PRODUTOS').AsString = 'True' then
  chk_desconto_produto.Checked := True
  Else
  chk_desconto_produto.Checked := False;

  if  QConfig.FieldByName('COMISSAO_PRODUTOS').AsString = 'True' then
  chk_comissao_produto.Checked := True
  Else
  chk_comissao_produto.Checked := False;


  if QConfig.FieldByName('FECHAR_STREAMING').AsString = 'True' then
  chk_FecharStreaming.Checked := True
  Else
  chk_FecharStreaming.Checked := False;

  if QConfig.FieldByName('REMARCA_NOVO').AsString = 'True' then
  chk_RemarcaNovo.Checked := True
  Else
  chk_RemarcaNovo.Checked := False;

  if QConfig.FieldByName('ESTOQUE_NEGATIVO_ORC_INTERNO').AsString = 'True' then
  Estoque_Negativo_Orc_Interno.Checked := True
  Else
  Estoque_Negativo_Orc_Interno.Checked := False;

  if QConfig.FieldByName('LIBERA_VALOR_UNITARIO_OS').AsString = 'True' then
  Libera_Valor_Unitario_OS.Checked := True
  Else
  Libera_Valor_Unitario_OS.Checked := False;

  if QConfig.FieldByName('PRECO_ATACADO_LISTA_PRODUTOS').AsString = 'True' then
    Chk_PrecoAtacado.Checked := True
  Else
    Chk_PrecoAtacado.Checked := False;

  if QConfig.FieldByName('DESABILITAR_IMPOSTOS').AsString = 'True'  then
  ...
  //Orçamento
   ...
   user_auth.Text     := DecryptMsg(QConfig.FieldByName('WEB_HTTP_USER').AsString, 65);

   //Connecta
   Login_Connecta.Text := QConfig.FieldByName('LOGIN_CONNECTA').AsString;
   Senha_connecta.Text := DecryptMsg(QConfig.FieldByName('SENHA_CONNECTA').AsString, 65);
   ID_Connecta.Text    := QConfig.FieldByName('ID_CONNECTA').AsString;

   //Servidor Eficaz Filiais
   Banco_Nuvem.Text    := QConfig.FieldByName('BANCO_NUVEM').AsString;
   Senha_Nuvem.Text    := QConfig.FieldByName('SENHA_NUVEM').AsString;
   Porta_Nuvem.Text    := QConfig.FieldByName('PORTA_NUVEM').AsString;
   IP_Nuvem.Text       := QConfig.FieldByName('IP_NUVEM').AsString;
   USUARIO_Nuvem.Text  := QConfig.FieldByName('USUARIO_NUVEM').AsString;


  Caixa_id.Text := LeIni(Arq_Ini, 'Caixas', 'Total');


  Objeto_Ordem.Text       := QConfig.FieldByName('OBJETO_ORDEM').AsString;
  Carencia.Value          := QConfig.FieldByName('CARENCIA').AsInteger;
  Juros_Mensal.Value      := QConfig.FieldByName('JUROS_MENSAL').AsFloat;
  Multa.Value             := QConfig.FieldByName('MULTA').AsFloat;
  Desconto_Mensal.Value   := QConfig.FieldByName('DESCONTO_MENSAL').AsFloat;
  Dias_Pedido.Value       := QConfig.FieldByName('DIAS_PEDIDO').AsInteger;
  Versao_Nfe.Text         := QConfig.FieldByName('VERSAO_NFE').AsString;
  Versao_Mdfe.Text        := QConfig.FieldByName('VERSAO_MDFE').AsString;
  Cod_ins_trib.Value      := QConfig.FieldByName('COD_INS_TRIB').AsInteger;
  COD_ATV_ECON.Text       := QConfig.FieldByName('COD_ATV_ECON').AsString;
  TimeOut_Ecfs.Value      := QConfig.FieldByName('TIMEOUT_ECFS').AsInteger;
  h_inicio_s.Text         := QConfig.FieldByName('H_INICIO_S').AsString;
  h_fim_s.Text            := QConfig.FieldByName('H_FIM_S').AsString;
  Func_Salao.Value        := QConfig.FieldByName('FUNC_SALAO').AsInteger;
  ...
  dias_perda.value          := QConfig.FieldByName('DIAS_PERDA').AsInteger;
  Autorizacao.Text          := QConfig.FieldByName('AUTORIZACAO').AsString;
  Casas_Decimais.Text       := QConfig.FieldByName('CASAS_DECIMAIS_DANFE').AsString;
  Time_Streaming.Value      := QConfig.FieldByName('TIME_STREAMING').AsInteger;
  CHAT.Text                 := QConfig.FieldByName('CHAT').AsString;
  Email_Nfce.Text           := QConfig.FieldByName('EMAIL_NFCE').AsString;
  Margem_Lucro.Value        := QConfig.FieldByName('MARGEM_LUCRO').AsFloat;
  Raw_Orcamento.Text        := LeIni(Arq_Ini, 'Sistema', 'RAW Código Orçamento');
  Raw_Recebimento.Text      := LeIni(Arq_Ini, 'Sistema', 'RAW Recebimento');
  Vr_Pontos.Value           := QConfig.FieldByName('VR_PONTOS').AsFloat;
  Valor_Sangria.Value      := QConfig.FieldByName('VALOR_SANGRIA').AsFloat;
  ...
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 4'));
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 5'));
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 6'));
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 7'));
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 8'));
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 9'));
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 10'));
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 11'));
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 12'));
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 13'));
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 14'));
  Mensagem_ordem.Lines.Add(LeIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 15'));
  ...
  T := 192;
    ...
    QArq.Open;
     ...
     Label44.caption :=  'Dias Off-Line : ' + DecryptMsg(QConfig.FieldByName('DIAS_OFFLINE').ASSTRING,33);
     Label46.caption :=  'On-Line : ' + DecryptMsg(QConfig.FieldByName('DT_CONEXAO').ASSTRING,33);


 if UpperCase(DecryptMsg(QConfig.FieldByName('VERSAO').AsString, 65)) = 'LIGHT' then
 Begin
  ...
  try
    ...
    begin
      try
        ...
        QArq.ParamByName('SERIE').AsString                     := StrZero(SerieNf.Text,3,0);
        QArq.ParamByName('EMPRESA_ID').AsInteger               := FrmData.QAcesso.FieldByName('EMPRESA_ID').AsInteger;
        QArq.ParamByName('HORA_CONSISTE').AsString             := Hora_Consiste.Text;

        ModeloPadrao.Text := StrZero(ModeloPadrao.Text,2,0);
        if ModeloPadrao.Text = '00' then
          ModeloPadrao.Text := '';
        QArq.ParamByName('MODELOPADRAO').AsString              := ModeloPadrao.Text;
        ...
        if Chk_VeiculoNovo.Checked then
        QArq.ParamByName('VENDA_VEICULOS_NOVOS').AsString := 'True'
        Else
        QArq.ParamByName('VENDA_VEICULOS_NOVOS').AsString := 'False';
        ...
        if chk_ReformaTributaria.Checked then
        QArq.ParamByName('REFORMA_TRIBUTARIA').AsInteger := 1
        Else
        QArq.ParamByName('REFORMA_TRIBUTARIA').AsInteger := 0;
        ...
        if Chk_Comissao_Produto.Checked then
        QArq.ParamByName('COMISSAO_PRODUTOS').AsString    := 'True'
        Else
        QArq.ParamByName('COMISSAO_PRODUTOS').AsString    := 'False';

        if Chk_FecharStreaming.Checked then
        QArq.ParamByName('FECHAR_STREAMING').AsString    := 'True'
        Else
        QArq.ParamByName('FECHAR_STREAMING').AsString    := 'False';

        if chk_RemarcaNovo.Checked then
        QArq.ParamByName('REMARCA_NOVO').AsString    := 'True'
        Else
        QArq.ParamByName('REMARCA_NOVO').AsString    := 'False';

        if Estoque_Negativo_Orc_Interno.Checked then
        QArq.ParamByName('ESTOQUE_NEGATIVO_ORC_INTERNO').AsString    := 'True'
        Else
        QArq.ParamByName('ESTOQUE_NEGATIVO_ORC_INTERNO').AsString    := 'False';

        if Chk_CustoProdutos.Checked then
        begin
          GravaIni(Arq_Ini, 'Sistema', 'Alterar Preço de Custo Produtos', 'True');
          QArq.ParamByName('ALTERAR_PRECO_CUSTO').AsString    := 'True';
        end
        else
        begin
          GravaIni(Arq_Ini, 'Sistema', 'Alterar Preço de Custo Produtos', 'False');
          QArq.ParamByName('ALTERAR_PRECO_CUSTO').AsString    := 'False';
        end;

        if Chk_Nao_Exibir_Estoque_Fiscal_No_Gerencial.Checked then
        begin
          GravaIni(Arq_Ini, 'Sistema', 'Não Exibir Estoque F no G', 'True');
          QArq.ParamByName('NAO_EXIBIR_F_NO_G').AsString    := 'True';
        end
        else
        begin
          GravaIni(Arq_Ini, 'Sistema', 'Não Exibir Estoque F no G', 'False');
          QArq.ParamByName('NAO_EXIBIR_F_NO_G').AsString    := 'False';
        end;

        if Libera_Valor_Unitario_OS.Checked then
        QArq.ParamByName('LIBERA_VALOR_UNITARIO_OS').AsString := 'True'
        Else
        QArq.ParamByName('LIBERA_VALOR_UNITARIO_OS').AsString := 'False';

        if Chk_PrecoAtacado.Checked then
          QArq.ParamByName('PRECO_ATACADO_LISTA_PRODUTOS').AsString := 'True'
        Else
          QArq.ParamByName('PRECO_ATACADO_LISTA_PRODUTOS').AsString := 'False';

        if chk_desabilitarimpostos.Checked then
        ...
        QArq.ParamByName('VALOR_SANGRIA').AsFloat    := Valor_Sangria.Value;
     ...
     { if Estoque_Negativo.Checked then
      ...
      GravaIni(Arq_Ini, 'Sistema', 'Fonte Observação Orçamento', FonteObs.Text);
      ...
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 4', '');
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 5', '');
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 6', '');
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 7', '');
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 8', '');
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 9', '');
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 10', '');
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 11', '');
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 12', '');
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 13', '');
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 14', '');
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 15', '');
      ...
      if Mensagem_Ordem.Lines[4] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 4', Mensagem_Ordem.Lines[4]);
      if Mensagem_Ordem.Lines[5] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 5', Mensagem_Ordem.Lines[5]);
      if Mensagem_Ordem.Lines[6] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 6', Mensagem_Ordem.Lines[6]);
      if Mensagem_Ordem.Lines[7] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 7', Mensagem_Ordem.Lines[7]);
      if Mensagem_Ordem.Lines[8] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 8', Mensagem_Ordem.Lines[8]);
      if Mensagem_Ordem.Lines[9] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 9', Mensagem_Ordem.Lines[9]);
      if Mensagem_Ordem.Lines[10] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 10', Mensagem_Ordem.Lines[10]);
      if Mensagem_Ordem.Lines[11] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 11', Mensagem_Ordem.Lines[11]);
      if Mensagem_Ordem.Lines[12] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 12', Mensagem_Ordem.Lines[12]);
      if Mensagem_Ordem.Lines[13] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 13', Mensagem_Ordem.Lines[13]);
      if Mensagem_Ordem.Lines[14] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 14', Mensagem_Ordem.Lines[14]);
      if Mensagem_Ordem.Lines[15] <> '' Then
      GravaIni(Arq_Ini, 'Sistema', 'Mensagem Ordem Serviço 15', Mensagem_Ordem.Lines[15]);
      ...
      QConfig.Close;
      QConfig.Open;
      FrmPrincipal.AtualizaCertificado;
    ...
    while QConfig.FieldByName('EMPRESA_ID').AsInteger <> FrmPrincipal.QEmpresa.FieldByName('EMPRESA_ID').AsInteger do
    begin
      QConfig.Next;
    end;
