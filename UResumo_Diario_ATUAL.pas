...
begin
  ...
  begin
    ...
    //  QRel.Sql.Add('AND ((TRANSPARCELAS.BANCO_ID = 0) OR (TRANSPARCELAS.BANCO_ID IS NULL))');
      ...
      Begin
      QRel.Sql.Clear;
      QRel.Sql.Add('SELECT TRANSACOES.*, FINALIZADORAS.DESCRICAO NOME, FINALIZADORAS.TIPO_TRANSACAO, 0 PARCELAS , 0 BANCO_ID_PAGTO FROM TRANSACOES ');
      ...
      begin
        ...
        Begin
        if (QRel.FieldByName('CONDUTA').AsString = '01') and (QRel.FieldByName('DEPTO').AsString = '07') and (QRel.FieldByName('BANCO_ID_PAGTO').AsInteger > 0) and (QRel.FieldByName('TIPO_VENDA').AsString = 'VENDA') then
        ...
        if (((QRel.FieldByName('CONDUTA').AsString = '01') OR (QRel.FieldByName('CONDUTA').AsString = '02') ) and (QRel.FieldByName('DEPTO').AsString = '07') and (QRel.FieldByName('COND_PAGTO').AsString = 'A PRAZO')) then
        Begin
        {Table_Temp.FieldByName('VALOR').AsFloat        := QRel.FieldByName('VALOR_REC').AsFloat;
        (NAO APARECER VENDAS QUITADAS SOLICITAÇÃO Wagner/José Afonso via Telefone 04/08/14}
      ...
      Begin
      QRel.Sql.Clear;
      QRel.Sql.Add('SELECT TRANSACOES.*,  TRANSPARCELAS.VALOR AS VR_RECEBIMENTO, TRANSPARCELAS.DT_VENCIMENTO ');
