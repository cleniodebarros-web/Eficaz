...
type
  TFrmEmpresas = class(TForm)
    ...
    EMPRESA_ID_MATRIZ: TCurrencyEdit;
    Label46: TLabel;
    OpenPictureDialog1: TOpenPictureDialog;
    CHK_SYNC_APP: TCheckBox;
    SYNC_APP: TEdit;
...
begin
  ...
  if Imagem.Width > 0 then
    Sql := Sql + ', LOGO_STREAM = :LOGO_STREAM';
  ...
  begin
    ...
    Imagem.Graphic.SaveToStream(Stream);
    IQuery.ParamByName('LOGO_STREAM').LoadFromStream(Stream, FTBlob);
  end;
...
begin
  ...
  if (CRT.Text <> '1 - SIMPLES NACIONAL') and (CRT.Text <> '2 - SIMPLES NACIONAL - EXC. REC. BRUTA') and (CRT.Text <> '3 - REGIME NORMAL - LUCRO PRESUMIDO') and (CRT.Text <> '4 - REGIME NORMAL - LUCRO REAL')  and (CRT.Text <> '5 - SIMPLES NACIONAL - SUBLIMITE') then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if Temp is TCurrencyEdit then
        TCurrencyEdit(Temp).Value := QTabela.FieldByName(TCurrencyEdit(Temp).Name).AsInteger;
  ...
  if SYNC_APP.Text = 'sim' then
    CHK_SYNC_APP.Checked := True
  else
    CHK_SYNC_APP.Checked := False;
...
procedure TFrmEmpresas.btnLogoClick(Sender: TObject);
begin
FrmPrincipal.OpenDialog1.Title      := 'Selecione o Logo';
FrmPrincipal.OpenDialog1.InitialDir := ExtractFilePath(ParamStr(0));
FrmPrincipal.OpenDialog1.DefaultExt := '*.jpg';
FrmPrincipal.OpenDialog1.Filter     := 'Arquivos jpg (*.jpg)|*.jpg|Todos os Arquivos (*.*)|*.*';

 if FrmPrincipal.OpenDialog1.Execute then
 Begin
   Logo_Empresa.Text := FrmPrincipal.OpenDialog1.FileName;
   Imagem.LoadFromFile(Logo_Empresa.Text);
 End;
