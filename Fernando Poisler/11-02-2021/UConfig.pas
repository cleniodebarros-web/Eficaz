...
type
  TFrmConfig = class(TForm)
    ...
    Chk_Juntar_Fiscal_Gerencial_Relatorios: TCheckBox;
    ...
    procedure chk_basepiscofinsClick(Sender: TObject);
    procedure Tipo_CargaClick(Sender: TObject);
    procedure FormShow(Sender: TObject);
    procedure TabSheet5Show(Sender: TObject);
    procedure ts1Show(Sender: TObject);
    procedure chk_Controle_Estq_LocClick(Sender: TObject);
...
begin
 ...
 { if LeIni(Arq_Ini, 'Sistema', 'Permitir Estoque Negativo') = 'True' then
  ...
  if LeIni(Arq_Ini, 'Sistema', 'Totalizar G-F em Usuario Gerencial') = 'True' then
    Chk_Juntar_Fiscal_Gerencial_Relatorios.Checked := True
  else
    Chk_Juntar_Fiscal_Gerencial_Relatorios.Checked := False;
...
begin
  ...
  chk_basepiscofins.Checked := True
  Else
  chk_basepiscofins.Checked := False;
...
begin
  try
    Conta_Estoque.Value := GetConsulta('PLANO', 0, 0, StrToInt(Conta_Estoque.Text));
  except
    Conta_Estoque.Value := GetConsulta('PLANO', 0, 0, 0);
  end;
end;
  ...
  try
    ...
    begin
      try
        ...
        QArq.SQL.Add('ATUALIZAR_NCM_NF = :ATUALIZAR_NCM_NF,ALT_BASE_PISCOFINS = :ALT_BASE_PISCOFINS, TIMEOUT_ECFS = :TIMEOUT_ECFS, H_INICIO_S = :H_INICIO_S, H_FIM_S = :H_FIM_S,FUNC_SALAO = :FUNC_SALAO,');
        ...
        QArq.SQL.Add('GERAR_BLOCO1400 = :GERAR_BLOCO1400, CONSISTIR_GERENCIAL = :CONSISTIR_GERENCIAL, DIFERENCA_ICMS = :DIFERENCA_ICMS');
        ...
        if chk_basepiscofins.Checked then
        QArq.ParamByName('ALT_BASE_PISCOFINS').AsString := 'True'
        Else
        QArq.ParamByName('ALT_BASE_PISCOFINS').AsString := 'False';
        {
        if Chk_Controle_Estq_Loc.Checked then
        QArq.ParamByName('CONTROLE_ESTQ_LOC').AsString := 'True'
        Else
        QArq.ParamByName('CONTROLE_ESTQ_LOC').AsString := 'False';
        }
     ...
     { if Estoque_Negativo.Checked then
      ...
      if Chk_Juntar_Fiscal_Gerencial_Relatorios.Checked then
        GravaIni(Arq_Ini, 'Sistema', 'Totalizar G-F em Usuario Gerencial', 'True')
      else
        GravaIni(Arq_Ini, 'Sistema', 'Totalizar G-F em Usuario Gerencial', 'False');
...
procedure TFrmConfig.chk_basepiscofinsClick(Sender: TObject);
begin
 if (not Carrega_Dados) and (chk_basepiscofins.Checked) then
 Begin
 Application.CreateForm(TFrmPasswordDlg, FrmPasswordDlg);
  try
    if FrmPasswordDlg.ShowModal = mrOk then
...
procedure TFrmConfig.chk_Controle_Estq_LocClick(Sender: TObject);
begin
  {
  if Chk_Controle_Estq_Loc.Checked Then
  Begin
