...
begin
  ...
  Peso_Total.Value := Peso_Total.Value - QSub_Detail.FieldByName('PESO_UNITARIO').AsFloat;
end;
...
begin
  ...
  if (FrmPrincipal.Config.FieldByName('ESTOQUE_NEGATIVO_ORC_INTERNO').AsString = 'False')
              ...
              or (QProduto.FieldByName('QUANTIDADE_G').AsFloat - QSub_Detail.FieldByName('QUANTIDADE').AsFloat < 0))
  then
  begin
    Application.MessageBox(PWideChar('Não há estoque para este produto.'
                          ...
                          + #13 + 'Quantidade lançada: ' + QSub_Detail.FieldByName('QUANTIDADE').AsString), PChar(Msg_Title), MB_ICONWARNING);
  ...
  Begin
   ...
   Begin
    if Roundto(QSub_Detail.FieldByName('VR_TOTAL').AsFloat,-2) <> Roundto(((QSub_Detail.FieldByName('VR_UNITARIO').AsFloat - QSub_Detail.FieldByName('VR_DESCONTO').AsFloat) * QSub_Detail.FieldByName('MT_2').AsFloat), -2) then
    begin
      Application.MessageBox('Valor Total inválido', PChar(Msg_Title), mb_IconStop);
      Itens.SelectedIndex := 4;
      abort;
...
begin
  ...
  if FrmPrincipal.Config.FieldByName('OBJETO_ORDEM').AsString = 'Madereira' then
  begin
    RDprint1.Imp(linha,01,'Peso Total: ');
    RDprint1.ImpVal(linha,13,'##,##0.00',QTabela.FieldByName('PESO_TOTAL').AsFloat,[]);
  end;
 ...
 { Else if LeIni(Arq_Ini, 'Sistema', 'Imp. Orçamento') = 'RDPrint MP-4200' then
  ...
  begin
     ...
     { if Application.MessageBox('Deseja imprimir o comprovante?', PChar(Msg_Title), mb_YesNo + mb_IconQuestion + mb_DefButton2) = idYes then
      ...
      {  if peso_orcamento > 0 Then
        ...
        if FrmPrincipal.Config.FieldByName('OBJETO_ORDEM').AsString = 'Madereira' then
        begin
          Writeln(MyFile, 'Peso Total: ' + FormatFloat('#,##0.00',QTabela.FieldByName('PESO_TOTAL').AsFloat));
        end;
        Writeln(MyFile, 'Forma Pagamento: ' + QTabela.FieldByName('COND_PAGTO').AsString);
        Writeln(MyFile, '');
        Writeln(MyFile, '');
  ...
  Begin
      ...
      if FrmPrincipal.Config.FieldByName('OBJETO_ORDEM').AsString = 'Madereira' then
      begin
        QRLabel84.Visible  := True;
        QRDBText65.Visible := True;
        QRLabel85.Visible  := True;
        QRDBText66.Visible := True;
        QRLabel86.Visible  := True;
        QRDBText67.Visible := True;
      end
      else
      begin
        QRLabel84.Visible  := False;
        QRDBText65.Visible := False;
        QRLabel85.Visible  := False;
        QRDBText66.Visible := False;
        QRLabel86.Visible  := False;
        QRDBText67.Visible := False;
      end;
      ...
      begin
        ...
        begin
         ...
         { if Dinheiro.Value > 0 then
          ...
          if FrmPrincipal.Config.FieldByName('OBJETO_ORDEM').AsString = 'Madereira' then
          begin
            Writeln(MyFile, FormatFloat('#,##0.00', QTabela.FieldByName('PESO_TOTAL').AsFloat));
            Writeln(MyFile, '');
          end;
  ...
  begin
      ...
      begin
        ...
        Begin
          if QProduto.FieldbyName('FRACAO_VENDA').AsFloat > 0 then
            ...
            QSub_Detail.FieldByName('MT_2').AsFloat       := QSub_Detail.FieldByName('QUANTIDADE').AsFloat;
        End
        else
        if QProduto.FieldbyName('FRACAO_VENDA').AsFloat > 0 then
        ...
        Peso_Total.Value := Peso_Total.Value - QSub_Detail.FieldByName('PESO_UNITARIO').AsFloat;
        QSub_Detail.FieldByName('PESO_UNITARIO').AsFloat := QSub_Detail.FieldByName('QUANTIDADE').AsFloat * QProduto.FieldbyName('PESO_UNITARIO').AsFloat;
        Peso_Total.Value := Peso_Total.Value + QSub_Detail.FieldByName('PESO_UNITARIO').AsFloat;
      end;
    end;
    ...
    Begin
      if QProduto.FieldbyName('FRACAO_VENDA').AsFloat > 0 then
        ...
        QSub_Detail.FieldByName('QUANTIDADE').AsFloat := QSub_Detail.FieldByName('MT_2').AsFloat;
      Peso_Total.Value := Peso_Total.Value - QSub_Detail.FieldByName('PESO_UNITARIO').AsFloat;
      QSub_Detail.FieldByName('PESO_UNITARIO').AsFloat := QSub_Detail.FieldByName('QUANTIDADE').AsFloat * QProduto.FieldbyName('PESO_UNITARIO').AsFloat;
      Peso_Total.Value := Peso_Total.Value + QSub_Detail.FieldByName('PESO_UNITARIO').AsFloat;
    End;
    ...
    Begin
      ...
      Desc_por := 0;
        ...
        //begin
        //    Application.MessageBox(Pchar('Vr. Desconto máximo no item permitido para ' +  IQuery.FieldByName('NOME').AsString + ' ' + ': '+ IQuery.FieldByName('DESCONTO_MAX_ITEM').AsString +'%') , PChar(Msg_Title), mb_IconStop);
        //    Itens.SelectedIndex := 5;
        //    abort;
        //end;
          ...
          begin
            ...
            begin
              ...
              try
                ...
                QSub_Detail.FieldByName('VR_TOTAL').AsFloat := Roundto(((QSub_Detail.FieldByName('VR_UNITARIO').AsFloat - QSub_Detail.FieldByName('VR_DESCONTO').AsFloat) * QSub_Detail.FieldByName('MT_2').AsFloat), -2);
                end
                else
                  Itens.SelectedIndex := 6;
                  abort;
...
begin
  ...
  begin
    ...
    //then
    //begin
    //  Application.MessageBox('Não há estoque para este produto.', PChar(Msg_Title), MB_ICONWARNING);
    //  Itens.SelectedIndex := 1;
    //  Abort;
    //end;
  end;
...
begin
  ...
  CmdSelectNull := 'WHERE (ORCAMENTO_ID IS NOT NULL) AND (DT_MOVIMENTO = :DT_MOVIMENTO) AND (EMPRESA_ID = :EMPRESA_ID) AND (COMANDA IS NULL OR COMANDA = '''')';
  ...
  If FrmPrincipal.Config.FieldByName('OBJETO_ORDEM').AsString = 'Madereira' Then
  Begin
    Itens.Columns[4].Visible  := True;  //mostra a coluna mt_2
    Itens.Columns[10].Visible := True; //mostra a coluna peso
    Itens.Columns[2].Width    := 250;    //diminui o espaço da descrição
    Itens.Columns[10].Width   := 50;    //define o espaço do peso
