...
type
  TFrmXml_Contabilidade = class(TForm)
    Label1: TLabel;
    Dtmen: TDateEdit;
    Dtmai: TDateEdit;
    Bevel1: TBevel;
...
var
  FrmXml_Contabilidade: TFrmXml_Contabilidade;
  ZipFile: TZipFile;
  dir,dest_dir:string;
...
procedure TFrmXml_Contabilidade.Procura;

begin
  QNotas.Sql.Clear;
  QNotas.Sql.Add('SELECT * FROM COMPL_NFISCAL');
  ...
  begin
   ...
   FrmPrincipal.ACBrNFe1.NotasFiscais.GravarXML(FrmPrincipal.ACBrNFe1.Configuracoes.Arquivos.PathNFe + '\' + QNotas.FieldByName('CHAVE_NFE').AsString + '-nfe.Xml' );
   End;
  ...
  begin
    ...
    if  FileExists(FrmPrincipal.ACBrNFe1.Configuracoes.Arquivos.PathNFe + '\' + QNotas.FieldByName('CHAVE_NFE').AsString + '-nfe.Xml') then
   // FileExists(ExtractFilePath(ParamStr(0)) + 'NFe\' + QNotas.FieldByName('CHAVE_NFE').AsString + '-nfe.Xml') then
      ZipFile.Add(FrmPrincipal.ACBrNFe1.Configuracoes.Arquivos.PathNFe + '\' + QNotas.FieldByName('CHAVE_NFE').AsString + '-nfe.Xml');
   // ShowMessage(Dir  + '\' );
  ...
  while not QNotas.Eof do
  begin
    DeleteFile(FrmPrincipal.ACBrNFe1.Configuracoes.Arquivos.PathNFe + '\' + QNotas.FieldByName('CHAVE_NFE').AsString + '-nfe.Xml');
    QNotas.Next;
  end;

end;

procedure TFrmXml_Contabilidade.btnExecutaClick(Sender: TObject);
begin
   ...
   Begin
    ...
    ZipFile.Open(dir  + '_' + FormatDateTime('ddmmyyyy', Dtmen.Date) +  '.Zip',zmWrite);//ZipFile.Open(ExtractFilePath(ParamStr(0)) + 'Bck_Nfe_' + FormatDateTime('ddmmyyyy', Dtmen.Date) + '_' + FormatDateTime('ddmmyyyy', Dtmen.Date) + '.Zip',zmWrite);
...
procedure TFrmXml_Contabilidade.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmXml_Contabilidade.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmXml_Contabilidade.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmXml_Contabilidade.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmXml_Contabilidade.FormCreate(Sender: TObject);
begin
