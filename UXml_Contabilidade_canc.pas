...
type
  TFrmXml_Contabilidade_cancelada = class(TForm)
    Label1: TLabel;
    Dtmen: TDateEdit;
    Dtmai: TDateEdit;
    Bevel1: TBevel;
...
var
  FrmXml_Contabilidade_cancelada: TFrmXml_Contabilidade_cancelada;
  ZipFile: TZipFile;
  dir,dest_dir:string;
...
procedure TFrmXml_Contabilidade_cancelada.Procura;

begin
  QNotas.Sql.Clear;
  QNotas.Sql.Add('SELECT * FROM NOTAS_CANCELADAS');
...
procedure TFrmXml_Contabilidade_cancelada.btnExecutaClick(Sender: TObject);
begin
...
procedure TFrmXml_Contabilidade_cancelada.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmXml_Contabilidade_cancelada.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmXml_Contabilidade_cancelada.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmXml_Contabilidade_cancelada.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmXml_Contabilidade_cancelada.FormCreate(Sender: TObject);
begin
