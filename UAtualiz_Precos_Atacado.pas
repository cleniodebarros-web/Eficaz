...
type
  TFrmAtualiz_Precos_Atacado = class(TForm)
    Itens: TDBGrid;
    Alfabeto: TTabSet;
    Panel1: TPanel;
    btnRetorna: TBitBtn;
...
var
  FrmAtualiz_Precos_Atacado: TFrmAtualiz_Precos_Atacado;
  Produto: Integer;
...
procedure TFrmAtualiz_Precos_Atacado.AlfabetoClick(Sender: TObject);
begin
...
procedure TFrmAtualiz_Precos_Atacado.btnPesquisaClick(Sender: TObject);
var
Condicao: String;
begin
...
procedure TFrmAtualiz_Precos_Atacado.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmAtualiz_Precos_Atacado.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmAtualiz_Precos_Atacado.FormCloseQuery(Sender: TObject;
...
procedure TFrmAtualiz_Precos_Atacado.FormCreate(Sender: TObject);
begin
...
procedure TFrmAtualiz_Precos_Atacado.ItensKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
InputString: String;
begin
...
procedure TFrmAtualiz_Precos_Atacado.ItensKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #46 then
    Key := #44;
...
procedure TFrmAtualiz_Precos_Atacado.ItensTitleClick(Column: TColumn);
begin
...
procedure TFrmAtualiz_Precos_Atacado.QProdutosAfterOpen(DataSet: TDataSet);
begin
TFloatField(QProdutos.FieldByName('PRECO_ATACADO')).DisplayFormat  := '#,##0.00';
TFloatField(QProdutos.FieldByName('QUANT_MAXIMA')).DisplayFormat   := '#,##0.00';
TFloatField(QProdutos.FieldByName('VALOR_COMPRA')).DisplayFormat  := '#,##0.00';
end;

procedure TFrmAtualiz_Precos_Atacado.QProdutosAfterPost(DataSet: TDataSet);
begin
...
procedure TFrmAtualiz_Precos_Atacado.QProdutosBeforeDelete(DataSet: TDataSet);
begin
  abort;
end;

procedure TFrmAtualiz_Precos_Atacado.QProdutosBeforePost(DataSet: TDataSet);
begin
  ...
  begin
    QUpdate.Sql.Clear;
    QUpdate.Sql.Add('UPDATE PRODUTOS SET PRECO_ATACADO = :PRECO_ATACADO, QUANT_MAXIMA = :QUANT_MAXIMA, DT_PRECO = :DT_PRECO, FUNCIONARIO_ID = :FUNCIONARIO_ID ' +
                                         'WHERE ' +
                                         '(FAMILIA_ID = :FAMILIA_ID)' +
                                         'AND (PRODUTO_ID <> :PRODUTO_ID)');

    QUpdate.ParamByName('PRECO_ATACADO').AsFloat := QProdutos.FieldByName('PRECO_ATACADO').AsFloat;
    QUpdate.ParamByName('QUANT_MAXIMA').AsFloat  := QProdutos.FieldByName('QAUNT_MAXIMA').AsFloat;
...
procedure TFrmAtualiz_Precos_Atacado.QProdutosNewRecord(DataSet: TDataSet);
begin
  QProdutos.Cancel;
end;
