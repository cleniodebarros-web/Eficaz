unit URelatorio_Precos_Alterados;
...
type
  TFrmRelatorio_Precos_Alterados = class(TForm)
    Label1: TLabel;
    Dtmen: TDateEdit;
    Dtmai: TDateEdit;
    Bevel1: TBevel;
...
var
  FrmRelatorio_Precos_Alterados: TFrmRelatorio_Precos_Alterados;
...
procedure TFrmRelatorio_Precos_Alterados.btnExecutaClick(Sender: TObject);
begin
// 125
// 325
  ...
  try
    ...
    else if Ordem.Text = 'FUNCIONÁRIO' then
      QRel.Sql.Add('ORDER BY 4, 4')
...
procedure TFrmRelatorio_Precos_Alterados.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmRelatorio_Precos_Alterados.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmRelatorio_Precos_Alterados.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmRelatorio_Precos_Alterados.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmRelatorio_Precos_Alterados.FormCreate(Sender: TObject);
begin
  Dtmen.Date := StrToDate('01/' + Copy(DateToStr(date), 4, 7));
  Dtmai.Date := Ult_Dia_Mes(date);
end;

procedure TFrmRelatorio_Precos_Alterados.OrdemKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
