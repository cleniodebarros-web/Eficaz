...
begin
  ...
  begin
    try
      ...
      QRel.Sql.Clear;
      QRel.Sql.Add('SELECT TRANSACOES.*, CLIENTES.NOME , FINALIZADORAS.TIPO_TRANSACAO, CLIENTES.CLIENTE_ID ');

      if Chk_Parcelas.Checked then
      QRel.Sql.Add(', TRANSPARCELAS.BANCO_ID BANCO_ID_PAGTO, TRANSPARCELAS.VALOR VALOR_REC, TRANSPARCELAS.PARCELA_ID PARCELAS')
      Else
      QRel.Sql.Add(', BANCO_ID BANCO_ID_PAGTO, VALOR VALOR_REC, PARCELA_ID PARCELAS');
    ...
    //  QRel.Sql.Add('AND ((TRANSPARCELAS.BANCO_ID = 0) OR (TRANSPARCELAS.BANCO_ID IS NULL))');
      ...
      Begin
        QRel.Sql.Clear;
        QRel.Sql.Add('SELECT TRANSACOES.*, FINALIZADORAS.DESCRICAO NOME, FINALIZADORAS.TIPO_TRANSACAO, 0 PARCELAS , 0 BANCO_ID_PAGTO,TRANSPARCELAS.VALOR VALOR_REC FROM TRANSACOES ');
      ...
      begin
       ...
       if Chk_parcelas.Checked then
        Begin
        if (QRel.FieldByName('CONDUTA').AsString = '01') and (QRel.FieldByName('DEPTO').AsString = '07') and (QRel.FieldByName('COND_PAGTO').AsString = 'A PRAZO') AND (QRel.FieldByName('BANCO_ID_PAGTO').AsInteger = 0) then
            Sub_Tit := '*** Venda a Prazo por Parcelas ***';
        End
       Else
        Begin
