...
var
  ...
  Total_Valor_BASE, Total_VR_ICMS, Total_VR_Acrescimo, Total_Valor: Real;
...
try
      ...
      QRel.Sql.Clear;
      QRel.Sql.Text := 'SELECT TRANSITENS.TRANSACAO_ID, TRANSITENS.PRODUTO_ID,  TRANSITENS.DESCRICAO AS DESCRICAO_PRODUTO, NCM, CEST, ' +
     ...
     // QRel.ParamByName('STATUS').AsString  := 'A';
      ...
      if Ordem.ItemIndex = 0 then
         QRel.Sql.Add('ORDER BY TRANSITENS.CFOP,TRANSITENS.PRODUTO_ID');

      if Ordem.ItemIndex = 1 then
         QRel.Sql.Add('ORDER BY TRANSITENS.CFOP,TRANSITENS.DESCRICAO');
    ...
    Begin
        ...
        QLayoutFastReport.ParamByName('NOME_RELATORIO').AsString := 'URel_Conferencia_Tributaria_Vendas';
        ...
        Relatorio_Principal.InheritFromTemplate('URel_Conferencia_Tributaria.fr3');

        Relatorio_Principal := FrmPrincipal.Configura_Relatorio_FastReport(Relatorio_Principal, 'Período: ' + Dtmen.Text + ' a ' + Dtmai.Text, 'Relatório Conferência Tributária');


        //Relatorio_Principal.InheritFromTemplate('URel_Conferencia_Tributaria.fr3');

        //Relatorio_Principal.LoadFromFile('C:\Eficaz7\NewEficazPg\Fr3\URel_Conferencia_Tributaria.fr3');
        ...
        // RDprint1.ImpD(linha, C12+5, QRel.FieldByName('CST_PIS_SAIDA').AsString,[]);
         ...
         Total_Valor_BASE := Total_Valor_BASE + QRel.FieldByName('BASE_CALC_ICMS').AsFloat;
         Total_VR_ICMS    := Total_VR_ICMS + QRel.FieldByName('VALOR_ICMS').AsFloat ;
       ...
       RDprint1.ImpF(linha, 45, 'TOTAL   -->>>', [negrito]);
       RDprint1.ImpVal(linha,C15 -5 ,',##,0.00', Total_Valor, []);
       RDprint1.ImpVal(linha,C11-5,',##,##0.00', Total_Valor_BASE, []);
       RDprint1.ImpVal(linha,C12-3,',##,0.00', Total_VR_ICMS, []);
...
procedure TFrmRel_Conferencia_Tributaria_Vendas.Relatorio_PrincipalLoadTemplate(
...
procedure TFrmRel_Conferencia_Tributaria_Vendas.Button1Click(Sender: TObject);
begin
