...
uses
  ...
  IBUpdateSQL, Math, QRCtrls, QuickRpt, RDprint, IBStoredProc,
...
var
  FrmOrcamento_2: TFrmOrcamento_2;
  Operacao: String;
  ID , linha ,linha2,linha3, pagina,Tamanho_pagina,ID_ORC: Integer;
  Desc_por:Real;
...
procedure TFrmOrcamento_2.OrcamentoBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TFrmOrcamento_2.Orcamento_ReduzidoBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure  TFrmOrcamento_2.Ordenar(Valores: tStrings);
var i, j : integer;
    temp : string;
begin
...
procedure TFrmOrcamento_2.CLIENTE_IDExit(Sender: TObject);
begin
  DetailSearch('Cliente');
end;

procedure TFrmOrcamento_2.ConsultaShow(Sender: TObject);
begin
  Botoes_Normal;
end;

procedure TFrmOrcamento_2.Botoes_Disable;
begin
...
procedure TFrmOrcamento_2.Botoes_Editing;
begin
  btnPrior.Enabled     := False;
  btnNext.Enabled      := False;
  btnInsert.Enabled    := False;
...
procedure TFrmOrcamento_2.Botoes_Normal;
begin
  ...
  IQuery.Sql.clear;
  IQuery.Sql.Add('SELECT CAST(SUM(VR_TOTAL) AS NUMERIC(15,2)) VALOR_TOTAL');
  IQuery.sql.Add('FROM ORCITENS');
  IQuery.sql.Add('WHERE ORCAMENTO_ID = :ORCAMENTO_ID');
  ...
  if (roundto(IQuery.FieldByName('VALOR_TOTAL').AsFloat - VR_DESCONTO.VALUE + VR_ACRESCIMO.VALUE ,-2) > roundto(VALOR.VALUE,-2)) OR  (roundto(IQuery.FieldByName('VALOR_TOTAL').AsFloat - VR_DESCONTO.VALUE + VR_ACRESCIMO.VALUE ,-2) < roundto(VALOR.VALUE,-2)) then
...
procedure TFrmOrcamento_2.DetailSearch(Tabela: String);
begin
...
procedure TFrmOrcamento_2.DiasClick(Sender: TObject);
begin
...
procedure TFrmOrcamento_2.DT_ORCAMENTOEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmOrcamento_2.DT_ORCAMENTOKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmOrcamento_2.Habilitar(Status: Boolean);
var
I: Integer;
Temp: TComponent;
begin
  ...
  begin
    ...
    begin
      IQuery.sql.Clear;
      IQuery.sql.Add('SELECT SUM(VR_DESCONTO) TOTAL_DESCONTO FROM ORCITENS WHERE ORCAMENTO_ID = :ORCAMENTO_ID');
      IQuery.ParamByName('ORCAMENTO_ID').AsInteger := QTabela.FieldByName('ORCAMENTO_ID').AsInteger;
      IQuery.Prepare;
      IQuery.Open;

      if (IQuery.FieldByName('TOTAL_DESCONTO').AsFloat > 0)  then
...
procedure TFrmOrcamento_2.Insert;
var
I: Integer;
Temp: TComponent;
Sql, Par: String;
...
procedure TFrmOrcamento_2.Edit;
var
I: Integer;
Temp: TComponent;
Sql: String;
...
procedure TFrmOrcamento_2.ProdutoSearch;
begin
...
procedure TFrmOrcamento_2.ServicoSearch;
begin
  QServico.Close;
...
procedure TFrmOrcamento_2.TributoSearch;
begin
...
procedure TFrmOrcamento_2.EMPRESA_IDExit(Sender: TObject);
begin
  DetailSearch('Empresa');
end;

procedure TFrmOrcamento_2.EMPRESA_IDKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
function TFrmOrcamento_2.Validacao: Boolean;
begin
  Result := False;
...
procedure TFrmOrcamento_2.VENDEDOR_IDExit(Sender: TObject);
begin
  DetailSearch('Vendedor');
end;

procedure TFrmOrcamento_2.VR_DESCONTOExit(Sender: TObject);
begin
  if FrmPrincipal.Config.FieldByName('DESC_ORC_USU').AsString = 'True' then
  Begin
  Desc_por := 0;
...
procedure TFrmOrcamento_2.ManutencaoShow(Sender: TObject);
begin
  Set_Campos(False);
  Botoes_Normal;
...
procedure TFrmOrcamento_2.OBSERVACAOEnter(Sender: TObject);
begin
Observacao.Text := Observacao.Text + ' ';
end;

procedure TFrmOrcamento_2.QSub_DetailAfterOpen(DataSet: TDataSet);
begin
...
procedure TFrmOrcamento_2.QSub_DetailBeforeDelete(DataSet: TDataSet);
begin
...
procedure TFrmOrcamento_2.QSub_DetailBeforeEdit(DataSet: TDataSet);
begin
...
procedure TFrmOrcamento_2.QSub_DetailBeforeInsert(DataSet: TDataSet);
begin
  if (QTabela.FieldByName('DT_MOVIMENTO').AsDateTime < FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime) then
  begin
    Application.MessageBox('Movimento já encerrado', PChar(Msg_Title), mb_IconStop);
...
procedure TFrmOrcamento_2.QSub_DetailBeforePost(DataSet: TDataSet);
var
Cst: String;
Promocao: Boolean;
begin
  ...
  begin
   ...
   if Roundto(((QSub_Detail.FieldByName('VR_UNITARIO').AsFloat * QSub_Detail.FieldByName('QUANTIDADE').AsFloat) - QSub_Detail.FieldByName('VR_TOTAL').AsFloat) / QSub_Detail.FieldByName('QUANTIDADE').AsFloat , -2) > 0 Then
   QSub_Detail.FieldByName('VR_DESCONTO').AsFloat   := Roundto(((QSub_Detail.FieldByName('VR_UNITARIO').AsFloat * QSub_Detail.FieldByName('QUANTIDADE').AsFloat) - QSub_Detail.FieldByName('VR_TOTAL').AsFloat) / QSub_Detail.FieldByName('QUANTIDADE').AsFloat , -2);
  ...
  Desc_por := 0;
    ...
    begin
      ...
      if IQuery.FieldByName('DESCONTO_MAX_SUBTOTAL').AsFloat > 0 then
      begin
        if (QSub_Detail.FieldByName('VR_DESCONTO').AsFloat * QSub_Detail.FieldByName('QUANTIDADE').AsFloat)  > (((QSub_Detail.FieldByName('VR_UNITARIO').AsFloat * QSub_Detail.FieldByName('QUANTIDADE').AsFloat) * IQuery.FieldByName('DESCONTO_MAX_ITEM').AsFloat) / 100) then
        begin
  ...
  if (Roundto(QSub_Detail.FieldByName('VR_TOTAL').AsFloat,-2) <> RoundTo(((QSub_Detail.FieldByName('VR_UNITARIO').AsFloat - QSub_Detail.FieldByName('VR_DESCONTO').AsFloat) * QSub_Detail.FieldByName('QUANTIDADE').AsFloat) , -2)) AND (QSub_Detail.FieldByName('VR_DESCONTO').AsFloat > 0) then
  begin
    Application.MessageBox('Correção Desconto Unitario!', PChar(Msg_Title), mb_IconStop);
    Itens.SelectedIndex := 4;
    abort;
  end;
...
procedure TFrmOrcamento_2.QSub_DetailNewRecord(DataSet: TDataSet);
begin
  DataSet['ORCAMENTO_ID'] := QTabela.FieldByName('ORCAMENTO_ID').AsInteger;
  DataSet['TP_PROD_SERV'] := 'P';
end;

procedure TFrmOrcamento_2.QTabelaAfterOpen(DataSet: TDataSet);
begin
TFloatField(QTabela.FieldByName('VALOR')).DisplayFormat  := '#,##0.00';
end;

procedure TFrmOrcamento_2.RDprint1BeforeNewPage(Sender: TObject; Pagina: Integer);
begin
...
procedure TFrmOrcamento_2.RDprint1NewPage(Sender: TObject; Pagina: Integer);
begin
...
procedure TFrmOrcamento_2.RDprint2BeforeNewPage(Sender: TObject; Pagina: Integer);
begin
  if linha <= Tamanho_pagina -4   then
  Begin
  RDprint1.Imp(Tamanho_pagina - 3, 01, '==================================');
  RDprint1.Imp(Tamanho_pagina -2 , 01, 'Eficaz Automação e Sistemas Ltda');
  RDprint1.ImpDir(Tamanho_pagina -2, 28, 80, 'http://www.eficazautomacao.com.br', []);
  RDprint1.Imp(Tamanho_pagina,01,' ');
  End;

end;

procedure TFrmOrcamento_2.Set_Campos(Vazio: Boolean);
var
I: Integer;
Temp: TComponent;
begin
  ...
  begin
    ...
    begin
      ...
      Label21.Caption := 'Vr. Prod.  : R$ 0,00';
      Label12.Caption := 'Desc.Itens : R$ 0,00';
    ...
    begin
      ...
      if Temp is TMemo then
       ...
       IQuery.Sql.Clear;
       IQuery.Sql.ADD('SELECT CAST(SUM(VR_UNITARIO * QUANTIDADE) AS NUMERIC(15,2)) VALOR_TOTAL, CAST(SUM(VR_DESCONTO * QUANTIDADE) AS NUMERIC(15,2)) VALOR_DESCONTO');
       IQuery.Sql.ADD('FROM ORCITENS');
       IQuery.Sql.ADD('WHERE ORCAMENTO_ID = :ORCAMENTO_ID');
       ...
       Label21.Caption := 'Vr. Prod.  : R$ '+ FloatToStrF(IQuery.FieldByName('VALOR_TOTAL').asfLOAT, ffFixed, 7, 2);
       Label12.Caption := 'Desc.Itens : R$ '+ FloatToStrF(IQuery.FieldByName('VALOR_DESCONTO').asfLOAT, ffFixed, 7, 2);
...
procedure TFrmOrcamento_2.btnDeleteClick(Sender: TObject);
begin
  if FrmData.QAcesso.FieldByName('EXCLUSAO').AsString = 'NÃO' then
  begin
    Application.MessageBox('Usuário sem permissão para exclusão', PChar(Msg_Title), mb_IconStop);
...
procedure TFrmOrcamento_2.btnDiscardClick(Sender: TObject);
begin
  Botoes_Normal;
  Set_Campos(False);
...
procedure TFrmOrcamento_2.btnEditClick(Sender: TObject);
begin
  if FrmData.QAcesso.FieldByName('ALTERACAO').AsString = 'NÃO' then
  begin
    Application.MessageBox('Usuário sem permissão para alteração', PChar(Msg_Title), mb_IconStop);
  ...
  ID_orc   := QTabela.FieldByName('ORCAMENTO_ID').AsInteger;
...
procedure TFrmOrcamento_2.btnEmpresaClick(Sender: TObject);
begin
  try
...
procedure TFrmOrcamento_2.btnClienteClick(Sender: TObject);
begin
  try
...
procedure TFrmOrcamento_2.btnInsertClick(Sender: TObject);
begin
  ...
  IQuery.SQL.Clear;
  IQuery.SQL.Add('SELECT NEXTVAL(:GEN_ORCAMENTOS) ID');
  IQuery.ParamByName('GEN_ORCAMENTOS').AsString :=  'GEN_ORCAMENTOS';

  IQuery.Prepare;
  IQuery.Open;


  ID_ORC := IQuery.FieldByName('ID').AsInteger;
  ...
  dt_validade.Date  := FrmPrincipal.Abertura.FieldByName('DT_MOVIMENTO').AsDateTime + 7 ;
...
procedure TFrmOrcamento_2.btnLogClick(Sender: TObject);
var
F1: TextFile;
begin
   Set_Campos(False);
...
procedure TFrmOrcamento_2.btnNextClick(Sender: TObject);
begin
  if not QTabela.Eof then
  begin
    QTabela.Next;
...
procedure TFrmOrcamento_2.btnOrcamentoClick(Sender: TObject);
var
...
begin
  ...
  RDprint1.Imp(linha, 56, QCliente.FieldByName('TELEFONE_1').AsString);
  inc(linha);
  ...
  IQuery.Open;
   ...
   RDprint1.LinhaH(linha,45,56,0,False);
   inc(linha);
  ...
  Begin
  RDprint1.ImpF(linha2,01,Copy(QTabela.FieldByName('OBSERVACAO').AsString,1,40),[normal,negrito]);
  inc(linha2);
  inc(linha3);
  End;
  if Copy(QTabela.FieldByName('OBSERVACAO').AsString,41,40) <> '' then
  Begin
  RDprint1.ImpF(linha2,01,Copy(QTabela.FieldByName('OBSERVACAO').AsString,41,40),[negrito]);
  inc(linha2);
  inc(linha3);
  End;
  if Copy(QTabela.FieldByName('OBSERVACAO').AsString,81,40) <> '' then
  Begin
  RDprint1.ImpF(linha2,01,Copy(QTabela.FieldByName('OBSERVACAO').AsString,81,40),[negrito]);
  inc(linha2);
  inc(linha3);
  End;
  if Copy(QTabela.FieldByName('OBSERVACAO').AsString,121,40) <> '' then
  Begin
  RDprint1.ImpF(linha2,01,Copy(QTabela.FieldByName('OBSERVACAO').AsString,121,40),[negrito]);
  inc(linha2);
  inc(linha3);
  End;
  if Copy(QTabela.FieldByName('OBSERVACAO').AsString,161,40) <> '' then
  Begin
  RDprint1.ImpF(linha2,01,Copy(QTabela.FieldByName('OBSERVACAO').AsString,161,40),[negrito]);
  inc(linha2);
  inc(linha3);
  End;
  if Copy(QTabela.FieldByName('OBSERVACAO').AsString,201,40) <> '' then
  Begin
  RDprint1.ImpF(linha2,01,Copy(QTabela.FieldByName('OBSERVACAO').AsString,201,40),[negrito]);
  inc(linha2);
  inc(linha3);
  End;
  ...
  RDprint1.LinhaH(linha3,10,34,0,False);
  RDprint1.Imp(linha3,01,'Vendedor:' );
  RDprint1.LinhaH(linha3,53,77,0,False);
  RDprint1.Imp(linha3,45,'Cliente:' );
  inc(linha3);
          ...
          begin
           ...
           { for Copias := 1 to StrToInt(InputString) do
            begin}
              ...
              begin
                Writeln(MyFile, ForcaComprimento('Sub-Total', 42 - Length(FormatFloat('#,##0.00', Valor.value)), ' ') + FormatFloat('#,##0.00', Valor.value));
                ...
                Writeln(MyFile, ForcaComprimento('Total', 42 - Length(FormatFloat('#,##0.00', Valor.value + Vr_Acrescimo.value - Vr_Desconto.value)), ' ') + FormatFloat('#,##0.00', Valor.value + Vr_Acrescimo.value - Vr_Desconto.value));
              end
              else
                Writeln(MyFile, ForcaComprimento('Total', 42 - Length(FormatFloat('#,##0.00', Valor.value + Vr_Acrescimo.value - Vr_Desconto.value)), ' ') + FormatFloat('#,##0.00', Valor.value + Vr_Acrescimo.value - Vr_Desconto.value));
             ...
             { QParcelas.SQL.Clear;
              QParcelas.SQL.Add('SELECT NOME FROM OPERADORES WHERE OPERADOR_ID = :OPERADOR');
              QParcelas.ParamByName('OPERADOR').AsString := StrZero(IntToStr(operador_caixa), 14, 0);
              QParcelas.Prepare;
              QParcelas.Open;
              ...
              Writeln(MyFile, '');
              Writeln(MyFile, '------------------------------------------');
              Writeln(MyFile, 'Caixa: ' + StrZero(LeIni(Arq_Ini, 'Sistema', 'Caixa'),3,0));
              Writeln(MyFile, '');
              Writeln(MyFile, 'Operador de Caixa: ' + QParcelas.FieldByName('NOME').AsString);
              Writeln(MyFile, '------------------------------------------');}
  ...
  IQuery.Open;
    ...
    if LeIni(Arq_Ini, 'Sistema', 'Imp. Orçamento') = '40 Colunas' then
        begin
          ...
          Else
          if (LeIni(Arq_Ini, 'Sistema', 'Imp. Orçamento') = 'Normal Reduzido')  AND
               (LeIni(Arq_Ini, 'Sistema', 'Largura Papel') > '0') and (LeIni(Arq_Ini, 'Sistema', 'Altura Papel') > '0') then
          Begin
          ...
          End;
          if (LeIni(Arq_Ini, 'Sistema', 'Imp. Orçamento') = 'Normal') OR (LeIni(Arq_Ini, 'Sistema', 'Imp. Orçamento') = 'RDPrint-Reduzido') Then
          begin
...
procedure TFrmOrcamento_2.btnParcelasClick(Sender: TObject);
begin
  if QTabela.FieldByName('VALOR').AsFloat > 0 then
...
procedure TFrmOrcamento_2.btnPesquisaClick(Sender: TObject);
var
Condicao: String;
begin
...
procedure TFrmOrcamento_2.btnPriorClick(Sender: TObject);
begin
  if not QTabela.Bof then
  begin
    QTabela.Prior;
...
procedure TFrmOrcamento_2.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmOrcamento_2.btnSaveClick(Sender: TObject);
begin
  DetailSearch('');
  ...
  begin
    ...
    begin
      ...
      QTabela.Locate('ORCAMENTO_ID',ID_ORC, [loCaseInsensitive]);
   ...
   If (FrmPrincipal.Config.FieldByName('AUDITORIA').AsString = 'True') and (valor.value > 0) Then
    ...
    if (LeIni(Arq_Ini, 'Sistema', 'Path Orçamento') <> '') and (not QSub_Detail.IsEmpty) then
      btnLogClick(Sender);
...
procedure TFrmOrcamento_2.btnVendedorClick(Sender: TObject);
begin
  try
...
procedure TFrmOrcamento_2.DBGrid1DblClick(Sender: TObject);
begin
  Manutencao.Show;
end;

procedure TFrmOrcamento_2.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    Manutencao.Show;
end;

procedure TFrmOrcamento_2.DBGrid1TitleClick(Column: TColumn);
begin
...
procedure TFrmOrcamento_2.ItensColEnter(Sender: TObject);
begin
...
procedure TFrmOrcamento_2.ItensColExit(Sender: TObject);
begin
   if QSub_Detail.State in [dsInsert, dsEdit] then
  begin
    ...
    begin
      ...
      begin
        QSub_Detail.FieldByName('DESCRICAO').AsString    := Copy(QProduto.FieldByName('DESCRICAO').AsString,1,40);
        QSub_Detail.FieldByName('TRIBUTO_ID').AsInteger  := QProduto.FieldByName('TRIBUTO_ID').AsInteger;
        QSub_Detail.FieldByName('ALIQUOTA_ICMS').AsFloat := QProduto.FieldByName('ALIQUOTA_ICMS').AsFloat;
        QSub_Detail.FieldByName('VR_UNITARIO').AsFloat   := QProduto.FieldByName('PRECO_VAREJO').AsFloat;
      end;
    ...
    Begin
        ...
        if (QTabela.FieldByName('VR_DESCONTO').AsFloat > 0) and (QSub_Detail.FieldByName('VR_DESCONTO').AsFloat > 0)     then
        begin
         Application.MessageBox('Não e possível desconto no item: '  + #13  +'Desconto rodapé já aplicado no orçamento.', PChar(Msg_Title), mb_IconStop);
         Itens.SelectedIndex := 5;
         abort;
        end;

        if RoundTo(QSub_Detail.FieldByName('VR_DESCONTO').AsFloat,-2) >= RoundTo((QSub_Detail.FieldByName('VR_UNITARIO').AsFloat * QSub_Detail.FieldByName('QUANTIDADE').AsFloat ),-2) then
        begin
            Application.MessageBox('Desconto inválido', PChar(Msg_Title), mb_IconStop);
            Itens.SelectedIndex := 5;
            abort;
      ...
      Desc_por := 0;
        ...
        if (IQuery.FieldByName('DESCONTO_MAX_ITEM').AsFloat = 0) and (QSub_Detail.FieldByName('VR_DESCONTO').AsFloat > 0) then
        begin
            Application.MessageBox(Pchar('Vr. Desconto no item máximo  permitido para ' +  IQuery.FieldByName('NOME').AsString + ' ' + ': '+ IQuery.FieldByName('DESCONTO_MAX_ITEM').AsString +'%') , PChar(Msg_Title), mb_IconStop);
            Itens.SelectedIndex := 5;
            abort;
        end;
          ...
          begin
            if QSub_Detail.FieldByName('VR_DESCONTO').AsFloat > (((QSub_Detail.FieldByName('VR_UNITARIO').AsFloat * QSub_Detail.FieldByName('QUANTIDADE').AsFloat) * IQuery.FieldByName('DESCONTO_MAX_ITEM').AsFloat) / 100) then
            begin
              Application.MessageBox(Pchar('Vr. Desconto no item máximo permitido para ' +  IQuery.FieldByName('NOME').AsString + ' ' + ': '+ IQuery.FieldByName('DESCONTO_MAX_SUBTOTAL').AsString +'%') , PChar(Msg_Title), mb_IconStop);
              ...
              begin
                ...
                try
                  ...
                  begin
                    if Desc_Por > FrmGerente.QOperador.FieldByName('DESCONTO_MAX_ITEM').AsFloat then
                    begin
                      Application.MessageBox('Desconto no item acima do máximo permitido', 'Orçamento', MB_IconStop + MB_OK);
                     Itens.SelectedIndex := 5;
                     abort;
                    end;
                  Itens.SelectedIndex := 6;
...
procedure TFrmOrcamento_2.ItensEnter(Sender: TObject);
begin
  Botoes_Disable;
  Consulta.TabVisible := False;
end;

procedure TFrmOrcamento_2.ItensExit(Sender: TObject);
begin
  if QSub_Detail.State in [dsInsert, dsEdit] then
  begin
    QSub_Detail.Cancel;
...
procedure TFrmOrcamento_2.ItensKeyDown(Sender: TObject;
...
begin
  ...
  Begin
    ...
    Valor.value :=  (IQuery.FieldByName('VALOR_TOTAL').asfLOAT - IQuery.FieldByName('VALOR_DESCONTO').asfLOAT);
...
procedure TFrmOrcamento_2.ItensKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #46 then
    Key := #44;
...
procedure TFrmOrcamento_2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmOrcamento_2.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if (Operacao = 'Inserindo') or (Operacao = 'Alterando') or (QSub_Detail.State in [dsInsert, dsEdit]) OR
  (Itens.Focused) then
  begin
...
procedure TFrmOrcamento_2.FormCreate(Sender: TObject);
var
X, Ult_Dia: Integer;
begin
  ...
  DBGrid1.Columns[4].Width := 118;
  DBGrid1.Columns[5].Width := 109;
  ...
  Itens.Columns[2].Width  := 330;
  Itens.Columns[3].Width  := 74;
  Itens.Columns[4].Width  := 64;
  Itens.Columns[5].Width  := 78;
  Itens.Columns[6].Width  := 64;
...
procedure TFrmOrcamento_2.FormShow(Sender: TObject);
begin
  DBGrid1.SetFocus;
end;
