...
var
  FrmRel_Orcamentos: TFrmRel_Orcamentos;
...
procedure TFrmRel_Orcamentos.btnExecutaClick(Sender: TObject);
begin
// 125, 325
  try
    ...
    QRel.Sql.Add('ORDER BY ORCAMENTOS.COND_PAGTO, ORCAMENTOS.ORCAMENTO_ID, ORCAMENTOS.DT_ORCAMENTO');
...
procedure TFrmRel_Orcamentos.btnRetornaClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmRel_Orcamentos.DtmenEnter(Sender: TObject);
begin
  Keybd_Event(VK_LEFT, 0, 0, 0);
end;

procedure TFrmRel_Orcamentos.DtmenKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmRel_Orcamentos.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TFrmRel_Orcamentos.FormCreate(Sender: TObject);
begin
  Dtmen.Date := StrToDate('01/' + Copy(DateToStr(date), 4, 7));
  Dtmai.Date := Ult_Dia_Mes(date);
end;

procedure TFrmRel_Orcamentos.Tipo_OrcamentoKeyDown(Sender: TObject;
