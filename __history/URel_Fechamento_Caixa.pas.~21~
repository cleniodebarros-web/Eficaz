...
begin
  try
    ...
    QRel.Sql.Add('FROM (');
    QRel.Sql.Add('SELECT CAIXA_ID CAIXA ,DATA, HORA, LPAD(CUPOM,8,''0'') CUPOM, LEGENDA, CASE WHEN (VALOR_CUPOM ) = VALOR_FINALIZADORA');
      ...
      begin
        ...
        begin
          ...
          Writeln(MyFile, QRel.FieldByName('CUPOM').AsString + '  ' +
                          ...
                          ForcaComprimento(' ', (9 - Length(FormatFloat('#,##0.00', QRel.FieldByName('VALOR_FINALIZADORA').AsFloat))), ' ') +
                          FormatFloat('#,##0.00', QRel.FieldByName('VALOR_FINALIZADORA').AsFloat) + '  ' +
                          ForcaComprimento(' ', (8 - Length(FormatFloat('#,##0.00', QRel.FieldByName('DESCONTO').AsFloat))), ' ') +
...
begin
  ...
  begin
    Writeln(MyFile, '                           ---------------------');
    Writeln(MyFile, ForcaComprimento(' ', 29, ' ') +
                    ForcaComprimento(' ', (9 - Length(FormatFloat('#,##0.00', Vr_Fin))), ' ') +
                    FormatFloat('#,##0.00', Vr_Fin) + '  ' +
                    ForcaComprimento(' ', (8 - Length(FormatFloat('#,##0.00', Vr_Troco))), ' ') +
  ...
  QFechamento.Sql.Add('(');
  QFechamento.Sql.Add('SELECT FECHAMENTO_CUPOM.LEGENDA, TIPO_TRANSACAO, VALOR_CUPOM VALOR , VALOR_FINALIZADORA');
  ...
  QFechamento.Sql.Add('AND (FECHAMENTO_CUPOM.LEGENDA NOT LIKE :LEGENDA)');
  ...
  QFechamento.Sql.Add('SELECT FINALIZADORAS.LEGENDA || '' Recebimento'' LEGENDA, TIPO_TRANSACAO, 0 VALOR, SUM(VALOR) VALOR_FINALIZADORA ');
  //Else
  ...
  QFechamento.Sql.Add('AND (BANCO_ID IN (SELECT BANCO_ID FROM BANCOS WHERE TIPO_LIMITE <> ' +#39 + 'EXCLUSO DO SALDO' + #39' ))');
  ...
  if FrmData.QAcesso.FieldByName('TPCTB').AsString = '3' then
    ...
    QFechamento.ParamByName('FISCAL1').AsString    :=  'S';
  ...
  QSangriaSuprimentoSintetico.Sql.Add('AND (LEGENDA NOT LIKE :LEGENDA)');
  ...
  begin
    ...
    //begin
     ...
     Writeln(MyFile, ForcaComprimento(QFechamento.FieldByName('LEGENDA').AsString, 39, ' ') +
            ForcaComprimento(' ', (9 - Length(FormatFloat('#,##0.00', QFechamento.FieldByName('VALOR_FINALIZADORA').AsFloat))), ' ') +
            FormatFloat('#,##0.00', QFechamento.FieldByName('VALOR_FINALIZADORA').AsFloat));


     Vr_Fin := Vr_Fin +  QFechamento.FieldByName('VALOR_FINALIZADORA').AsFloat;
    //end;

    if (QFechamento.FieldByName('TIPO_TRANSACAO').AsInteger = 1) and (QFechamento.FieldByName('VALOR_FINALIZADORA').AsFloat > 0) then
      Vr_Caixa := Vr_Caixa + QFechamento.FieldByName('VALOR_FINALIZADORA').AsFloat;
  ...
  QFechamento.Sql.Add('AND (LEGENDA NOT LIKE :LEGENDA)');
